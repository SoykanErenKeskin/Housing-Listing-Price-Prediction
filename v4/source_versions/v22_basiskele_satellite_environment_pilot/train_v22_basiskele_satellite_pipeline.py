...
Example:
  python train_v22_basiskele_satellite_pipeline.py --out outputs/v18_basiskele_full \\
...
if __name__ == "__main__":
    ...
    if not _no_interactive:
        sys.stdout.write("V22 satellite environment pilot ayar sihirbazı açılıyor...\n")
        sys.stdout.flush()
...
try:
    from site_project_extraction import (
        SITE_UNIT_PRICE_COL,
...
SAT_MODE_RUNTIME = "none"
SAT_CSV_RUNTIME = "data/external/satellite_features/basiskele/sentinel_features_v22.csv"
SAT_JOIN_REPORT: dict = {}
SAT_MODEL_FEATURES_RUNTIME: list[str] = []

from satellite_feature_builder import (
    get_satellite_feature_names,
    get_model_satellite_feature_names,
    satellite_ablation_experiments,
    normalize_satellite_feature_mode,
)
from satellite_feature_joiner import (
    join_satellite_features,
    resolve_satellite_csv_path,
    estimate_satellite_csv_coverage,
    SAT_COVERAGE_GATE,
)
from diagnostics_v22_satellite import (
    V21_REF,
    COVERAGE_MIN,
    evaluate_satellite_candidate,
    satellite_feature_coverage_table,
    satellite_feature_distribution_table,
    save_satellite_feature_importance,
    select_satellite_experiment,
    write_metrics_summary_v22,
    run_spatial_leakage_diagnostic,
)
...
class RunConfig:
    ...
    satellite_feature_csv: str = "data/external/satellite_features/basiskele/sentinel_features_v22.csv"
    run_satellite_ablation: bool = False
    run_spatial_diagnostic: bool = False
...
def resolve_numeric_features(
    ...
    site_extraction_mode: str = "full",
    site_project_encoding: str = "foldsafe_target",
    satellite_feature_mode: str = "none",
) -> list[str]:
    ...
    names = (
        ...
        + site_names
        + list(SAT_MODEL_FEATURES_RUNTIME)
    )
    seen: set[str] = set()
    out: list[str] = []
    for n in names:
...
def build_preprocessor(
    ...
    cat_features = resolve_categorical_features(site_extraction_mode, site_project_encoding)
...
def build_pipeline(
    ...
    numeric_features = resolve_numeric_features(mode, detail_mode, specialist_mode, lh_regime, kar_mode, location_feature_mode, geo_context_mode, str(comparable_mode or "none"), site_mode, site_enc)
    steps = [
    ...
    steps.extend(
        [
            ("feature_columns", FeatureColumnKeeper(numeric_features, resolve_categorical_features(site_mode, site_enc))),
            (
                "preprocess",
                build_preprocessor(
...
def train_and_evaluate(X: pd.DataFrame, y: pd.Series, cfg: RunConfig, out_dirs: dict[str, Path]) -> tuple[ModelBundle, dict[str, Any]]:
    ...
    write_leakage_guard(out_dirs["reports"] / "site_project_encoding_leakage_guard_v21.json", site_leak)
    ...
    try:
        ...
        if site_mode_d == "v20_parity":
            ...
            pd.DataFrame([{"metric": k, "value": v} for k, v in coverage_metrics.items()]).to_csv(
                out_dirs["reports"] / "site_project_coverage_v21.csv", index=False, encoding="utf-8-sig"
            )
            dictionary_table().to_csv(out_dirs["reports"] / "site_project_dictionary_v21.csv", index=False, encoding="utf-8-sig")
            candidates_table(prem_frame).to_csv(out_dirs["reports"] / "site_project_candidates_v21.csv", index=False, encoding="utf-8-sig")
            pd.DataFrame(columns=["raw_folded", "canonical_id"]).to_csv(
                out_dirs["reports"] / "site_project_alias_map_v21.csv", index=False, encoding="utf-8-sig"
            )
            pd.DataFrame(
                columns=[
                    "raw_name",
            ...
            ).to_csv(out_dirs["reports"] / "site_project_alias_review_needed_v21.csv", index=False, encoding="utf-8-sig")
            pd.DataFrame(
                columns=[
                    "canonical_id",
                    "raw_variants",
            ...
            ).to_csv(out_dirs["reports"] / "site_project_merge_audit_v21.csv", index=False, encoding="utf-8-sig")
        else:
            ...
            coverage_table(adder).to_csv(out_dirs["reports"] / "site_project_coverage_v21.csv", index=False, encoding="utf-8-sig")
            # overwrite with enriched metrics including expensive missing
            pd.DataFrame([{"metric": k, "value": v} for k, v in coverage_metrics.items()]).to_csv(
                out_dirs["reports"] / "site_project_coverage_v21.csv", index=False, encoding="utf-8-sig"
            )
            alias_map_table(adder).to_csv(out_dirs["reports"] / "site_project_alias_map_v21.csv", index=False, encoding="utf-8-sig")
            ...
            review_df.to_csv(out_dirs["reports"] / "site_project_alias_review_needed_v21.csv", index=False, encoding="utf-8-sig")
            ...
            merge_df.to_csv(out_dirs["reports"] / "site_project_merge_audit_v21.csv", index=False, encoding="utf-8-sig")
            dictionary_table().to_csv(out_dirs["reports"] / "site_project_dictionary_v21.csv", index=False, encoding="utf-8-sig")
            candidates_table(prem_frame).to_csv(out_dirs["reports"] / "site_project_candidates_v21.csv", index=False, encoding="utf-8-sig")
            ...
            if "site_project_id" in tmp.columns:
                ...
                perf.to_csv(out_dirs["reports"] / "site_project_performance_v21.csv", index=False, encoding="utf-8-sig")

        metrics_summary["site_coverage"] = coverage_metrics
        metrics_summary["merge_audit_severe_warnings"] = int(
            count_severe_bad_merges(pd.read_csv(out_dirs["reports"] / "site_project_merge_audit_v21.csv"))
            if (out_dirs["reports"] / "site_project_merge_audit_v21.csv").exists()
            else 0
        )
    ...
    if SAT_MODEL_FEATURES_RUNTIME:
        fi_sat = save_satellite_feature_importance(
            selected_models,
            out_dirs["reports"],
            used_sat_features=list(SAT_MODEL_FEATURES_RUNTIME),
            get_inner_model=get_inner_model,
            get_preprocess_feature_names=get_preprocess_feature_names,
        )
        if not fi_sat.get("ok"):
            warnings.warn(
                "satellite_feature_importance_v22 unavailable: "
                + "; ".join(fi_sat.get("notes") or [])
            )
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        ...
        choices=["none", "basic", "radii", "full"],
        default="full",
        help="V22 satellite/environment feature pack.",
    )
    ap.add_argument(
        "--satellite-feature-csv",
        default="data/external/satellite_features/basiskele/sentinel_features_v22.csv",
        help="Path to sentinel_features_v22.csv",
    )
    ap.add_argument(
        "--run-satellite-ablation",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Run V22 satellite ablation (control/basic/radii/full).",
    )
    ap.add_argument(
        "--run-spatial-diagnostic",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Optional random vs grouped CV diagnostic for satellite spatial leakage.",
    )
    ap.add_argument(
...
def main() -> None:
    ...
    cfg = RunConfig(
        ...
        satellite_feature_mode=str(getattr(args, "satellite_feature_mode", "full")),
        satellite_feature_csv=str(getattr(args, "satellite_feature_csv", "data/external/satellite_features/basiskele/sentinel_features_v22.csv")),
        run_satellite_ablation=bool(getattr(args, "run_satellite_ablation", False)),
        run_spatial_diagnostic=bool(getattr(args, "run_spatial_diagnostic", False)),
    ...
    (out_dirs["base"] / "run_config_v22_basiskele.json").write_text(json.dumps(asdict(cfg), indent=2, ensure_ascii=False), encoding="utf-8")
    ...
    def run_one_mode(
        ...
        satellite_feature_mode: str | None = None,
    ):
        global SAT_MODE_RUNTIME, SAT_CSV_RUNTIME, SAT_JOIN_REPORT, SAT_MODEL_FEATURES_RUNTIME
        ...
        sat_mode = normalize_satellite_feature_mode(
            satellite_feature_mode if satellite_feature_mode is not None else getattr(cfg, "satellite_feature_mode", "none")
        )
        SAT_MODE_RUNTIME = sat_mode
        SAT_CSV_RUNTIME = str(getattr(cfg, "satellite_feature_csv", SAT_CSV_RUNTIME))
        mode_cfg = RunConfig(
            **{
                **asdict(cfg),
                ...
                "site_extraction_mode": site_mode,
                "site_project_encoding": site_enc,
                "satellite_feature_mode": sat_mode,
            }
        )
        ...
        sales_with_demo, sat_join_report = join_satellite_features(
            sales_with_demo,
            SAT_CSV_RUNTIME,
            satellite_feature_mode=sat_mode if sat_mode != "none" else "full",
        )
        SAT_JOIN_REPORT = dict(sat_join_report)
        if sat_mode == "none":
            SAT_MODEL_FEATURES_RUNTIME = []
            drop_sat = [c for c in sales_with_demo.columns if str(c).startswith("sat_") and c not in {"sat_has_features", "sat_missing_reason"}]
            sales_with_demo = sales_with_demo.drop(columns=drop_sat, errors="ignore")
        else:
            used, zero, qa = get_model_satellite_feature_names(sat_mode, sales_with_demo)
            SAT_MODEL_FEATURES_RUNTIME = list(used)
            SAT_JOIN_REPORT["used_in_model"] = list(used)
            SAT_JOIN_REPORT["zero_variance_excluded"] = list(zero)
            SAT_JOIN_REPORT["feature_qa"] = qa
            if zero:
                _plog(f"V22 zero_variance_excluded ({len(zero)}): {', '.join(zero)}")
            _plog(f"V22 model satellite features ({len(used)}): {', '.join(used)}")
        sales_with_demo.to_csv(mode_out_dirs["input"] / f"sales_training_table_v22_{demo_mode}_{attr_mode}_{sat_mode}.csv", index=False, encoding="utf-8-sig")
        ...
        metrics_summary["selected_satellite_feature_mode"] = str(getattr(mode_cfg, "satellite_feature_mode", "none"))
        metrics_summary["sat_feature_coverage"] = float(SAT_JOIN_REPORT.get("sat_feature_coverage") or 0.0)
        metrics_summary["satellite_join_report"] = dict(SAT_JOIN_REPORT)
        metrics_summary["sat_zero_variance_excluded"] = list(SAT_JOIN_REPORT.get("zero_variance_excluded") or [])
        metrics_summary["sat_used_in_model"] = list(SAT_JOIN_REPORT.get("used_in_model") or SAT_MODEL_FEATURES_RUNTIME)
    ...
    n_premium_runs = 7 if bool(getattr(args, "run_site_ablation", False)) else 0
    ...
    try:
        ...
        # V22 satellite mode + optional ablation
        selected_sat_mode = normalize_satellite_feature_mode(str(getattr(cfg, "satellite_feature_mode", "full")))
        sat_csv_path = str(getattr(cfg, "satellite_feature_csv", ""))
        sat_csv_ok = resolve_satellite_csv_path(sat_csv_path) is not None
        sat_csv_coverage = estimate_satellite_csv_coverage(sat_csv_path) if sat_csv_ok else 0.0
        sat_coverage_ok = bool(sat_csv_ok and sat_csv_coverage >= float(SAT_COVERAGE_GATE))
        satellite_ablation_rows: list[dict[str, Any]] = []
        decision_overall = "DIAGNOSTIC"
        if selected_sat_mode != "none" and not sat_csv_ok:
            warnings.warn(
                f"Satellite CSV missing ({sat_csv_path}); forcing satellite_feature_mode=none (control_v21 only)."
            )
            selected_sat_mode = "none"
            cfg.satellite_feature_mode = "none"
        elif selected_sat_mode != "none" and not sat_coverage_ok:
            warnings.warn(
                f"Satellite CSV coverage {sat_csv_coverage:.3f} < {SAT_COVERAGE_GATE:.2f}; "
                "forcing satellite_feature_mode=none (control_v21 only)."
            )
            selected_sat_mode = "none"
            cfg.satellite_feature_mode = "none"
        else:
            _plog(
                f"V22 satellite CSV ready: coverage={sat_csv_coverage:.3f} "
                f"(gate>={SAT_COVERAGE_GATE:.2f}) mode={selected_sat_mode}"
            )

        if bool(getattr(args, "run_satellite_ablation", False)):
            experiments = satellite_ablation_experiments() if sat_coverage_ok else [("control_v21", "none")]
            if not sat_coverage_ok:
                warnings.warn("Satellite ablation requested but CSV missing/low coverage — control_v21 only.")
            for exp_name, sat_m in experiments:
                _pstage(f"Satellite ablation: {exp_name}")
                _plog(f"\n========== V22 SAT ABLATION: {exp_name} mode={sat_m} ==========")
                mode_dirs = build_output_dirs(out_dirs["base"] / f"ablation_sat_{exp_name}")
                _b, ms, _d, _f = run_one_mode(
                    selected_demo,
                    selected_attr,
                    mode_dirs,
                ...
                vr = float(ms.get("variance_ratio") if ms.get("variance_ratio") is not None else ens.get("basiskele_variance_ratio") or float("nan"))
                notes = []
                cov = float(ms.get("sat_feature_coverage") or 0.0)
                if sat_m != "none" and cov < 0.65:
                    notes.append(f"low_sat_coverage ({cov:.3f} < 0.65)")
                if np.isfinite(r2) and r2 <= V21_REF["r2"] and exp_name != "control_v21":
                    notes.append(f"no_r2_lift_vs_v21 ({r2:.4f} <= {V21_REF['r2']:.4f})")
                if np.isfinite(mape) and mape > 0.1105:
                    notes.append(f"mape_guardrail_fail ({mape:.4f} > 0.1105)")
                satellite_ablation_rows.append(
                    {
                        "experiment": exp_name,
                        "satellite_feature_mode": sat_m,
                        "rows": ens.get("rows") or ms.get("rows"),
                        "sat_feature_coverage": cov,
                        "r2": r2,
                        "log_r2": ens.get("log_r2"),
                        "mape": mape,
                        "mae": ens.get("mae_tl_per_m2"),
                        ...
                        "cheap_decile_bias": ms.get("cheap_decile_bias"),
                        "selected": False,
                        "notes": "; ".join(notes),
                    }
                )
            pick, decision_overall = select_satellite_experiment(satellite_ablation_rows)
            for r in satellite_ablation_rows:
                r["selected"] = r.get("experiment") == pick
            pd.DataFrame(satellite_ablation_rows).to_csv(
                out_dirs["reports"] / "metrics_satellite_ablation_v22_basiskele.csv",
                index=False,
                encoding="utf-8-sig",
            )
            selected_sat_mode = next(
                (str(r["satellite_feature_mode"]) for r in satellite_ablation_rows if r["selected"]),
                "none",
            )
            cfg.satellite_feature_mode = selected_sat_mode
            _plog(f"Selected V22 satellite experiment: {pick} (mode={selected_sat_mode})")
        else:
            # Single-run / smoke: never auto-select; gates applied after final metrics.
            cfg.satellite_feature_mode = selected_sat_mode
            decision_overall = (
                "DIAGNOSTIC"
                if selected_sat_mode == "none" or not sat_coverage_ok
                else "SINGLE_RUN_DIAGNOSTIC"
            )

        # Stash for post-final summary writer
        args._v22_selected_sat_mode = selected_sat_mode
        args._v22_decision_overall = decision_overall
        args._v22_sat_csv_ok = sat_coverage_ok
        args._v22_sat_csv_coverage = sat_csv_coverage
        args._v22_satellite_ablation_rows = satellite_ablation_rows
        args._v22_is_single_run = not bool(getattr(args, "run_satellite_ablation", False))

        # V21 site-extraction ablation
        ...
        (out_dirs["base"] / "run_config_v21_basiskele.json").write_text(json.dumps(asdict(cfg), indent=2, ensure_ascii=False), encoding="utf-8")
        bundle, metrics_summary, demo_feature_report, anomaly_filter_report = run_one_mode(
            ...
            site_extraction_mode=selected_site_mode,
            site_project_encoding=selected_site_encoding,
            satellite_feature_mode=str(getattr(args, "_v22_selected_sat_mode", getattr(cfg, "satellite_feature_mode", "none"))),
        )
    ...
    # V22 satellite summary + diagnostics
    try:
        sat_mode_final = str(getattr(args, "_v22_selected_sat_mode", getattr(cfg, "satellite_feature_mode", "none")))
        sat_rows = list(getattr(args, "_v22_satellite_ablation_rows", []) or [])
        is_single = bool(getattr(args, "_v22_is_single_run", not sat_rows))
        sat_csv_ok = bool(getattr(args, "_v22_sat_csv_ok", False))
        decision_overall = str(getattr(args, "_v22_decision_overall", "DIAGNOSTIC"))
        v22_notes: list[str] = []
        candidate_exp: str | None = None
        selected_exp: str | None = "control_v21"
        selected_flag = False

        ens = metrics_summary.get("ensemble") or {}
        cand_r2 = float(metrics_summary.get("r2") if metrics_summary.get("r2") is not None else ens.get("r2") or float("nan"))
        cand_mape = float(metrics_summary.get("mape") if metrics_summary.get("mape") is not None else ens.get("mape") or float("nan"))
        sat_cov = float(metrics_summary.get("sat_feature_coverage") or getattr(args, "_v22_sat_csv_coverage", 0.0) or 0.0)

        if is_single and sat_mode_final != "none":
            candidate_exp = f"sat_{sat_mode_final}"
            gate = evaluate_satellite_candidate(
                {
                    "experiment": candidate_exp,
                    "r2": cand_r2,
                    "mape": cand_mape,
                    "sat_feature_coverage": sat_cov,
                    "expensive_decile_bias": metrics_summary.get("expensive_decile_bias"),
                }
            )
            v22_notes = list(gate.get("notes") or [])
            if gate.get("eligible"):
                selected_exp = candidate_exp
                selected_flag = True
                decision_overall = "PASS"
            else:
                selected_exp = "control_v21"
                selected_flag = False
                decision_overall = "DIAGNOSTIC"
                if not v22_notes:
                    v22_notes = ["sat_basic_did_not_beat_v21_reference"] if sat_mode_final == "basic" else ["candidate_did_not_beat_v21_reference"]
        elif sat_rows:
            selected_exp = next((r["experiment"] for r in sat_rows if r.get("selected")), "control_v21")
            # Prefer best sat R2 as candidate label even if gates fail
            sat_only = [r for r in sat_rows if str(r.get("experiment") or "") != "control_v21"]
            if sat_only:
                candidate_exp = max(
                    sat_only,
                    key=lambda r: float(r.get("r2") or -1e9),
                ).get("experiment")
            else:
                candidate_exp = None
            decision_overall = str(getattr(args, "_v22_decision_overall", "DIAGNOSTIC_NO_LIFT"))
            if decision_overall != "PASS":
                if decision_overall not in {"DIAGNOSTIC_NO_LIFT", "DIAGNOSTIC_FAIL"}:
                    decision_overall = "DIAGNOSTIC_NO_LIFT" if sat_only else "DIAGNOSTIC_FAIL"
                v22_notes = ["sentinel_features_did_not_improve_v21"]
            selected_flag = bool(selected_exp and selected_exp != "control_v21")
        else:
            selected_exp = "control_v21"
            candidate_exp = None
            decision_overall = "DIAGNOSTIC"
            if not sat_csv_ok:
                v22_notes = ["satellite_csv_missing_or_low_coverage_control_only"]

        # Ensure importance report exists at final out reports.
        # When ablation selects control_v21, final mode is none — copy from best sat arm reports.
        if sat_mode_final != "none":
            fi_status = save_satellite_feature_importance(
                getattr(bundle, "models", None) if bundle is not None else None,
                out_dirs["reports"],
                used_sat_features=list(metrics_summary.get("sat_used_in_model") or SAT_MODEL_FEATURES_RUNTIME or []),
                get_inner_model=get_inner_model,
                get_preprocess_feature_names=get_preprocess_feature_names,
            )
            if not fi_status.get("ok"):
                v22_notes.extend(list(fi_status.get("notes") or []))
                v22_notes.append("satellite_feature_importance_v22_missing_or_empty")
        elif sat_rows and candidate_exp:
            src = out_dirs["base"] / f"ablation_sat_{candidate_exp}" / "reports" / "satellite_feature_importance_v22.csv"
            dst = out_dirs["reports"] / "satellite_feature_importance_v22.csv"
            if src.exists():
                dst.write_bytes(src.read_bytes())
                notes_src = src.with_name("satellite_feature_importance_v22_notes.txt")
                note_lines = [f"copied_from={src.parent.parent.name}"]
                if notes_src.exists():
                    note_lines.append(notes_src.read_text(encoding="utf-8").strip())
                note_lines.append(f"selected={selected_exp}; candidate={candidate_exp}")
                (out_dirs["reports"] / "satellite_feature_importance_v22_notes.txt").write_text(
        ...
        write_metrics_summary_v22(
            out_dirs["reports"] / "metrics_summary_v22_basiskele.json",
            selected_experiment=selected_exp,
            sat_coverage=sat_cov,
            metrics=metrics_summary,
            decision_overall=decision_overall,
            warnings=([] if sat_csv_ok else ["satellite_csv_missing_or_low_coverage_control_only"]),
            qa_findings=[],
            candidate_experiment=candidate_exp,
            notes=v22_notes,
            coverage_definition="non_null_any_of_sat_ndvi_mean_250m|sat_ndbi_mean_250m|sat_ndwi_mean_250m",
        )
        if not sat_rows:
            pd.DataFrame(
                [
                    {
                        "experiment": candidate_exp or selected_exp or f"sat_{sat_mode_final}",
                        "satellite_feature_mode": sat_mode_final,
                        "rows": ens.get("rows") or metrics_summary.get("rows"),
                        "sat_feature_coverage": sat_cov,
                        "r2": cand_r2,
                        "log_r2": ens.get("log_r2"),
                        "mape": cand_mape,
                        ...
                        "expensive_decile_bias": metrics_summary.get("expensive_decile_bias"),
                        "cheap_decile_bias": metrics_summary.get("cheap_decile_bias"),
                        "selected": bool(selected_flag),
                        "decision": decision_overall,
                        "notes": "; ".join(v22_notes) if v22_notes else ("single_run_diagnostic" if is_single else "single_run"),
                    }
                ]
            ).to_csv(
                out_dirs["reports"] / "metrics_satellite_ablation_v22_basiskele.csv",
        ...
        sales_diag, _jr = join_satellite_features(
            sales_final_base,
            resolve_satellite_csv_path(str(getattr(cfg, "satellite_feature_csv", ""))),
            satellite_feature_mode=sat_mode_final if sat_mode_final != "none" else "full",
        )
        satellite_feature_coverage_table(sales_diag, sat_mode_final if sat_mode_final != "none" else "full").to_csv(
            out_dirs["reports"] / "satellite_feature_coverage_v22.csv", index=False, encoding="utf-8-sig"
        )
        satellite_feature_distribution_table(sales_diag, sat_mode_final if sat_mode_final != "none" else "full").to_csv(
            out_dirs["reports"] / "satellite_feature_distribution_v22.csv", index=False, encoding="utf-8-sig"
        )
        pd.DataFrame(
            [
                {"model": "v21_reference", "expensive_decile_bias": -10159, "r2": V21_REF["r2"], "mape": V21_REF["mape"]},
                {
                    "model": "v22_selected",
                    "expensive_decile_bias": metrics_summary.get("expensive_decile_bias"),
        ...
        ).to_csv(out_dirs["reports"] / "expensive_decile_bias_comparison_v22.csv", index=False, encoding="utf-8-sig")
        if bool(getattr(args, "run_spatial_diagnostic", False)) and sat_mode_final != "none":
            Xd, yd = make_training_matrix(sales_diag)
            run_spatial_leakage_diagnostic(Xd, yd).to_csv(
                out_dirs["reports"] / "spatial_leakage_diagnostic_v22.csv", index=False, encoding="utf-8-sig"
            )
    except Exception as _v22_exc:
        warnings.warn(f"V22 summary/diagnostics failed: {_v22_exc}")
