...
def train_and_evaluate(X: pd.DataFrame, y: pd.Series, cfg: RunConfig, out_dirs: dict[str, Path]) -> tuple[ModelBundle, dict[str, Any]]:
    ...
    metrics_summary["v20_selected_reference"] = {
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Run V21 site-extraction ablation matrix.",
    )
...
def main() -> None:
    ...
    n_premium_runs = 6 if bool(getattr(args, "run_site_ablation", False)) else 0
    ...
    try:
        ...
        # V24 Kocaeli site-aware ablation (primary vs control on same Kocaeli data)
        ...
        if bool(getattr(args, "run_site_ablation", False)):
            ...
            for exp_name, site_m, enc_m, dup_m in v24_experiments:
                _pstage(f"V24 site ablation: {exp_name}")
                _plog(
                    f"\n========== V24 KOCAELI ABLATION: {exp_name} "
            ...
            control = by_exp.get("control_v17_or_v21_like_global") or {}
            ctrl_r2 = float(control.get("r2") if control.get("r2") is not None else float("nan"))
            ctrl_mape = float(control.get("mape") if control.get("mape") is not None else float("nan"))
            ctrl_bas = float(control.get("basiskele_r2") if control.get("basiskele_r2") is not None else float("nan"))

            def _v24_eligible(row: dict[str, Any]) -> bool:
                if str(row.get("experiment")) == "control_v17_or_v21_like_global":
                    return False
                notes = str(row.get("notes") or "")
                if "leakage_guard_fail" in notes or "severe_possible_bad_merge" in notes:
                    return False
                try:
                    r2 = float(row.get("r2") or -9)
                    mape = float(row.get("mape") or 9)
                    if not np.isfinite(ctrl_r2) or r2 <= ctrl_r2:
                        return False
                    if np.isfinite(ctrl_mape) and mape > ctrl_mape + 0.005 + 1e-12:
                        return False
                    bas_r2 = float(row.get("basiskele_r2") if row.get("basiskele_r2") is not None else float("nan"))
                    if np.isfinite(ctrl_bas) and np.isfinite(bas_r2) and bas_r2 < ctrl_bas - 0.02:
                        return False
                    return True
                except Exception:
                    return False

            candidates = [r for r in site_ablation_rows_v24 if _v24_eligible(r)]
            if candidates:
                pick = max(candidates, key=lambda r: (float(r.get("r2") or -9), float(r.get("basiskele_r2") or -9), -float(r.get("mape") or 9)))["experiment"]
            else:
                pick = "control_v17_or_v21_like_global"
            for r in site_ablation_rows_v24:
                r["selected"] = r.get("experiment") == pick
                if r["selected"] and not r.get("notes"):
                    r["notes"] = "selected vs control_v17_or_v21_like_global (global R2/MAPE + Başiskele stability + leakage/merge gates)"
            ...
            _plog(
                f"Selected V24 after ablation: {pick} "
    ...
    # App-safe site options catalog
    try:
        # Prefer training table with site columns if present in input dumps
        sales_for_opt = sales_final_base
        oof_path = out_dirs["output"] / "oof_predictions_v18_basiskele.csv"
        # Fit a temporary adder to attach site ids for catalog
        opt_adder = SiteProjectExtractionAdder(
            site_extraction_mode=str(getattr(cfg, "site_extraction_mode", "full")),
            site_project_encoding=str(getattr(cfg, "site_project_encoding", "foldsafe_target")),
            min_site_freq=1,
        )
