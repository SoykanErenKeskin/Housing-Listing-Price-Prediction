...
"""
V23 Başiskele duplex / large-home refresh pipeline.

Fork of V21 site-extraction stack. Adds controlled duplex features
(detail_konut_tipi primary, title fallback) + large-home refresh diagnostics.
Does not modify V21 sources. Satellite/V22 is out of scope.

Example smoke:
  python train_v23_basiskele_duplex_pipeline.py --out v3/outputs/v23_basiskele_duplex_smoke \
    --fast --limit-sale 400 --limit-rental 300 --duplex-feature-mode flags \
    --no-run-duplex-ablation --use-trend --no-interactive
"""
...
if __name__ == "__main__":
    ...
    if not _no_interactive:
        sys.stdout.write("V23 Başiskele duplex ayar sihirbazı açılıyor...\n")
        sys.stdout.flush()
...
def train_and_evaluate(X: pd.DataFrame, y: pd.Series, cfg: RunConfig, out_dirs: dict[str, Path]) -> tuple[ModelBundle, dict[str, Any]]:
    ...
    (out_dirs["reports"] / "metrics_summary_v23_basiskele.json").write_text(
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--out", default="v3/outputs/v23_basiskele_duplex_largehome_refresh_full")
...
def main() -> None:
    ...
    (out_dirs["base"] / "run_config_v23_basiskele.json").write_text(json.dumps(asdict(cfg), indent=2, ensure_ascii=False), encoding="utf-8")
    ...
    try:
        ...
        drift.to_csv(out_dirs["reports"] / "inventory_drift_v23_vs_v21.csv", index=False, encoding="utf-8-sig")
        ...
        cov_by_type.to_csv(out_dirs["reports"] / "duplex_coverage_v23.csv", index=False, encoding="utf-8-sig")
        # price distributions
    ...
    try:
        ...
        # V23 duplex / large-home ablation (primary selection vs new-data control)
        duplex_ablation_rows = []
        if bool(getattr(args, "run_duplex_ablation", False)):
            OLD_V21_R2 = 0.5059
            OLD_V21_MAPE = 0.1055
            duplex_experiments = [
                ("control_v21_on_new_data", "none"),
                ("duplex_flags_only", "flags"),
                ("largehome_refresh_only", "largehome"),
                ("duplex_interactions", "interactions"),
                ("full_v23", "full"),
            ]
            for exp_name, dmode in duplex_experiments:
                _pstage(f"Duplex ablation: {exp_name}")
                _plog(
                    f"\n========== V23 DUPLEX ABLATION: {exp_name} duplex_mode={dmode} =========="
                )
                mode_dirs = build_output_dirs(out_dirs["base"] / f"ablation_duplex_{exp_name}")
                _b, ms, _d, _f = run_one_mode(
                    selected_demo,
                    selected_attr,
                    mode_dirs,
                ...
                duplex_ablation_rows.append(
                    {
                        ...
                        "expensive_decile_bias": exp_bias,
                        "cheap_decile_bias": cheap_bias,
                        "old_v21_r2_ref": OLD_V21_R2,
                        "old_v21_mape_ref": OLD_V21_MAPE,
                        "selected": False,
                        "notes": "; ".join(notes),
                    }
                )

            by_exp = {r["experiment"]: r for r in duplex_ablation_rows}
            control = by_exp.get("control_v21_on_new_data") or {}
            ...
            ctrl_lh = float(control.get("large_home_r2") if control.get("large_home_r2") is not None else float("nan"))
            ctrl_exp = float(control.get("expensive_decile_bias") if control.get("expensive_decile_bias") is not None else float("nan"))

            def _duplex_eligible(row: dict[str, Any]) -> bool:
                name = str(row.get("experiment") or "")
                if name == "control_v21_on_new_data":
                    return False
                notes = str(row.get("notes") or "")
                if "leakage_guard_fail" in notes:
                    return False
                try:
                    ...
                    lh = float(row.get("large_home_r2") if row.get("large_home_r2") is not None else float("nan"))
                    expb = float(row.get("expensive_decile_bias") if row.get("expensive_decile_bias") is not None else float("nan"))
                    if not np.isfinite(ctrl_r2) or r2 <= ctrl_r2:
                        return False
                    if np.isfinite(ctrl_mape) and mape > ctrl_mape + 0.005 + 1e-12:
                        return False
                    if np.isfinite(ctrl_lh) and np.isfinite(lh) and lh < ctrl_lh - 1e-12:
                        return False
                    # expensive bias: less negative or not materially worse (allow 2k TL slack)
                    if np.isfinite(ctrl_exp) and np.isfinite(expb) and expb < ctrl_exp - 2000:
                        return False
                    return True
                except Exception:
                    return False

            candidates = [r for r in duplex_ablation_rows if _duplex_eligible(r)]
            if candidates:
                pick = max(
                    candidates,
                    key=lambda r: (
                        ...
                        float(r.get("large_home_r2") or -9),
                        float(r.get("expensive_decile_bias") if r.get("expensive_decile_bias") is not None else -1e18),
                        -float(r.get("mape") or 9),
                    ),
                )["experiment"]
            else:
                pick = "control_v21_on_new_data"

            for r in duplex_ablation_rows:
                r["selected"] = r.get("experiment") == pick
                if r["selected"] and not r.get("notes"):
                    r["notes"] = "selected vs control_v21_on_new_data (R2 + MAPE + large_home_r2 + expensive bias gates)"
                # annotate gate failures for non-control
                if r.get("experiment") != "control_v21_on_new_data" and not r.get("selected"):
                    fail = []
                    try:
                        if float(r.get("r2") or -9) <= ctrl_r2:
                            fail.append("no_r2_lift_vs_control")
                        if np.isfinite(ctrl_mape) and float(r.get("mape") or 9) > ctrl_mape + 0.005:
                            fail.append("mape_guardrail_fail")
                        lh = float(r.get("large_home_r2") if r.get("large_home_r2") is not None else float("nan"))
                        if np.isfinite(ctrl_lh) and np.isfinite(lh) and lh < ctrl_lh:
                            fail.append("large_home_r2_worse")
                        expb = float(r.get("expensive_decile_bias") if r.get("expensive_decile_bias") is not None else float("nan"))
                        if np.isfinite(ctrl_exp) and np.isfinite(expb) and expb < ctrl_exp - 2000:
                            fail.append("expensive_bias_materially_worse")
                    except Exception:
                        pass
                    if fail:
                        r["notes"] = (r.get("notes") + "; " if r.get("notes") else "") + "; ".join(fail)

            pd.DataFrame(duplex_ablation_rows).to_csv(
                out_dirs["reports"] / "metrics_duplex_ablation_v23_basiskele.csv",
                index=False,
                encoding="utf-8-sig",
            )
            # expensive decile comparison table
            pd.DataFrame(
                [
                    {
                        ...
                        "r2": r.get("r2"),
                        "mape": r.get("mape"),
                        "large_home_r2": r.get("large_home_r2"),
                        "selected": r.get("selected"),
                    }
                    for r in duplex_ablation_rows
                ]
            ).to_csv(
                out_dirs["reports"] / "expensive_decile_bias_comparison_v23.csv",
                index=False,
                encoding="utf-8-sig",
            )
            picked = by_exp.get(pick) or {}
            ...
            _plog(
                f"Selected V23 duplex after ablation: {pick} "
                f"(duplex_mode={selected_duplex_mode}) "
                f"[best_checkpoint not auto-updated; primary vs control_v21_on_new_data; "
                f"old V21 ref R2={OLD_V21_R2} MAPE={OLD_V21_MAPE}]"
            )
