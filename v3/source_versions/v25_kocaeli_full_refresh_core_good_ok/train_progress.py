...
class TrainProgress:
    ...
    def _status_text(self, max_width: int | None = None) -> str:
        ...
        # Past the initial estimate but still running: keep bar below 100% and
        # soft-ETA so overflow ticks do not freeze at "100% ~0sn".
        overdue = (not self._closed) and self.done >= self.total
        if self._closed:
            pct = 100.0 * min(self.done, self.total) / max(self.total, 1)
            eta_s = _fmt_duration(0.0)
            shown_done = min(self.done, self.total)
            total_disp = self.total
        elif overdue:
            pct = 99.9
            soft_remain = max(4, int(round(0.12 * max(self.done, 1))))
            eta = (elapsed / max(self.done, 1)) * soft_remain
            eta_s = f"{_fmt_duration(eta)}+"
            shown_done = self.done
            total_disp = self.total
        else:
            pct = min(99.9, 100.0 * self.done / max(self.total, 1))
            remaining_units = max(0, self.total - self.done)
            if self.done > 0 and remaining_units > 0:
                eta_s = _fmt_duration((elapsed / self.done) * remaining_units)
            else:
                eta_s = "?"
            shown_done = self.done
            total_disp = self.total
        ...
        count_s = f"{shown_done}/{total_disp}+" if overdue else f"{shown_done}/{total_disp}"
        prefix = (
            f"V18 {spin} [{self._bar(pct)}] {pct:4.1f}% "
            f"{count_s} "
    ...
    def tick(self, label: str = "", n: int = 1) -> None:
        with self._lock:
            ...
            # Do not snap total up to done here: that forced 100%/~0sn ETA.
            # Display/ETA handle overdue via _status_text; finish() reconciles totals.
            if label:
                self._label = label
                self._stage = label
            self._render_unlocked()
...
def estimate_training_units(
    ...
    n_location_runs: int = 0,
) -> int:
    per_run = 1 + (2 * n_models) + (n_models * n_segments) + (n_models * n_counties_est) + 2
    n_runs = n_attr_runs + n_demo_runs + n_detail_runs + n_location_runs + (1 if include_final else 0)
