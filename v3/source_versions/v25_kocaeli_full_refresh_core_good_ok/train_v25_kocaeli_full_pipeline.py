...
try:
    from kocaeli_site_project_extraction import (
        SITE_UNIT_PRICE_COL,
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        "--model-scope",
        choices=["kocaeli_global", "basiskele_only"],
        default="kocaeli_global",
        help="V24 default: Kocaeli global multi-county model.",
    )
    ap.add_argument(
        "--comparable-mode",
    ...
    ap.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Run V25 core_good_ok ablation: control replay + full_v25.",
    )
    ap.add_argument(
...
def main() -> None:
    ...
    cfg = RunConfig(
        ...
        model_scope=str(getattr(args, "model_scope", "kocaeli_global")),
    ...
    # --- V25 county policy + filter + heating audits (aggregate only) ---
    write_county_policy_report(
        out_dirs["reports"] / "county_policy_v25.json",
    ...
    if unexpected:
        raise RuntimeError(f"V25 county_policy silent-drop guard failed; unexpected counties={unexpected}")
    for weak in policy_info["excluded_counties"]:
        if weak in after_counties:
            raise RuntimeError(f"V25 excluded WEAK county leaked into training frame: {weak}")
    row_filter_payload = {
        ...
        "notes": [
            "basic filters mirror V24.1 (gross_m2 typically 25-500 in apply_basic_filters).",
            "reason flags are not mutually exclusive.",
        ],
    }
    (out_dirs["reports"] / "row_count_and_filtering_v25.json").write_text(
    ...
    # V25 ablation trains N arms then returns early (no separate FINAL pass).
    n_v25_runs = 0
    if bool(getattr(args, "run_v25_ablation", False)):
        n_v25_runs = 2
        if bool(getattr(args, "v25_site_encoding_check", False)):
            n_v25_runs = 3
    include_final = not bool(getattr(args, "run_v25_ablation", False))
    ...
    try:
        ...
        # V25 core_good_ok ablation (control replay + full + optional encoding check)
        v25_ablation_rows: list[dict[str, Any]] = []
        if bool(getattr(args, "run_v25_ablation", False)):
            v25_experiments = [
                ("control_v24_1_replay_on_core_good_ok", "full", "foldsafe_target", "full"),
                ("full_v25_core_good_ok", "full", "foldsafe_target", "full"),
            ]
            if bool(getattr(args, "v25_site_encoding_check", False)):
                v25_experiments.append(("foldsafe_vs_frequency_site_encoding_check", "full", "frequency", "full"))
            for exp_name, site_m, enc_m, dup_m in v25_experiments:
                # Smoke may pass duplex flags / frequency via top-level CLI for non-ablation; ablation arms use explicit modes.
                if bool(getattr(args, "fast", False)):
                    # Keep ablation duplex mode but allow flags override in smoke if user set duplex-feature-mode
                    if str(getattr(args, "duplex_feature_mode", "full")) in {"flags", "none", "largehome"}:
                        dup_m = str(args.duplex_feature_mode)
                    if str(getattr(args, "site_project_encoding", "foldsafe_target")) == "frequency" and exp_name != "foldsafe_vs_frequency_site_encoding_check":
                        # For smoke control/full, honor CLI frequency to save time if requested
                        enc_m = "frequency"
                mode_dirs = build_output_dirs(out_dirs["base"] / f"ablation_v25_{exp_name}")
                cached_ms_path = mode_dirs["reports"] / "metrics_summary_v25_core_good_ok.json"
                alt_cached = mode_dirs["reports"] / "metrics_summary_v24_kocaeli.json"
                ms = None
                for cand in (cached_ms_path, alt_cached):
                    if cand.exists():
                        try:
                            ms = json.loads(cand.read_text(encoding="utf-8"))
                            ...
                            ms = None
                if ms is not None:
                    _plog(f"\n========== V25 RESUME SKIP (cached): {exp_name} ==========")
                else:
                    _pstage(f"V25 ablation: {exp_name}")
                    _plog(
                        f"\n========== V25 KOCAELI ABLATION: {exp_name} "
                        ...
                        payload_ms.update(
                            {
                                ...
                                "experiment": exp_name,
                                "v24_1_old_reference": V24_1_REFERENCE,
                            }
                        )
                    ...
                    try:
                        ...
                        diff = build_feature_schema_diff(
                            ...
                            v24_importance_csv=Path("v3/outputs/v24_1_kocaeli_site_merge_repair/ablation_v24_full_v24/reports/feature_importance_v18_basiskele.csv"),
                        ...
                        (mode_dirs["reports"] / "feature_schema_diff_v24_1_vs_v25.json").write_text(
                        ...
                        if str(diff.get("status")) == "FAIL":
                            raise RuntimeError(
                                f"V25 critical feature guard FAIL for {exp_name}: {diff.get('critical_missing_features')}"
                            )
                        heat_all.to_csv(mode_dirs["reports"] / "heating_feature_audit_v25.csv", index=False, encoding="utf-8-sig")
                        if _fidf is not None:
                            _fidf.to_csv(mode_dirs["reports"] / "feature_importance_v25.csv", index=False, encoding="utf-8-sig")
                            _fidf.to_csv(out_dirs["reports"] / "feature_importance_v25.csv", index=False, encoding="utf-8-sig")
                    except RuntimeError:
                        raise
                    except Exception as _guard_exc:
                        warnings.warn(f"V25 feature guard post-arm warning: {_guard_exc}")
            ...
            control = by_exp.get("control_v24_1_replay_on_core_good_ok") or {}
            full = by_exp.get("full_v25_core_good_ok") or {}
            ctrl_r2 = float(control.get("r2") or float("nan"))
            ctrl_mape = float(control.get("mape") or float("nan"))
            pick = "control_v24_1_replay_on_core_good_ok"
            best = False
            notes = []
            if full:
                ok = True
                if not bool(full.get("leakage_pass", True)):
                    ok = False
                    notes.append("leakage_fail")
                if int(full.get("merge_audit_severe_warnings") or 0) > 0:
                    ok = False
                    notes.append("severe_merge")
                fr2 = float(full.get("r2") or float("nan"))
                fmape = float(full.get("mape") or float("nan"))
                if np.isfinite(ctrl_r2) and np.isfinite(fr2) and fr2 + 1e-12 < ctrl_r2:
                    ok = False
                    notes.append("r2_below_control")
                if np.isfinite(ctrl_mape) and np.isfinite(fmape) and fmape > ctrl_mape + 0.005 + 1e-12:
                    ok = False
                    notes.append("mape_worse_than_control_plus_0.005")
                schema_path = out_dirs["reports"] / "feature_schema_diff_v24_1_vs_v25.json"
                if schema_path.exists():
                    try:
                        sch = json.loads(schema_path.read_text(encoding="utf-8"))
                        if str(sch.get("status")) == "FAIL":
                            ok = False
                            notes.append("feature_guard_fail")
                    except Exception:
                        pass
                if ok:
                    pick = "full_v25_core_good_ok"
                    best = True
                    notes.append("selected_vs_same_data_control; not auto-promoted over V24.1")
                else:
                    notes.append("kept_control_or_diagnostic")
            for r in v25_ablation_rows:
                r["selected"] = r.get("experiment") == pick
                if r["selected"]:
                    r["notes"] = "; ".join(notes)
            pd.DataFrame(v25_ablation_rows).to_csv(
                out_dirs["reports"] / "metrics_v25_core_good_ok_ablation.csv",
                index=False,
                encoding="utf-8-sig",
            )
            pd.DataFrame(
            ...
            ).to_csv(out_dirs["reports"] / "expensive_decile_bias_comparison_v25.csv", index=False, encoding="utf-8-sig")
            selection_payload = {
                "status": "selected_checkpoint_v25_core_good_ok" if best else "diagnostic_v25_core_good_ok",
                ...
                "full_v25": full,
                "v24_1_old_reference": V24_1_REFERENCE,
                "note": "App/EDER artifacts not updated in this task.",
            }
            (out_dirs["reports"] / "selection_decision_v25.json").write_text(
            ...
            readme_lines = [
                ...
                f"- promote_over_v24_1: `false`",
                "",
                "## Same-data control",
                f"- control R2={control.get('r2')} MAPE={control.get('mape')}",
                f"- full_v25 R2={full.get('r2')} MAPE={full.get('mape')}",
                "",
                "## Old V24.1 reference (different inventory distribution)",
                f"- R2={V24_1_REFERENCE['r2']} MAPE={V24_1_REFERENCE['mape']}",
                "",
                "Do not update EDER/app artifacts from this run.",
                "",
            ]
            (out_dirs["base"] / "README_v25_core_good_ok_run.md").write_text("\n".join(readme_lines), encoding="utf-8")
            # Do not also run the FINAL single arm when V25 ablation already produced selected arms.
        ...
        if bool(getattr(args, "run_site_ablation", False)) and not bool(getattr(args, "run_v25_ablation", False)):
