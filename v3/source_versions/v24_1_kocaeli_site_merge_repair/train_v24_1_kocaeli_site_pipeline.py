...
"""
V24 Kocaeli site-aware global refresh pipeline.

County-scoped site/project extraction + V23 duplex/large-home features
trained on all available Kocaeli counties (not Başiskele-only).

Primary comparison: control on the same Kocaeli data.
Do not promote until ablation gates pass.
"""
...
if __name__ == "__main__":
    ...
    if not _no_interactive:
        sys.stdout.write("V24 Kocaeli site-aware ayar sihirbazı açılıyor...\n")
        sys.stdout.flush()
...
DEFAULT_COUNTIES = ["Başiskele", "İzmit", "Gölcük", "Karamürsel", "Kartepe"]
...
def fetch_listing_table(
    ...
    """Fetch Kocaeli listings for one or more counties."""
    table = validate_table_name(table)
    limit_clause = f" LIMIT {int(limit)}" if limit else ""
    counties = list(counties or DEFAULT_COUNTIES)
    # Expand IN clause with bound params
    county_params = {f"c{i}": c for i, c in enumerate(counties)}
    in_clause = ", ".join(f":c{i}" for i in range(len(counties)))
    sql = text(
        f"""
        SELECT *
        FROM {table}
...
def load_raw_data(args: argparse.Namespace, out_dirs: dict[str, Path]) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if args.sale_json and args.rental_json:
        ...
        print(f"Raw sale rows: {len(sales_raw)} | counties={counties}")
...
def parse_args() -> argparse.Namespace:
    ...
    ap = argparse.ArgumentParser(
        description="V21 Başiskele premium-signal location pipeline. "
    ...
    ap.add_argument("--counties", default="Başiskele,İzmit,Gölcük,Karamürsel,Kartepe", help="Comma-separated county list.")
    ap.add_argument("--out", default="v3/outputs/v24_kocaeli_site_aware_global_refresh_full")
...
def main() -> None:
    ...
    total_units = estimate_training_units(
        ...
        n_segments=4,
        n_counties_est=4,
        n_location_runs=n_location_runs + n_comparable_runs + n_premium_runs + n_duplex_runs,
    )
