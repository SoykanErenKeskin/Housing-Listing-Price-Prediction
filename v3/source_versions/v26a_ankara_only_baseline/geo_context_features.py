...
DEFAULT_CACHE_DIR = Path(__file__).resolve().parents[1] / "data" / "external" / "geo_context"
...
class GeoContextFeatureAdder(BaseEstimator, TransformerMixin):
    ...
    def _resolve_cache_dir(self) -> Path:
        ...
        candidates = [
            DEFAULT_CACHE_DIR,
            Path(__file__).resolve().parents[1] / "data" / "external" / "geo_context",
            Path.cwd() / "data" / "external" / "geo_context",
        ]
        for c in candidates:
            if (c / "geo_context_metadata.json").exists() or (c / "ankara_pois.parquet").exists() or (c / "ankara_pois.csv").exists():
                return c
        return candidates[0]
    ...
    def _load_cache(self) -> None:
        ...
        # V26A (Ankara) intentionally looks for ankara_*.parquet/csv, never kocaeli_*.
        # Ankara has no OSM POI/road/coastline cache yet and is landlocked, so this is
        # expected to come back empty — every downstream feature already degrades to a
        # neutral/zero value when *_ is empty, and load_report_ records what was missing.
        self.pois_ = _load_table(cache_dir / "ankara_pois")
        self.roads_ = _load_table(cache_dir / "ankara_roads")
        self.coast_ = pd.DataFrame()  # Ankara is landlocked: never look up a coastline cache.
