"""V26A Ankara city+county-aware site/project extraction (forked from V24 Kocaeli logic).

Reuses V21/V23 extraction/dictionary logic, then namespaces every canonical id as:

    {city_slug}::{county_slug}::{site_project_slug}

so the same display name never collapses across counties *or* across cities
(Ankara vs Kocaeli). No cross-city or cross-county merge happens automatically.

Başiskele curated dictionary (Kocaeli-only) is preserved for parity but will simply
never match Ankara rows; unknown counties/cities use conservative frequency-based
stems only.
"""
...
# This module is a per-city fork (V26A = Ankara only). Every canonical site id is
# namespaced city::county::site so it can never collide with a Kocaeli id, even if
# frames from both cities are ever concatenated by mistake downstream.
CITY_SLUG = "ankara"
CITY_DISPLAY = "Ankara"
...
def scope_site_id(county: Any, local_id: str) -> str:
    ...
    if lid in {MANUAL_UNKNOWN_ID, NO_SITE_ID}:
        return lid
    if lid == "other":
        return f"{CITY_SLUG}::{county_slug(county)}::other"
    return f"{CITY_SLUG}::{county_slug(county)}::{lid}"


def unscope_site_id(scoped: str) -> tuple[str, str, str]:
    """Return (city_slug, county_slug, local_id). Legacy 2-part (county::id) ids
    from pre-V26A data are tolerated with city_slug='unknown'."""
    s = str(scoped or "")
    parts = s.split("::")
    if len(parts) >= 3:
...
class AnkaraSiteProjectExtractionAdder(_BaseSiteAdder):
    """City+county-scoped site extraction for the Ankara-only V26A model."""
    ...
    def fit(self, X, y=None):
        # Fit base (includes V24.1 severe-split), then rebuild county-scoped counts + merge audit.
        ...
        meta = list(getattr(self, "_fit_rows_meta_", []) or [])
        counties = df["county"] if "county" in df.columns else pd.Series(["unknown"] * len(df), index=df.index)

        if meta and len(meta) == len(df):
            for r, c in zip(meta, counties):
                local = str(r.get("canonical_id") or "missing")
                r["county"] = c
                r["scoped_id"] = scope_site_id(c, local)
            vc = pd.Series([r["scoped_id"] for r in meta]).value_counts()
            self.site_counts_ = {str(k): int(v) for k, v in vc.items() if str(k) not in {"missing"}}
            self.county_site_counts_ = dict(self.site_counts_)
            # Gate/report merge audit must use county-scoped IDs (fixes cross-county false severes).
            self.merge_rows_ = build_merge_audit_rows(
                meta,
                self.site_counts_,
                self.merge_gap_warning_tl,
                id_field="scoped_id",
                county_field="county",
            )
            self._fit_rows_meta_ = meta  # type: ignore[attr-defined]
            scoped_iter = [r["scoped_id"] for r in meta]
        else:
            tmp = super().transform(df)
            local_ids = (
                tmp["site_project_id"].astype(str)
                if "site_project_id" in tmp.columns
                else pd.Series(["missing"] * len(df))
            )
            scoped_iter = [scope_site_id(c, lid) for c, lid in zip(counties, local_ids)]
            vc = pd.Series(scoped_iter).value_counts()
            self.site_counts_ = {str(k): int(v) for k, v in vc.items() if str(k) not in {"missing"}}
            self.county_site_counts_ = dict(self.site_counts_)

        by_c: dict[str, set[str]] = {}
        for c, sid in zip(counties.map(county_slug), scoped_iter):
...
def build_site_project_options(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def _display(sid: str) -> str:
        _city, _county, local = unscope_site_id(sid)
        return local.replace("_", " ").strip().title()

    g["display_name"] = g["site_project_id"].astype(str).map(_display)
    g["canonical_name"] = g["site_project_id"].astype(str).map(lambda s: unscope_site_id(s)[2])
...
def write_site_project_options(df: pd.DataFrame, artifacts_dir: Path) -> Path:
    ...
    csv_path = artifacts_dir / "site_project_options_ankara_v26a.csv"
    json_path = artifacts_dir / "site_project_options_ankara_v26a.json"
    ...
    payload = {
        ...
        "prediction_notes": {
            ...
            "match_existing_alias": "If user text matches alias, use city+county-scoped canonical id (ankara::<county>::<site>). Never merged across cities or counties automatically.",
        },
...
SiteProjectExtractionAdder = AnkaraSiteProjectExtractionAdder
