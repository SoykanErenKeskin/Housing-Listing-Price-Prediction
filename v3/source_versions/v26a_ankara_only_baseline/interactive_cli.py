...
def build_early_arg_parser() -> Any:
    ...
    ap.add_argument("--sale-table", default="market.sale_listings")
    ap.add_argument("--rental-table", default="market.rental_listings")
    ap.add_argument("--trend-table", default="market.price_observations")
    ap.add_argument("--city", default="Ankara")
    ap.add_argument(
        "--counties",
        default="Darıca,İzmit,Gebze,Başiskele,Çayırova,Gölcük,Kartepe,Körfez,Derince",
    )
    ap.add_argument(
        "--county-policy",
        default="core_good_ok",
        choices=["core_good_ok", "all", "explicit"],
        help="V25 county inclusion policy.",
    )
    ...
    ap.add_argument("--demographics-table", default="geo.neighborhood_demographics")
    ...
    ap.add_argument("--location-scope", choices=["global", "basiskele_only"], default="global")
    ...
    ap.add_argument("--model-scope", choices=["kocaeli_global", "basiskele_only", "ankara_only"], default="ankara_only")
    ap.add_argument(
        "--comparable-mode",
    ...
    ap.add_argument("--run-site-ablation", action=argparse.BooleanOptionalAction, default=False)
    ap.add_argument(
        "--run-v25-ablation",
    ...
    ap.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Optional V25 frequency site encoding check arm.",
    )
    ap.add_argument(
        "--site-repair-only",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="V24.1: rerun only site arms; use frozen V24 control/duplex refs.",
    )
    ap.add_argument(
        "--duplex-feature-mode",
        choices=["none", "flags", "largehome", "interactions", "full"],
        default="full",
        help="V23 duplex/large-home feature pack.",
    )
    ap.add_argument(
        "--run-duplex-ablation",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Run V23 duplex ablation vs control_v21_on_new_data.",
    )
    ap.add_argument("--merge-gap-warning-tl", type=float, default=8000.0)
