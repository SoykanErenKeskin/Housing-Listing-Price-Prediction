"""V26A Ankara-only core_good_ok policy + feature guards (forked from V25 Kocaeli policy).

No training logic — helpers consumed by train_v26a_ankara_only_pipeline.py.

County policy source: analysis_outputs/ankara_inventory_statistical_audit_2026-09-11_full/
(readiness thresholds defined in shared_scripts/audit_ankara_inventory_and_stats.py).
This is a research checkpoint only — Ankara-only, no multi-city, no deploy.
"""
...
MODEL_SCOPE = "ankara_only"
COUNTY_POLICY = "core_good_ok"

# Ankara GOOD + OK counties from the 2026-09-11 full inventory/statistical audit.
INCLUDED_COUNTIES = [
    "Çankaya",
    "Keçiören",
    "Mamak",
...
# Ankara WEAK counties (below OK readiness thresholds); deferred, not silently dropped.
EXCLUDED_COUNTIES = [
    "Kahramankazan",
    "Beypazarı",
    "Elmadağ",
...
EXCLUSION_REASON = (
    "WEAK inventory readiness on the 2026-09-11 Ankara full audit "
    "(analysis_outputs/ankara_inventory_statistical_audit_2026-09-11_full/): "
    "low sale/rental volume and/or sparse neighborhoods. Deferred from first V26A run."
)
...
# Informational cross-city reference only — V26A is Ankara-only and is never selected
# against this number. Kept so reports can show "how does this compare to the other
# city's checkpoint" without implying a multi-city model exists yet.
KOCAELI_V25_REFERENCE = {
    "experiment": "v25_kocaeli_full_refresh_core_good_ok",
    "path": "v3/outputs/v25_kocaeli_full_refresh_core_good_ok_full/",
    "r2": 0.731,
    "mape": 0.108,
    "rows": 14363,
    "note": "Different city, different data — informational only. Not a selection gate for V26A.",
}
...
def assert_no_stale_basiskele_metadata(payload: dict[str, Any]) -> list[str]:
    """Return warnings if payload still looks Başiskele-only or Kocaeli-scoped (stale for V26A)."""
    warns = []
    ...
    if scope.lower() == "kocaeli_global":
        warns.append("stale_kocaeli_global_scope")
    ...
    if str(payload.get("city") or "").lower() == "kocaeli":
        warns.append("stale_city_kocaeli")
    mv = str(payload.get("model_version") or "")
    if mv and "v26a" not in mv.lower():
        if "v18" in mv.lower() or "basiskele" in mv.lower() or "kocaeli" in mv.lower():
