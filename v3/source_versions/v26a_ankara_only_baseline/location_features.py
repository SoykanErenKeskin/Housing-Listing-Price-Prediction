...
# ---------------------------------------------------------------------------
# Ankara bounds (soft validation) — V26A fork of the Kocaeli geo module.
# Ankara province is landlocked; coast-distance features are disabled below
# (CITY_HAS_COAST=False) rather than computed against Kocaeli's gulf coastline.
# ---------------------------------------------------------------------------
LAT_MIN, LAT_MAX = 38.5, 40.9
LON_MIN, LON_MAX = 31.0, 34.2

CITY_HAS_COAST = False

# Approx static anchors (README: approximate public coordinates, not survey-grade)
# Ankara GOOD-readiness county centers (see v25_policy.INCLUDED_COUNTIES).
STATIC_ANCHORS: dict[str, tuple[float, float]] = {
    "ankara_center": (39.9334, 32.8597),
    "cankaya_center": (39.9255, 32.8663),
    "kecioren_center": (39.9900, 32.8650),
    "mamak_center": (39.9300, 32.9250),
    "sincan_center": (39.9650, 32.5750),
    "etimesgut_center": (39.9500, 32.6700),
    "yenimahalle_center": (39.9750, 32.7900),
}

ANCHOR_FEATURE_MAP = {
    "ankara_center": "distance_to_ankara_center_m",
    "cankaya_center": "distance_to_cankaya_center_m",
    "kecioren_center": "distance_to_kecioren_center_m",
    "mamak_center": "distance_to_mamak_center_m",
    "sincan_center": "distance_to_sincan_center_m",
    "etimesgut_center": "distance_to_etimesgut_center_m",
    "yenimahalle_center": "distance_to_yenimahalle_center_m",
}

# Ankara has no coastline. Kept as an empty polyline (not Kocaeli's gulf coast) so
# distance-to-coast features are skipped safely instead of computed against a
# geographically meaningless reference. See CITY_HAS_COAST below.
...
class LocationFeatureAdder(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        # Coast — safely disabled for landlocked cities (CITY_HAS_COAST=False / empty
        # COAST_POLYLINE) instead of computing distance against a foreign coastline.
        if CITY_HAS_COAST and COAST_POLYLINE:
            ...
            # No coast for this city: report unavailable, don't fabricate a distance.
            # coast_d stays defined (zeros) so downstream interaction features below
            # degrade to a neutral 0 contribution instead of crashing.
            coast_d = np.zeros(n, dtype=float)
            df["distance_to_coastline_m"] = np.nan
            df["is_coastal_500m"] = 0.0
            df["is_coastal_1000m"] = 0.0
            df["is_coastal_2000m"] = 0.0
            df["coast_distance_bucket"] = "unavailable_no_coast"
