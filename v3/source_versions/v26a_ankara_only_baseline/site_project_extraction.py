...
SITE_NAME_STOP = {
    ...
    "missing",
    "other",
    # Furniture / listing adjectives — never site stems (V24.1)
    "esyali",
    "esya",
    "mobilyali",
    "esyali_daire",
...
# Stems that must never become canonical site IDs even if extraction matched a phrase.
NON_SITE_STEMS = {
    "esyali",
    "esya",
    "mobilyali",
...
BASISKELE_SITE_DICTIONARY: tuple[SiteDictEntry, ...] = (
    ...
    # Bare "yakamoz" removed in V24.1 — it over-merged "yakamoz sitesi" into konutlari (severe gap).
    SiteDictEntry("yakamoz_konutlari", ("yakamoz konutlari",), "mid", False, "curated mid; bare yakamoz not aliased"),
...
def is_non_site_stem(stem: str) -> bool:
    s = fold_text(stem)
    if not s or s == "missing":
        return True
    if s in NON_SITE_STEMS:
        return True
    toks = [t for t in s.split() if t]
    return bool(toks) and all(t in NON_SITE_STEMS or t in SITE_NAME_STOP for t in toks)


def variant_key_to_id(raw_or_stem: str) -> str:
    """Anti-overmerge id from a raw variant key.

    Keep the folded phrase nearly intact (including sitesi/konutlari and county
    tokens) so severe price-gap variants do not collapse back to the same stem.
    """
    t = fold_text(raw_or_stem)
    if not t or t == "missing":
        return "missing"
    if is_non_site_stem(t):
        return "missing"
    return stem_to_id(t)


def build_merge_audit_rows(
    rows_meta: list[dict[str, Any]],
    site_counts: dict[str, int],
    merge_gap_warning_tl: float,
    *,
    id_field: str = "canonical_id",
    county_field: str | None = None,
) -> list[dict[str, Any]]:
    """Build merge-audit rows keyed by canonical (optionally county-scoped) id."""
    price_by_variant: dict[str, dict[str, list[float]]] = {}
    county_by_id: dict[str, str] = {}
    for r in rows_meta:
        cid = str(r.get(id_field) or "missing")
        if cid in {"missing", "other"} or cid.endswith("::other"):
            continue
        raw_k = fold_text(r.get("raw") or r.get("stem") or "") or "missing"
        p = r.get("unit_price")
        price_by_variant.setdefault(cid, {}).setdefault(raw_k, [])
        if pd.notna(p):
            price_by_variant[cid][raw_k].append(float(p))
        if county_field and county_field in r and cid not in county_by_id:
            county_by_id[cid] = str(r.get(county_field) or "")
    ...
    for cid, variants in price_by_variant.items():
        ...
        warning = "possible_bad_merge" if gap >= float(merge_gap_warning_tl) else ""
        row = {
            "canonical_id": cid,
            "raw_variants": " | ".join(sorted(variants.keys())),
            "n_variants": len(variants),
            "total_count": int(sum(len(ps) for ps in variants.values()) or site_counts.get(cid, 0)),
            "median_unit_price_by_variant": json.dumps(medians, ensure_ascii=False),
            "max_variant_price_gap": gap,
            "warning": warning,
        }
        if county_field is not None:
            if "::" in cid:
                row["county"] = unscope_county_display(cid)
            else:
                row["county"] = county_by_id.get(cid, "")
        out.append(row)
    return out


def unscope_county_display(scoped_id: str) -> str:
    """Best-effort county slug from scoped id (display left to caller if needed)."""
    s = str(scoped_id or "")
    if "::" not in s:
        return ""
    return s.split("::", 1)[0]
...
def severe_merge_mask(merge_df: pd.DataFrame) -> pd.Series:
    if merge_df is None or merge_df.empty:
        return pd.Series(dtype=bool)
    warn = merge_df["warning"].astype(str).eq("possible_bad_merge")
    return warn & (
        (pd.to_numeric(merge_df["total_count"], errors="coerce").fillna(0) >= 8)
        | (pd.to_numeric(merge_df["n_variants"], errors="coerce").fillna(0) >= 3)
    )


def split_severe_overmerges(
    rows_meta: list[dict[str, Any]],
    merge_gap_warning_tl: float,
    site_counts: dict[str, int],
) -> tuple[list[dict[str, Any]], list[dict[str, Any]], dict[str, int]]:
    """Split severe price-gap merges into per-variant IDs (anti-overmerge).

    Dict-locked rows keep their dictionary canonical id unless that id itself is severe,
    in which case non-dict variants are peeled off via variant_key_to_id.
    """
    merge_rows = build_merge_audit_rows(rows_meta, site_counts, merge_gap_warning_tl)
    merge_df = pd.DataFrame(merge_rows)
    if merge_df.empty:
        return rows_meta, merge_rows, site_counts

    severe_ids = set(merge_df.loc[severe_merge_mask(merge_df), "canonical_id"].astype(str).tolist())
    if not severe_ids:
        return rows_meta, merge_rows, site_counts

    for r in rows_meta:
        cid = str(r.get("canonical_id") or "missing")
        if cid not in severe_ids:
            continue
        raw_k = fold_text(r.get("raw") or r.get("stem") or "")
        new_id = variant_key_to_id(raw_k)
        if new_id in {"missing", ""}:
            # Drop garbage stems entirely
            r["canonical_id"] = "missing"
            r["entry"] = None
            continue
        # Peel off: use variant-specific id (even if dict previously collapsed them)
        r["canonical_id"] = new_id
        r["entry"] = None
        r["split_from_severe_merge"] = cid

    vc = pd.Series([r["canonical_id"] for r in rows_meta]).value_counts()
    new_counts = {str(k): int(v) for k, v in vc.items() if str(k) not in {"missing"}}
    new_merge = build_merge_audit_rows(rows_meta, new_counts, merge_gap_warning_tl)
    return rows_meta, new_merge, new_counts
...
class SiteProjectExtractionAdder(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        for _, row in df.iterrows():
            ...
            if is_non_site_stem(stem) or cid in NON_SITE_STEMS:
                cid = "missing"
                entry = None
                match_src = "missing"
        ...
        # V24.1: split severe price-gap over-merges into per-variant IDs, then rebuild audit
        rows_meta, self.merge_rows_, self.site_counts_ = split_severe_overmerges(
            rows_meta, self.merge_gap_warning_tl, self.site_counts_
        )

        # Refresh raw→canonical map after possible splits
        self.raw_to_canonical_ = {}
        for r in rows_meta:
            key = fold_text(r["raw"] or r["stem"])
            if key and r["canonical_id"] not in {"missing"}:
                self.raw_to_canonical_[key] = r["canonical_id"]
    ...
    def _resolve_one(self, row: pd.Series) -> dict[str, Any]:
        ...
        stem = normalize_site_stem(raw) if raw else "missing"
        audit = fold_text(build_audit_text(row))
        if is_non_site_stem(stem):
            return {
                "raw": raw,
                "stem": stem,
                "site_project_id": "missing",
                "has_site_project_id": 0,
                "site_project_listing_count": 0.0,
                "site_project_freq_bucket": 0.0,
                "site_project_known_premium_flag": 0,
                "site_project_match_source": "missing",
                "site_quality_tier": "unknown",
                "site_is_premium_tier": 0,
                "site_is_mid_tier": 0,
                "site_tier_code": 0.0,
            }

        # Prefer fit-time anti-overmerge map (severe split / rare remap)
        map_key = fold_text(raw or stem)
        mapped = self.raw_to_canonical_.get(map_key) if map_key else None

        entry = None
        if mapped is not None:
            cid = str(mapped)
            match_source = src if src != "missing" else "alias_map"
            known = 0
            tier = "unknown"
            # recover dict metadata when mapped id is a curated entry
            for e in BASISKELE_SITE_DICTIONARY:
                if e.canonical_id == cid:
                    entry = e
                    known = int(e.known_premium)
                    tier = e.quality_tier if mode in {"tier", "interactions", "full"} else "unknown"
            ...
            if entry is not None:
                ...
                # Prefer variant-specific id when available (anti-overmerge), else stem id
                cid = variant_key_to_id(raw) if raw else (stem_to_id(stem) if stem not in {"missing", ""} else "missing")
                if cid in {"missing"} and stem not in {"missing", ""}:
                    cid = stem_to_id(stem)
                if cid not in {"missing"} and int(self.site_counts_.get(cid, 0)) < self.min_site_freq:
                    # site_counts_ may be county-scoped in Kocaeli adder — fallback other only for local keys
                    local_cnt = int(self.site_counts_.get(cid, 0))
                    if local_cnt < self.min_site_freq and not any(str(k).endswith(f"::{cid}") for k in self.site_counts_):
                        cid = "other"
                match_source = src
                known = 0
                tier = "unknown"

        # Frequency: try exact id then any county-scoped suffix match
        cnt = 0.0
        if cid not in {"missing"}:
            if cid in self.site_counts_:
                cnt = float(self.site_counts_[cid])
            else:
                suf = f"::{cid}"
                cnt = float(sum(v for k, v in self.site_counts_.items() if str(k).endswith(suf)))
        return {
            "raw": raw,
            "stem": stem,
            "site_project_id": cid,
...
def count_severe_bad_merges(merge_df: pd.DataFrame) -> int:
    if merge_df is None or merge_df.empty:
        return 0
    return int(severe_merge_mask(merge_df).sum())


def severe_merge_rows(merge_df: pd.DataFrame, experiment: str = "") -> pd.DataFrame:
    """Return severe merge rows with required review columns (+ optional experiment)."""
    cols = [
        "canonical_id",
        "raw_variants",
        "county",
    ...
    m = merge_df.copy()
    if "county" not in m.columns:
        m["county"] = m["canonical_id"].astype(str).map(
            lambda x: x.split("::", 1)[0] if "::" in str(x) else ""
        )
    m["experiment"] = str(experiment or "")
    out = m.loc[severe_merge_mask(m), cols].copy()
