...
"""
V25 Kocaeli Full Refresh Global Model — core_good_ok.

Scaffolded from V24.1 without modifying V24.1 trees.
County policy: GOOD+OK only (exclude Karamürsel/Kandıra/Dilovası).
Preserve V24.1 feature stack; add schema/heating/county guards.

Primary comparison: control_v24_1_replay_on_core_good_ok on same refreshed data.
Do not auto-promote over V24.1 production in this task.
"""
...
if __name__ == "__main__":
    ...
    if not _no_interactive:
        sys.stdout.write("V25 Kocaeli full-refresh core_good_ok ayar sihirbazı açılıyor...\n")
        sys.stdout.flush()
...
try:
    from ankara_site_project_extraction import (
        SITE_UNIT_PRICE_COL,
        SiteProjectExtractionAdder,
        SiteProjectFoldSafeEncoder,
        get_site_categorical_feature_names,
        get_site_feature_names,
        coverage_table,
        alias_map_table,
        dictionary_table,
        candidates_table,
        ...
        write_leakage_guard,
        coverage_by_county,
        write_site_project_options,
        SITE_NUMERIC_FEATURES,
        SITE_CATEGORICAL_FEATURES,
        INTERACTION_CATEGORICAL,
        FOLDSAFE_NUMERIC,
    )
    from site_project_extraction import severe_merge_rows
except ImportError:
    from ankara_site_project_extraction import (
        SITE_UNIT_PRICE_COL,
        SiteProjectExtractionAdder,
...
# V20 parity control only
from premium_signal_features import (
    PremiumSignalFeatureAdder as V20PremiumSignalFeatureAdder,
    SiteProjectFoldSafeEncoder as V20SiteProjectFoldSafeEncoder,
    PREMIUM_UNIT_PRICE_COL,
    get_premium_feature_names as get_v20_premium_feature_names,
    get_premium_categorical_feature_names as get_v20_premium_categorical_feature_names,
)

from duplex_largehome_features import (
    DUPLEX_TEXT_SOURCE_COLUMNS,
    DuplexLargeHomeFeatureAdder,
    get_duplex_categorical_feature_names,
    get_duplex_numeric_feature_names,
    write_duplex_extraction_reports,
    extract_duplex_frame,
    duplex_source_coverage_metrics,
    LARGE_HOME_M2,
)
...
from v25_policy import (
    MODEL_VERSION as V25_MODEL_VERSION,
    MODEL_SCOPE as V25_MODEL_SCOPE,
    COUNTY_POLICY as V25_COUNTY_POLICY,
    INCLUDED_COUNTIES as V25_INCLUDED_COUNTIES,
    EXCLUDED_COUNTIES as V25_EXCLUDED_COUNTIES,
    V24_1_REFERENCE,
    KOCAELI_V25_REFERENCE,
    normalize_source_site,
    resolve_counties_for_policy,
    limit_per_county,
    write_county_policy_report,
    detailed_filter_reasons,
    heating_yerden_audit,
    build_feature_schema_diff,
    assert_no_stale_basiskele_metadata,
)
...
DEFAULT_SOURCE_SITE = normalize_source_site(os.getenv("SOURCE_SITE", "sahibinden"))
DEFAULT_TREND_TABLE = "market.price_observations"
DEFAULT_CITY = "Ankara"
DEFAULT_COUNTIES = list(V25_INCLUDED_COUNTIES)
MODEL_SCOPE = V25_MODEL_SCOPE
TARGET = "unit_price_gross"
RANDOM_STATE = 42

# Frozen V24 full-run references (do not retrain unless needed)
V24_CONTROL_REF = {
    "experiment": "control_v17_or_v21_like_global",
    "r2": 0.6355303940198862,
    "mape": 0.12012936749595975,
    "variance_ratio": 0.6147205314789795,
    "basiskele_r2": 0.4862605571949984,
    "izmit_r2": 0.6625129410605489,
    "golcuk_r2": 0.6609125400563349,
    "karamursel_r2": 0.5774965050458485,
    "kartepe_r2": 0.5309089223206018,
    "merge_audit_severe_warnings": 0,
    "leakage_pass": True,
}
V24_SAFE_DUPLEX_REF = {
    "experiment": "duplex_largehome_global",
    "site_extraction_mode": "none",
    "site_project_encoding": "none",
    "duplex_feature_mode": "full",
    "r2": 0.6358656162289547,
    "mape": 0.12003687930512741,
    "variance_ratio": 0.6184810005227498,
    "basiskele_r2": 0.48961528851390135,
    "izmit_r2": 0.6633843128240349,
    "golcuk_r2": 0.657070542679931,
    "karamursel_r2": 0.5768160507548209,
    "kartepe_r2": 0.5304984704509776,
    "merge_audit_severe_warnings": 0,
    "leakage_pass": True,
    "note": "V24 safe selected arm; keep until site merge severe=0 and gates pass.",
}
...
SITE_CATEGORICAL_FEATURES_V21 = list(SITE_CATEGORICAL_FEATURES) + list(INTERACTION_CATEGORICAL)
...
def resolve_categorical_features(
    ...
    duplex_feature_mode: str = "none",
) -> list[str]:
    names = list(CATEGORICAL_FEATURES_BASE)
    mode = str(site_extraction_mode or "none").lower()
    if mode == "v20_parity":
        names += get_v20_premium_categorical_feature_names("site", site_project_encoding)
    else:
        names += get_site_categorical_feature_names(site_extraction_mode, site_project_encoding)
    names += get_duplex_categorical_feature_names(duplex_feature_mode)
    seen: set[str] = set()
    out: list[str] = []
    for n in names:
        if n not in seen:
...
CATEGORICAL_FEATURES = CATEGORICAL_FEATURES_BASE + list(SITE_CATEGORICAL_FEATURES_V21)
...
# V21 site extraction needs free-text sources in X for SiteProjectExtractionAdder.
...
_PREMIUM_TEXT_SOURCE_COLUMNS = {
    ...
    "detail_ic_ozellikler",
    "detail_dis_ozellikler",
} | set(DUPLEX_TEXT_SOURCE_COLUMNS)
LEAKAGE_OR_UNUSED_COLUMNS_V21 = LEAKAGE_OR_UNUSED_COLUMNS - _PREMIUM_TEXT_SOURCE_COLUMNS
LEAKAGE_OR_UNUSED_COLUMNS_V23 = LEAKAGE_OR_UNUSED_COLUMNS_V21
...
class RunConfig:
    ...
    site_extraction_mode: str = "full"
    site_project_encoding: str = "foldsafe_target"
    run_site_ablation: bool = False
    site_repair_only: bool = False
    duplex_feature_mode: str = "full"
    run_duplex_ablation: bool = False
    merge_gap_warning_tl: float = 8000.0
    model_scope: str = "kocaeli_global"
...
def parse_counties(s: str | None) -> list[str]:
    """Parse comma-separated counties; default = Kocaeli core set."""
    if s is None or not str(s).strip():
        return list(DEFAULT_COUNTIES)
    parts = [p.strip() for p in str(s).split(",") if p.strip()]
    return parts or list(DEFAULT_COUNTIES)
...
def fetch_listing_table(
    ...
    """Fetch Kocaeli listings for one or more counties (canonical DB + legacy DF aliases)."""
    try:
    ...
    return _fetch_listing_table(
        ...
        limit=limit,
        counties=counties,
        source_site=normalize_source_site(DEFAULT_SOURCE_SITE),
...
def load_raw_data(args: argparse.Namespace, out_dirs: dict[str, Path]) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if args.sale_json and args.rental_json:
        ...
        counties = parse_counties(getattr(args, "counties", None))
        sales_raw = fetch_listing_table(engine, args.sale_table, "sale", args.city, args.limit_sale, counties=counties)
        lsp = getattr(args, "limit_sale_per_county", None)
        if lsp:
            sales_raw = limit_per_county(sales_raw, int(lsp))
        print(f"Raw sale rows: {len(sales_raw)} | counties={counties} | limit_sale_per_county={lsp}")

        print("DB rental listings are being fetched...")
        rentals_raw = fetch_listing_table(engine, args.rental_table, "rental", args.city, args.limit_rental, counties=counties)
        lrp = getattr(args, "limit_rental_per_county", None)
        if lrp:
            rentals_raw = limit_per_county(rentals_raw, int(lrp))
        print(f"Raw rental rows: {len(rentals_raw)} | limit_rental_per_county={lrp}")
...
def resolve_numeric_features(
    ...
    mode = str(site_extraction_mode or "none").lower()
    if mode == "v20_parity":
        site_names = get_v20_premium_feature_names("site", site_project_encoding)
    else:
        site_names = get_site_feature_names(site_extraction_mode, site_project_encoding)
    names = (
        ...
        + site_names
        + get_duplex_numeric_feature_names(duplex_feature_mode)
    )
    seen: set[str] = set()
    out: list[str] = []
    for n in names:
...
def build_preprocessor(
    ...
    cat_features = resolve_categorical_features(site_extraction_mode, site_project_encoding, duplex_feature_mode)
...
def build_pipeline(
    ...
    site_mode = str(site_extraction_mode or "none").lower()
    site_enc = str(site_project_encoding or "none").lower()
    duplex_mode = str(duplex_feature_mode or "none").lower()
    numeric_features = resolve_numeric_features(mode, detail_mode, specialist_mode, lh_regime, kar_mode, location_feature_mode, geo_context_mode, str(comparable_mode or "none"), site_mode, site_enc, duplex_mode)
    steps = [
    ...
    if site_mode == "v20_parity":
        steps.append(
            (
                "premium_signals",
                V20PremiumSignalFeatureAdder(
                    premium_feature_mode="site",
                    site_project_encoding=site_enc if site_enc != "none" else "frequency",
                    min_site_freq=3,
                ),
            )
        )
        steps.append(
            (
                ...
                V20SiteProjectFoldSafeEncoder(
                    enabled=(site_enc == "foldsafe_target"),
                    min_count=3,
                    alpha=20.0,
                    premium_feature_mode="site",
                ),
        ...
        steps.append(
            (
                "site_extraction",
                SiteProjectExtractionAdder(
                    site_extraction_mode=site_mode,
                    site_project_encoding=site_enc,
                    min_site_freq=1,
                    merge_gap_warning_tl=float(merge_gap_warning_tl or 8000.0),
                ),
            )
        )
        steps.append(
            (
                "site_project_foldsafe",
                SiteProjectFoldSafeEncoder(
                    enabled=(site_enc == "foldsafe_target" and site_mode not in {"", "none"}),
                    min_count=3,
                    alpha=20.0,
                    site_extraction_mode=site_mode,
                ),
    ...
    if duplex_mode not in {"", "none"}:
        steps.append(
            (
                "duplex_largehome",
                DuplexLargeHomeFeatureAdder(duplex_feature_mode=duplex_mode),
            )
        )
    steps.append(
        (
    ...
    steps.extend(
        [
            ("feature_columns", FeatureColumnKeeper(numeric_features, resolve_categorical_features(site_mode, site_enc, duplex_mode))),
            (
                "preprocess",
                build_preprocessor(
...
def model_specs(target_mode: str, selected_models: list[str] | None = None, fast_mode: bool = False, attribute_mode: str = "full", detail_effect_mode: str = "group", basiskele_specialist_mode: str = "premium_target_stats", basiskele_large_home_regime: str = "none", karamursel_baseline_mode: str = "none", location_feature_mode: str = "full", geo_context_mode: str = "full", location_min_precision: str = "any", enable_coordinate_noise_check: bool = True, comparable_k_list: str = "5,10,20", comparable_mode: str = "none", site_extraction_mode: str = "full", site_project_encoding: str = "foldsafe_target", merge_gap_warning_tl: float = 8000.0, geo_context_cache_dir: str = "data/external/geo_context", location_scope: str = "basiskele_only", location_coverage_min: float = 0.40, duplex_feature_mode: str = "none") -> dict[str, Any]:
    ...
    registry = {
        "ridge": build_pipeline(
            ...
            duplex_feature_mode=duplex_feature_mode,
        ),
...
def make_training_matrix(sales: pd.DataFrame) -> tuple[pd.DataFrame, pd.Series]:
    y = pd.to_numeric(sales[TARGET], errors="coerce")
    X = sales.drop(
        columns=[c for c in LEAKAGE_OR_UNUSED_COLUMNS_V21 if c in sales.columns],
        errors="ignore",
    ).copy()
...
def train_and_evaluate(X: pd.DataFrame, y: pd.Series, cfg: RunConfig, out_dirs: dict[str, Path]) -> tuple[ModelBundle, dict[str, Any]]:
    ...
    leakage = {
        ...
        "model_scope": str(getattr(cfg, "model_scope", MODEL_SCOPE) or MODEL_SCOPE),
        "county": None,
    ...
    metrics_summary["model_scope"] = str(getattr(cfg, "model_scope", MODEL_SCOPE) or MODEL_SCOPE)
    metrics_summary["county"] = None
    metrics_summary["rows"] = int(len(oof))
    metrics_summary["r2"] = float(ensemble_metrics.get("r2", bsk_metrics["r2"]))
    metrics_summary["mape"] = float(ensemble_metrics.get("mape", bsk_metrics["mape"]))
    metrics_summary["variance_ratio"] = float(
        metrics_summary.get("variance_ratio")
        if metrics_summary.get("variance_ratio") is not None
        else bsk_metrics["variance_ratio"]
    )
    ...
    site_mode_cfg = str(getattr(cfg, "site_extraction_mode", "full"))
    site_enc_cfg = str(getattr(cfg, "site_project_encoding", "foldsafe_target"))
    metrics_summary["selected_site_extraction_mode"] = site_mode_cfg
    metrics_summary["selected_site_project_encoding"] = site_enc_cfg
    metrics_summary["historical_reference"] = {
        "v20_selected_reference": {
            "selected_experiment": "site_foldsafe_target",
            "r2": 0.5017,
            "mape": 0.1060,
            "variance_ratio": 0.4616,
            "expensive_decile_bias": -10139,
            "has_site_project_name_rate": 0.20,
            "known_premium_rate": 0.067,
        },
        "v18_geo_control_reference": {
            "r2": 0.4730,
            "mape": 0.1093,
        ...
        "v24_control_reference": dict(V24_CONTROL_REF),
        "v24_safe_duplex_reference": dict(V24_SAFE_DUPLEX_REF),
        "decision_basiskele_slice": bsk_decision,
    }
    # Keep nested Başiskele slice for diagnostics, but do not present as top-level scope.
    metrics_summary["basiskele_slice"] = {
        "rows": bsk_metrics["rows"],
        "r2": bsk_metrics["r2"],
        "mape": bsk_metrics["mape"],
        "variance_ratio": bsk_metrics["variance_ratio"],
        "decision": bsk_decision,
    }
    metrics_summary["comparable_leakage_guard"] = leakage
    metrics_summary["note"] = (
        "V24.1 Kocaeli global site-merge repair. model_scope=kocaeli_global; "
        "not auto-promoted until severe merge warnings are cleared and selection gates pass."
    )
    ...
    write_leakage_guard(out_dirs["reports"] / "site_project_encoding_leakage_guard_v24.json", site_leak)
    ...
    # V21 site-extraction diagnostics
    ...
    try:
        ...
        for col in ("district", "gross_m2", "room_count", "site_inside", "distance_to_coastline_m", "unit_price_gross"):
            if col in X.columns and col not in prem_frame.columns:
                prem_frame[col] = X[col].to_numpy()
        if "unit_price_gross" not in prem_frame.columns and "actual_unit_price_gross" in prem_frame.columns:
            prem_frame["unit_price_gross"] = prem_frame["actual_unit_price_gross"]

        site_mode_d = str(getattr(cfg, "site_extraction_mode", "full"))
        site_enc_d = str(getattr(cfg, "site_project_encoding", "foldsafe_target"))
        gap_tl = float(getattr(cfg, "merge_gap_warning_tl", 8000.0))

        if site_mode_d == "v20_parity":
            adder = V20PremiumSignalFeatureAdder(
                premium_feature_mode="site",
                site_project_encoding=site_enc_d if site_enc_d != "none" else "frequency",
                min_site_freq=3,
            )
            prem_frame = adder.fit_transform(prem_frame)
            # map V20 columns to V21 diagnostic names
            if "site_project_name_normalized" in prem_frame.columns and "site_project_id" not in prem_frame.columns:
                prem_frame["site_project_id"] = prem_frame["site_project_name_normalized"]
            if "has_site_project_name" in prem_frame.columns and "has_site_project_id" not in prem_frame.columns:
                prem_frame["has_site_project_id"] = prem_frame["has_site_project_name"]
            coverage_metrics = {
                "extracted_raw_rate": float(pd.to_numeric(prem_frame.get("has_site_project_id"), errors="coerce").fillna(0).mean()),
                "alias_hit_rate": float("nan"),
                "dict_hit_rate": float(pd.to_numeric(prem_frame.get("site_project_known_premium_flag"), errors="coerce").fillna(0).mean()),
                "canonical_non_missing_rate": float(pd.to_numeric(prem_frame.get("has_site_project_id"), errors="coerce").fillna(0).mean()),
                "foldsafe_encoded_site_count": float((site_leak or {}).get("n_sites_encoded") or 0),
            }
            pd.DataFrame([{"metric": k, "value": v} for k, v in coverage_metrics.items()]).to_csv(
                out_dirs["reports"] / "site_project_coverage_v24.csv", index=False, encoding="utf-8-sig"
            )
            dictionary_table().to_csv(out_dirs["reports"] / "site_project_dictionary_v24.csv", index=False, encoding="utf-8-sig")
            candidates_table(prem_frame).to_csv(out_dirs["reports"] / "site_project_candidates_v24.csv", index=False, encoding="utf-8-sig")
            pd.DataFrame(columns=["raw_folded", "canonical_id"]).to_csv(
                out_dirs["reports"] / "site_project_alias_map_v24.csv", index=False, encoding="utf-8-sig"
            )
            pd.DataFrame(
                columns=[
                    "raw_name",
            ...
            ).to_csv(out_dirs["reports"] / "site_project_alias_review_needed_v24.csv", index=False, encoding="utf-8-sig")
            pd.DataFrame(
                columns=[
                    "canonical_id",
                    "raw_variants",
                    "n_variants",
                    "total_count",
                    "median_unit_price_by_variant",
                    "max_variant_price_gap",
                    "warning",
                ]
            ).to_csv(out_dirs["reports"] / "site_project_merge_audit_v24.csv", index=False, encoding="utf-8-sig")
        else:
            adder = SiteProjectExtractionAdder(
                site_extraction_mode=site_mode_d,
                site_project_encoding=site_enc_d,
                min_site_freq=1,
                merge_gap_warning_tl=gap_tl,
            )
            prem_frame = adder.fit_transform(prem_frame)
            coverage_metrics = dict(adder.coverage_ or {})
            coverage_metrics["foldsafe_encoded_site_count"] = float((site_leak or {}).get("n_sites_encoded") or 0)

            # expensive underpred site-missing rate
            ...
            try:
                ...
                miss = pd.Series(prem_frame["site_project_id"]).astype(str).isin(["missing", "other", ""])
                coverage_metrics["top_expensive_underpredicted_site_missing_rate"] = (
                    float(miss[exp_mask].mean()) if int(exp_mask.sum()) else float("nan")
                )
            except Exception:
                coverage_metrics["top_expensive_underpredicted_site_missing_rate"] = float("nan")

            coverage_table(adder).to_csv(out_dirs["reports"] / "site_project_coverage_v24.csv", index=False, encoding="utf-8-sig")
            # overwrite with enriched metrics including expensive missing
            pd.DataFrame([{"metric": k, "value": v} for k, v in coverage_metrics.items()]).to_csv(
                out_dirs["reports"] / "site_project_coverage_v24.csv", index=False, encoding="utf-8-sig"
            )
            alias_map_table(adder).to_csv(out_dirs["reports"] / "site_project_alias_map_v24.csv", index=False, encoding="utf-8-sig")
            review_df = pd.DataFrame(adder.review_rows_ or [])
            if not review_df.empty:
                review_df = enrich_review_with_prices(review_df, prem_frame)
            else:
            ...
            review_df.to_csv(out_dirs["reports"] / "site_project_alias_review_needed_v24.csv", index=False, encoding="utf-8-sig")
            merge_df = pd.DataFrame(adder.merge_rows_ or [])
            if merge_df.empty:
            ...
            if "county" not in merge_df.columns:
                merge_df["county"] = merge_df["canonical_id"].astype(str).map(
                    lambda x: x.split("::", 1)[0] if "::" in str(x) else ""
                )
            exp_tag = str(getattr(cfg, "ablation_experiment_name", "") or "")
            merge_df.to_csv(out_dirs["reports"] / "site_project_merge_audit_v24.csv", index=False, encoding="utf-8-sig")
            severe_df = severe_merge_rows(merge_df, experiment=exp_tag or site_mode_d)
            severe_df.to_csv(
                out_dirs["reports"] / "site_project_merge_audit_severe_v24.csv",
                index=False,
                encoding="utf-8-sig",
            )
            dictionary_table().to_csv(out_dirs["reports"] / "site_project_dictionary_v24.csv", index=False, encoding="utf-8-sig")
            candidates_table(prem_frame).to_csv(out_dirs["reports"] / "site_project_candidates_v24.csv", index=False, encoding="utf-8-sig")
            try:
                coverage_by_county(prem_frame).to_csv(
                    out_dirs["reports"] / "site_project_coverage_v24_by_county.csv",
                    index=False,
                    encoding="utf-8-sig",
                )
            except Exception:
            ...
            tmp["_y"] = yv.to_numpy()
            tmp["_p"] = pv.to_numpy()
            if "site_project_id" in tmp.columns:
                perf = (
                    tmp.groupby("site_project_id", dropna=False)
                    .agg(n=("_y", "size"), mean_actual=("_y", "mean"), mean_pred=("_p", "mean"), mean_residual=("_resid", "mean"))
                    .reset_index()
                    .sort_values("mean_residual")
                )
                perf.to_csv(out_dirs["reports"] / "site_project_performance_v24.csv", index=False, encoding="utf-8-sig")

        metrics_summary["site_coverage"] = coverage_metrics
        metrics_summary["merge_audit_severe_warnings"] = int(
            count_severe_bad_merges(pd.read_csv(out_dirs["reports"] / "site_project_merge_audit_v24.csv"))
            if (out_dirs["reports"] / "site_project_merge_audit_v24.csv").exists()
            else 0
        )
    except Exception as site_diag_exc:
        warnings.warn(f"V21 site diagnostics skipped: {site_diag_exc}")
        coverage_metrics = {"error": str(site_diag_exc)}
        metrics_summary["site_coverage"] = coverage_metrics

    # decision block for V21 summary
    decision = {
        "selected_site_extraction_mode": site_mode_cfg,
        ...
        "expensive_decile_bias": exp_bias,
        "large_home_r2": lh_r2,
        "leakage_guard_pass": bool((site_leak or {}).get("pass", True)),
        "coverage": coverage_metrics,
        "best_checkpoint": False,
        "note": "V24.1 is not auto-promoted; promote only after severe merge warnings=0 and selection gates pass.",
    }
    metrics_summary["decision"] = decision


    metrics_summary["warnings"] = list(metrics_summary.get("warnings") or [])
    if not bool((site_leak or {}).get("pass", True)):
        metrics_summary["warnings"].append("site_project_encoding_leakage_guard_failed")
    metrics_summary["qa_findings"] = {
        "site_extraction_mode": site_mode_cfg,
    ...
    (out_dirs["reports"] / "metrics_summary_v24_kocaeli.json").write_text(
        json.dumps(metrics_summary, indent=2, ensure_ascii=False), encoding="utf-8"
    )
    (out_dirs["reports"] / "metrics_summary_v21_basiskele.json").write_text(
        json.dumps(metrics_summary, indent=2, ensure_ascii=False), encoding="utf-8"
    )
...
def parse_args() -> argparse.Namespace:
    ...
    ap = argparse.ArgumentParser(
        description="V25 Kocaeli full-refresh core_good_ok global pipeline. "
    ...
    ap.add_argument("--counties", default=",".join(V25_INCLUDED_COUNTIES), help="Comma-separated county list (overridden by --county-policy unless explicit).")
    ap.add_argument("--county-policy", default="core_good_ok", choices=["core_good_ok", "all", "explicit"], help="V25 county inclusion policy.")
    ap.add_argument("--out", default="v3/outputs/v25_kocaeli_full_refresh_core_good_ok")
    ...
    ap.add_argument("--limit-sale-per-county", type=int, default=None, help="Smoke/sample cap per county for sale rows.")
    ap.add_argument("--limit-rental-per-county", type=int, default=None, help="Smoke/sample cap per county for rental rows.")
    ...
    ap.add_argument(
        ...
        default="global",
        help="Apply location features globally (V24 default) or only to Başiskele.",
    )
    ap.add_argument(
    ...
    ap.add_argument(
        "--model-scope",
        choices=["kocaeli_global", "basiskele_only", "ankara_only"],
        default="ankara_only",
        help="V26A default: Ankara-only multi-county (within Ankara) research model.",
    )
    ap.add_argument(
        "--comparable-mode",
    ...
    ap.add_argument(
        "--site-extraction-mode",
        choices=["none", "v20_parity", "alias", "dict", "tier", "interactions", "full"],
        default="full",
        help="V21 site extraction: none/v20_parity/alias/dict/tier/interactions/full.",
    )
    ap.add_argument(
        "--site-project-encoding",
        choices=["none", "frequency", "foldsafe_target"],
        default="foldsafe_target",
    ...
    ap.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Run V24/V24.1 site-extraction ablation matrix.",
    )
    ap.add_argument(
        "--run-v25-ablation",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Run V25 core_good_ok ablation: control replay + full_v25. (Kocaeli scaffold; unused for Ankara.)",
    )
    ap.add_argument(
        "--run-v26a-ablation",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Run V26A Ankara-only ablation: control_basic_geo_ankara + full_v26a_ankara + no_site_project_ablation.",
    )
    ap.add_argument(
        "--v25-site-encoding-check",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Optional V25 arm: frequency site encoding check.",
    )
    ap.add_argument(
        "--site-repair-only",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="V24.1: rerun only site arms (skip control/duplex); use frozen V24 refs for selection.",
    )

    ap.add_argument(
        "--duplex-feature-mode",
        choices=["none", "flags", "largehome", "interactions", "full"],
        default="full",
        help="V23 duplex/large-home feature pack: none/flags/largehome/interactions/full.",
    )
    ap.add_argument(
        "--run-duplex-ablation",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Run V23 duplex ablation matrix vs control_v21_on_new_data.",
    )
    ap.add_argument(
        "--merge-gap-warning-tl",
        type=float,
        default=8000.0,
        help="Median unit-price gap across merged variants that triggers possible_bad_merge.",
    )
    ap.add_argument(
        "--interactive",
        action=argparse.BooleanOptionalAction,
...
# ---------------------------------------------------------------------------
# V26A preflight: coordinate quality audit + heating feature audit
# ---------------------------------------------------------------------------

_ANKARA_LAT_MIN, _ANKARA_LAT_MAX = 38.5, 40.9
_ANKARA_LON_MIN, _ANKARA_LON_MAX = 31.0, 34.2


def build_coordinate_quality_audit(sales: pd.DataFrame, rentals: pd.DataFrame) -> dict[str, Any]:
    """Preflight coordinate-quality check for V26A (Ankara). Never trains anything.

    PASS/WARN/FAIL gates the ablation loop below: FAIL blocks full training.
    """
    both = pd.concat([sales.assign(_purpose="sale"), rentals.assign(_purpose="rental")], ignore_index=True) if len(sales) or len(rentals) else pd.DataFrame()
    result: dict[str, Any] = {
        "model_version": "v26a-ankara-only-core-good-ok",
        "bounds_used": {"lat_min": _ANKARA_LAT_MIN, "lat_max": _ANKARA_LAT_MAX, "lon_min": _ANKARA_LON_MIN, "lon_max": _ANKARA_LON_MAX},
        "total_rows": int(len(both)),
    }
    lat_col = "lat" if "lat" in both.columns else ("latitude" if "latitude" in both.columns else None)
    lon_col = "lon" if "lon" in both.columns else ("longitude" if "longitude" in both.columns else None)
    if both.empty or lat_col is None or lon_col is None:
        result["coordinateQualityStatus"] = "FAIL"
        result["reason"] = "no lat/lon (or latitude/longitude) columns available"
        return result
    if lat_col != "lat":
        both = both.assign(lat=both[lat_col])
    if lon_col != "lon":
        both = both.assign(lon=both[lon_col])

    lat = pd.to_numeric(both["lat"], errors="coerce")
    lon = pd.to_numeric(both["lon"], errors="coerce")
    has_coord = lat.notna() & lon.notna()
    coverage = float(has_coord.mean()) if len(both) else float("nan")
    out_of_bounds = has_coord & ((lat < _ANKARA_LAT_MIN) | (lat > _ANKARA_LAT_MAX) | (lon < _ANKARA_LON_MIN) | (lon > _ANKARA_LON_MAX))
    out_of_bounds_rate = float(out_of_bounds.mean()) if len(both) else float("nan")

    pair = both.loc[has_coord].assign(_lat_r=lat.loc[has_coord].round(5), _lon_r=lon.loc[has_coord].round(5))
    dup_counts = pair.groupby(["_lat_r", "_lon_r"]).size().sort_values(ascending=False)
    dup_rate = float((dup_counts[dup_counts > 1].sum()) / len(pair)) if len(pair) else float("nan")
    top_repeated = [
        {"lat": float(la), "lon": float(lo), "count": int(c)}
        for (la, lo), c in dup_counts.head(15).items()
        if c > 1
    ]
    # Centroid-like clustering warning: many rows sharing the exact same point in a
    # single county usually means the scraper fell back to a county/district centroid
    # instead of a real address pin.
    centroid_like_counties: list[str] = []
    if "county" in pair.columns:
        for county, g in pair.groupby(pair["county"].astype(str)):
            if len(g) < 20:
                continue
            top = g.groupby(["_lat_r", "_lon_r"]).size().max()
            if top / len(g) >= 0.25:
                centroid_like_counties.append(str(county))

    by_county = []
    if "county" in both.columns:
        for county, g in both.groupby(both["county"].astype(str)):
            glat = pd.to_numeric(g["lat"], errors="coerce")
            glon = pd.to_numeric(g["lon"], errors="coerce")
            ghas = glat.notna() & glon.notna()
            by_county.append(
                {
                    "county": county,
                    "rows": int(len(g)),
                    "coverage": float(ghas.mean()) if len(g) else float("nan"),
                    "lat_min": float(glat.min()) if ghas.any() else None,
                    "lat_max": float(glat.max()) if ghas.any() else None,
                    "lon_min": float(glon.min()) if ghas.any() else None,
                    "lon_max": float(glon.max()) if ghas.any() else None,
                    "out_of_ankara_bounds_count": int(
                        (ghas & ((glat < _ANKARA_LAT_MIN) | (glat > _ANKARA_LAT_MAX) | (glon < _ANKARA_LON_MIN) | (glon > _ANKARA_LON_MAX))).sum()
                    ),
                }
            )

    all_identical = bool(len(pair) > 0 and dup_counts.iloc[0] == len(pair)) if len(dup_counts) else False
    mostly_centroided = bool(len(pair) > 0 and dup_counts.iloc[0] / len(pair) >= 0.5) if len(dup_counts) else False

    status = "PASS"
    reasons = []
    if all_identical or mostly_centroided or out_of_bounds_rate > 0.10:
        status = "FAIL"
        if all_identical:
            reasons.append("all_coordinates_identical")
        if mostly_centroided:
            reasons.append("majority_of_rows_share_one_point")
        if out_of_bounds_rate > 0.10:
            reasons.append(f"out_of_bounds_rate={out_of_bounds_rate:.3f}>0.10")
    elif dup_rate > 0.15 or centroid_like_counties or (0.0 < out_of_bounds_rate <= 0.10):
        status = "WARN"
        if dup_rate > 0.15:
            reasons.append(f"duplicate_coordinate_rate={dup_rate:.3f}>0.15")
        if centroid_like_counties:
            reasons.append(f"centroid_like_counties={centroid_like_counties}")
        if out_of_bounds_rate > 0.0:
            reasons.append(f"mild_out_of_bounds_rate={out_of_bounds_rate:.4f}")

    result.update(
        {
            "coordinate_coverage": coverage,
            "duplicate_coordinate_rate": dup_rate,
            "top_repeated_coordinates": top_repeated,
            "out_of_bounds_rate": out_of_bounds_rate,
            "centroid_like_counties_warning": centroid_like_counties,
            "by_county": by_county,
...
def build_heating_feature_audit_v26a(
    sales: pd.DataFrame,
    rentals: pd.DataFrame,
    encoded_feature_names: list[str] | None = None,
) -> pd.DataFrame:
    """Per-heating-value audit for V26A (Ankara). Explicitly checks Yerden Isıtma
    survives raw -> encoded features (not collapsed into unknown/other)."""
    enc = [str(n).lower() for n in (encoded_feature_names or [])]

    def _encoded_present(value: str) -> bool:
        if not enc:
            return False
        v = str(value).strip().lower()
        return any(v in n or "heating_" + v.replace(" ", "_") in n for n in enc) or any(
            "heating" in n and v.split()[0] in n for n in enc
        )

    rows = []
    values = set()
    if "heating" in sales.columns:
        values.update(sales["heating"].dropna().astype(str).unique().tolist())
    if "heating" in rentals.columns:
        values.update(rentals["heating"].dropna().astype(str).unique().tolist())

    city_median = float(pd.to_numeric(sales.get("unit_price_gross"), errors="coerce").median()) if len(sales) else float("nan")
    for value in sorted(values):
        s_mask = sales["heating"].astype(str) == value if "heating" in sales.columns else pd.Series(False, index=sales.index)
        r_mask = rentals["heating"].astype(str) == value if "heating" in rentals.columns else pd.Series(False, index=rentals.index)
        s_n = int(s_mask.sum())
        r_n = int(r_mask.sum())
        cc = sales.loc[s_mask, "county"].astype(str).nunique() if s_n and "county" in sales.columns else 0
        med = float(pd.to_numeric(sales.loc[s_mask, "unit_price_gross"], errors="coerce").median()) if s_n else float("nan")
        lift = (med / city_median - 1.0) if med == med and city_median == city_median and city_median else float("nan")
        candidate = bool((s_n + r_n) >= 50 and cc >= 2)
        rows.append(
            {
                "heating": value,
                "sale_count": s_n,
                ...
                "lift_vs_city_median": lift,
                "encoded_feature_present": _encoded_present(value),
                "candidate_for_model": candidate,
            }
        )
    return pd.DataFrame(rows).sort_values("sale_count", ascending=False) if rows else pd.DataFrame(
        columns=["heating", "sale_count", "rental_count", "county_count", "median_sale_unit_price", "lift_vs_city_median", "encoded_feature_present", "candidate_for_model"]
    )


def main() -> None:
    args = parse_args()
    global DEFAULT_SOURCE_SITE, DEFAULT_COUNTIES
    DEFAULT_SOURCE_SITE = normalize_source_site(DEFAULT_SOURCE_SITE or os.getenv("SOURCE_SITE"))
    policy_info = resolve_counties_for_policy(getattr(args, "county_policy", "core_good_ok"), getattr(args, "counties", None))
    if str(getattr(args, "county_policy", "core_good_ok")) != "explicit":
        args.counties = ",".join(policy_info["included_counties"])
        DEFAULT_COUNTIES = list(policy_info["included_counties"])
    print(
        f"V25 county_policy={policy_info['county_policy']} included={len(policy_info['included_counties'])} "
        f"excluded={policy_info['excluded_counties']} source_site={DEFAULT_SOURCE_SITE}"
    )
    ...
    cfg = RunConfig(
        ...
        site_extraction_mode=str(getattr(args, "site_extraction_mode", "full")),
        site_project_encoding=str(getattr(args, "site_project_encoding", "foldsafe_target")),
        run_site_ablation=bool(getattr(args, "run_site_ablation", False)),
        site_repair_only=bool(getattr(args, "site_repair_only", False)),
        duplex_feature_mode=str(getattr(args, "duplex_feature_mode", "full")),
        run_duplex_ablation=bool(getattr(args, "run_duplex_ablation", False)),
        merge_gap_warning_tl=float(getattr(args, "merge_gap_warning_tl", 8000.0)),
        model_scope=str(getattr(args, "model_scope", "ankara_only")),
        geo_context_mode=str(getattr(args, "geo_context_mode", "geo_with_coast")),
        location_scope=str(getattr(args, "location_scope", "global")),
    ...
    (out_dirs["base"] / "run_config_v24_kocaeli.json").write_text(json.dumps(asdict(cfg), indent=2, ensure_ascii=False), encoding="utf-8")
    ...
    # --- V26A county policy + filter + coordinate quality + heating audits (aggregate only) ---
    write_county_policy_report(
        out_dirs["reports"] / "county_policy_v26a.json",
        policy_info=policy_info,
        sales_before=sales_raw,
        rentals_before=rentals_raw,
        sales_after=sales_clean,
        rentals_after=rentals_clean,
    )
    after_counties = set(sales_clean["county"].astype(str)) | set(rentals_clean["county"].astype(str))
    unexpected = sorted(after_counties - set(policy_info["included_counties"]))
    if unexpected:
        raise RuntimeError(f"V26A county_policy silent-drop guard failed; unexpected counties={unexpected}")
    for weak in policy_info["excluded_counties"]:
        if weak in after_counties:
            raise RuntimeError(f"V26A excluded WEAK county leaked into training frame: {weak}")
    row_filter_payload = {
        "model_version": V25_MODEL_VERSION,
        "model_scope": V25_MODEL_SCOPE,
        "county_policy": policy_info["county_policy"],
        "clean_report": cleaning_report,
        "sale_removal_reason_flags_on_raw": detailed_filter_reasons(sales_raw, "sale", cfg),
        "rental_removal_reason_flags_on_raw": detailed_filter_reasons(rentals_raw, "rental", cfg),
        "notes": [
            "basic filters mirror V24.1/V25 (gross_m2 typically 25-500 in apply_basic_filters).",
            "reason flags are not mutually exclusive.",
        ],
    }
    (out_dirs["reports"] / "row_count_and_filtering_v26a.json").write_text(
        json.dumps(row_filter_payload, indent=2, ensure_ascii=False, default=str),
        encoding="utf-8",
    )

    # --- Preflight: coordinate quality audit (blocks full training on FAIL) ---
    coord_audit = build_coordinate_quality_audit(sales_clean, rentals_clean)
    (out_dirs["reports"] / "coordinate_quality_audit_v26a.json").write_text(
        json.dumps(coord_audit, indent=2, ensure_ascii=False, default=str),
        encoding="utf-8",
    )
    _plog(
        f"V26A coordinate quality: status={coord_audit.get('coordinateQualityStatus')} "
        f"coverage={coord_audit.get('coordinate_coverage')} "
        f"dup_rate={coord_audit.get('duplicate_coordinate_rate')} "
        f"out_of_bounds_rate={coord_audit.get('out_of_bounds_rate')}"
    )
    if str(coord_audit.get("coordinateQualityStatus")) == "FAIL":
        raise RuntimeError(
            f"V26A coordinate_quality_audit FAILED ({coord_audit.get('reasons')}); "
            "refusing to train. Inspect coordinate_quality_audit_v26a.json before re-running."
        )

    heat_audit = heating_yerden_audit(sales_clean, rentals_clean)
    heat_audit.to_csv(out_dirs["reports"] / "heating_feature_audit_v25.csv", index=False, encoding="utf-8-sig")
    heat_audit_v26a = build_heating_feature_audit_v26a(sales_clean, rentals_clean)
    heat_audit_v26a.to_csv(out_dirs["reports"] / "heating_feature_audit_v26a.csv", index=False, encoding="utf-8-sig")
    yerden_raw_present = bool((heat_audit_v26a["heating"].astype(str).str.lower().str.contains("yerden")).any()) if not heat_audit_v26a.empty else False
    _plog(f"V26A Yerden Isıtma present in raw heating values: {yerden_raw_present}")
    _plog(
        "V25 heating Yerden rows (sale __ALL__) = "
        + str(
            int(heat_audit[(heat_audit["purpose"] == "sale") & (heat_audit["county"] == "__ALL__")]["yerden_count"].iloc[0])
            if not heat_audit.empty
            and ((heat_audit["purpose"] == "sale") & (heat_audit["county"] == "__ALL__")).any()
            else 0
        )
    )
    ...
    # V23 duplex extraction audits (raw text stays in reports only)
    try:
        duplex_cov = write_duplex_extraction_reports(sales_final_base, out_dirs["reports"])
        # also audit rentals for coverage diagnostics
        if len(rentals_clean):
            rent_audit = extract_duplex_frame(rentals_clean)
            rent_cov = duplex_source_coverage_metrics(rent_audit)
            pd.DataFrame([{"split": "sale", **duplex_cov}, {"split": "rental", **rent_cov}]).to_csv(
                out_dirs["reports"] / "duplex_source_coverage_v23.csv",
                index=False,
                encoding="utf-8-sig",
            )
        # inventory drift vs old V21 reference counts (from user/checkpoint context)
        gross = pd.to_numeric(sales_final_base.get("gross_m2"), errors="coerce") if "gross_m2" in sales_final_base.columns else pd.Series(dtype=float)
        sale_lh = int((gross >= LARGE_HOME_M2).fillna(False).sum()) if len(gross) else 0
        drift = pd.DataFrame(
            [
                {
                    "metric": "sale_raw",
                    "v21_reference": None,
                    "v23_current": int(len(sales_raw)),
                },
                {
                    "metric": "rental_raw",
                    "v21_reference": None,
                    "v23_current": int(len(rentals_raw)),
                },
                {
                    "metric": "sale_after_basic_filter",
                    "v21_reference": None,
                    "v23_current": int(len(sales_clean)),
                },
                {
                    "metric": "rental_after_basic_filter",
                    "v21_reference": None,
                    "v23_current": int(len(rentals_clean)),
                },
                {
                    "metric": "sale_training_rows",
                    "v21_reference": None,
                    "v23_current": int(len(sales_final_base)),
                },
                {
                    "metric": "large_home_count_ge180",
                    "v21_reference": None,
                    "v23_current": sale_lh,
                },
                {
                    "metric": "large_home_share_ge180",
                    "v21_reference": None,
                    "v23_current": float(sale_lh / len(sales_final_base)) if len(sales_final_base) else None,
                },
                {
                    "metric": "duplex_count_sale",
                    "v21_reference": None,
                    "v23_current": int(duplex_cov.get("detail_duplex_count", 0) + duplex_cov.get("title_only_duplex_count", 0)),
                },
            ]
        )
        # county distribution
        if "county" in sales_final_base.columns:
            cd = (
                sales_final_base.groupby("county", dropna=False)
                .size()
                .rename("sale_count")
                .reset_index()
            )
            if len(rentals_clean) and "county" in rentals_clean.columns:
                rd = rentals_clean.groupby("county", dropna=False).size().rename("rental_count").reset_index()
                cd = cd.merge(rd, on="county", how="outer")
            cd.to_csv(out_dirs["reports"] / "county_distribution_v24.csv", index=False, encoding="utf-8-sig")
        drift.to_csv(out_dirs["reports"] / "inventory_drift_v24_kocaeli.csv", index=False, encoding="utf-8-sig")
        audit = extract_duplex_frame(sales_final_base)
        cov_by_type = (
            audit.groupby("duplex_type", dropna=False)
            .size()
            .rename("count")
            .reset_index()
        )
        cov_by_type["share"] = cov_by_type["count"] / max(len(audit), 1)
        cov_by_type.to_csv(out_dirs["reports"] / "duplex_coverage_v24.csv", index=False, encoding="utf-8-sig")
        # price distributions
        if "unit_price_gross" in sales_final_base.columns or True:
            up = pd.to_numeric(sales_final_base.get("unit_price_gross", sales_final_base.get(TARGET, np.nan)), errors="coerce")
            tmp = audit.copy()
            tmp["unit_price_gross"] = up.values
            tmp["gross_m2"] = gross.values
            tmp["is_large_home"] = (gross >= LARGE_HOME_M2).fillna(False).astype(int).values
            price_dist = (
                tmp.groupby(["duplex_type", "is_large_home"], dropna=False)["unit_price_gross"]
                .agg(n="count", median="median", mean="mean", p10=lambda s: s.quantile(0.1), p90=lambda s: s.quantile(0.9))
                .reset_index()
            )
            price_dist.to_csv(out_dirs["reports"] / "duplex_price_distribution_v23.csv", index=False, encoding="utf-8-sig")
        if len(rentals_clean):
            ra = extract_duplex_frame(rentals_clean)
            rent_m2 = pd.to_numeric(rentals_clean.get("rent_per_m2_gross", rentals_clean.get("rent_m2", np.nan)), errors="coerce")
            ra = ra.copy()
            ra["rent_m2"] = rent_m2.values if len(rent_m2) == len(ra) else np.nan
            rdist = (
                ra.groupby("duplex_type", dropna=False)["rent_m2"]
                .agg(n="count", median="median", mean="mean")
                .reset_index()
            )
            rdist.to_csv(out_dirs["reports"] / "duplex_rental_distribution_v23.csv", index=False, encoding="utf-8-sig")
        _plog(f"V23 duplex coverage: {duplex_cov}")
    except Exception as duplex_diag_exc:
        warnings.warn(f"V23 duplex extraction diagnostics skipped: {duplex_diag_exc}")
    ...
    def run_one_mode(
        ...
        site_extraction_mode: str | None = None,
        site_project_encoding: str | None = None,
        duplex_feature_mode: str | None = None,
        ablation_experiment_name: str | None = None,
    ):
        ...
        site_mode = str(site_extraction_mode if site_extraction_mode is not None else getattr(cfg, "site_extraction_mode", "full"))
        site_enc = str(site_project_encoding if site_project_encoding is not None else getattr(cfg, "site_project_encoding", "foldsafe_target"))
        duplex_mode = str(duplex_feature_mode if duplex_feature_mode is not None else getattr(cfg, "duplex_feature_mode", "full"))
        exp_name = str(
            ablation_experiment_name
            if ablation_experiment_name is not None
            else getattr(cfg, "ablation_experiment_name", "")
        )
        mode_cfg = RunConfig(
            **{
                **asdict(cfg),
                ...
                "site_project_encoding": site_enc,
                "duplex_feature_mode": duplex_mode,
                "ablation_experiment_name": exp_name,
            }
        )
        ...
        metrics_summary["ensemble"]["site_extraction_mode"] = str(getattr(mode_cfg, "site_extraction_mode", "full"))
        metrics_summary["ensemble"]["site_project_encoding"] = str(getattr(mode_cfg, "site_project_encoding", "frequency"))
        metrics_summary["ensemble"]["duplex_feature_mode"] = str(getattr(mode_cfg, "duplex_feature_mode", "none"))
        metrics_summary["selected_site_extraction_mode"] = str(getattr(mode_cfg, "site_extraction_mode", "full"))
        metrics_summary["selected_site_project_encoding"] = str(getattr(mode_cfg, "site_project_encoding", "frequency"))
        metrics_summary["selected_duplex_feature_mode"] = str(getattr(mode_cfg, "duplex_feature_mode", "none"))
        metrics_summary["ensemble"]["model_scope"] = str(getattr(mode_cfg, "model_scope", "kocaeli_global"))
    ...
    selected_location_scope = str(getattr(args, "location_scope", "global"))
    selected_comparable_mode = "none"
    selected_site_mode = str(getattr(cfg, "site_extraction_mode", "full"))
    selected_site_encoding = str(getattr(cfg, "site_project_encoding", "frequency"))
    selected_duplex_mode = str(getattr(cfg, "duplex_feature_mode", "full"))
    location_ablation_rows: list[dict[str, Any]] = []
    comparable_ablation_rows: list[dict[str, Any]] = []
    duplex_ablation_rows: list[dict[str, Any]] = []
    ...
    if bool(getattr(args, "run_site_ablation", False)):
        n_premium_runs = 4 if bool(getattr(args, "site_repair_only", False)) else 6
    else:
        n_premium_runs = 0
    n_duplex_runs = 5 if bool(getattr(args, "run_duplex_ablation", False)) else 0
    # V25/V26A ablation trains N arms then returns early (no separate FINAL pass).
    n_v25_runs = 0
    if bool(getattr(args, "run_v25_ablation", False)):
        n_v25_runs = 2
        if bool(getattr(args, "v25_site_encoding_check", False)):
            n_v25_runs = 3
    if bool(getattr(args, "run_v26a_ablation", False)):
        n_v25_runs = 3  # control_basic_geo_ankara + full_v26a_ankara + no_site_project_ablation
    include_final = not (bool(getattr(args, "run_v25_ablation", False)) or bool(getattr(args, "run_v26a_ablation", False)))
    total_units = estimate_training_units(
        ...
        n_segments=4,
        n_counties_est=4,
        n_location_runs=n_location_runs
        + n_comparable_runs
        + n_premium_runs
        + n_duplex_runs
        + n_v25_runs,
    ...
    _plog(
        ...
        f"v25_ablation_arms={n_v25_runs}, location ablation={n_location_runs}, "
        f"comparable ablation={n_comparable_runs})."
    )
    ...
    try:
        ...
        # V21 site-extraction ablation (disabled in V24 — replaced by Kocaeli matrix below)
        site_ablation_rows: list[dict[str, Any]] = []
        if False and bool(getattr(args, "run_site_ablation", False)):
            V20_R2 = 0.5017
            V20_MAPE = 0.1060
            V20_VR = 0.4616
            site_experiments = [
                ("control_v20_site_foldsafe", "v20_parity", "foldsafe_target"),
                ("alias_frequency", "alias", "frequency"),
                ("dict_frequency", "dict", "frequency"),
                ("dict_foldsafe", "dict", "foldsafe_target"),
                ("tier_foldsafe", "tier", "foldsafe_target"),
                ("interactions_foldsafe", "interactions", "foldsafe_target"),
                ("full_v21", "full", "foldsafe_target"),
            ]
            for exp_name, site_m, enc_m in site_experiments:
                _pstage(f"Site ablation: {exp_name}")
                _plog(
                    f"\n========== V21 SITE ABLATION: {exp_name} "
                    f"extraction={site_m} site_enc={enc_m} =========="
                )
                mode_dirs = build_output_dirs(out_dirs["base"] / f"ablation_site_{exp_name}")
                _b, ms, _d, _f = run_one_mode(
                    selected_demo,
                    selected_attr,
                    mode_dirs,
                    ...
                    location_scope=selected_location_scope,
                    comparable_mode="none",
                    site_extraction_mode=site_m,
                    site_project_encoding=enc_m,
                )
                ens = ms.get("ensemble", {})
                r2 = float(ms.get("r2") if ms.get("r2") is not None else ens.get("r2") or float("nan"))
                mape = float(ms.get("mape") if ms.get("mape") is not None else ens.get("mape") or float("nan"))
                ...
                cov = ms.get("site_coverage") or {}
                severe = int(ms.get("merge_audit_severe_warnings") or 0)
                notes = []
                if enc_m == "foldsafe_target" and leak and leak.get("pass") is False:
                    notes.append("leakage_guard_fail")
                if np.isfinite(r2) and r2 <= V20_R2 and exp_name != "control_v20_site_foldsafe":
                    notes.append(f"no_r2_lift_vs_v20 ({r2:.4f} <= {V20_R2:.4f})")
                if np.isfinite(mape) and mape > V20_MAPE + 0.005:
                    notes.append(f"mape_guardrail_fail ({mape:.4f} > {V20_MAPE + 0.005:.4f})")
                if severe > 0:
                    notes.append(f"severe_possible_bad_merge={severe}")
                site_ablation_rows.append(
                    {
                        "experiment": exp_name,
                        "site_extraction_mode": site_m,
                        "site_project_encoding": enc_m,
                        ...
                        "expensive_decile_bias": exp_bias,
                        "cheap_decile_bias": cheap_bias,
                        "extracted_raw_rate": cov.get("extracted_raw_rate"),
                        "alias_hit_rate": cov.get("alias_hit_rate"),
                        "dict_hit_rate": cov.get("dict_hit_rate"),
                        "canonical_non_missing_rate": cov.get("canonical_non_missing_rate"),
                        "foldsafe_encoded_site_count": cov.get("foldsafe_encoded_site_count"),
                        "top_expensive_underpredicted_site_missing_rate": cov.get(
                            "top_expensive_underpredicted_site_missing_rate"
                        ),
                        "merge_audit_severe_warnings": severe,
                        "selected": False,
                        "notes": "; ".join(notes),
                    }
                )

            def _site_eligible(row: dict[str, Any]) -> bool:
                name = str(row.get("experiment") or "")
                if name == "control_v20_site_foldsafe":
                    return False
                notes = str(row.get("notes") or "")
                if "leakage_guard_fail" in notes:
                    return False
                if "severe_possible_bad_merge" in notes:
                    return False
                try:
                    # coverage up + R2 down => reject
                    r2 = float(row.get("r2") or -9)
                    if r2 <= V20_R2:
                        return False
                    return float(row.get("mape") or 9) <= V20_MAPE + 0.005 + 1e-12
                except Exception:
                    return False

            candidates = [r for r in site_ablation_rows if _site_eligible(r)]
            if candidates:
                pick = max(
                    candidates,
                    key=lambda r: (
                        ...
                        float(r.get("variance_ratio") or -9),
                        float(r.get("canonical_non_missing_rate") or -9),
                        float(r.get("large_home_r2") or -9),
                        float(r.get("r2") or -9),
                    ),
                )["experiment"]
            else:
                pick = "control_v20_site_foldsafe"

            for r in site_ablation_rows:
                r["selected"] = r.get("experiment") == pick
                if r["selected"] and not r.get("notes"):
                    r["notes"] = "selected vs V20 (R2 lift + MAPE + no severe bad merge)"
                if r.get("selected") and "severe_possible_bad_merge" in str(r.get("notes") or ""):
                    r["selected"] = False
                    r["notes"] = (r.get("notes") or "") + "; blocked_until_manual_review"
                    pick = "control_v20_site_foldsafe"
            # re-mark control if blocked
            for r in site_ablation_rows:
                r["selected"] = r.get("experiment") == pick
            pd.DataFrame(site_ablation_rows).to_csv(
                out_dirs["reports"] / "metrics_site_ablation_v21_basiskele.csv",
                index=False,
                encoding="utf-8-sig",
            )
            by_exp = {r["experiment"]: r for r in site_ablation_rows}
            picked = by_exp.get(pick) or {}
            selected_site_mode = str(picked.get("site_extraction_mode") or selected_site_mode)
            selected_site_encoding = str(picked.get("site_project_encoding") or selected_site_encoding)
            _plog(
                f"Selected V21 site after ablation: {pick} "
                f"(extraction={selected_site_mode}, site_enc={selected_site_encoding}) "
                f"[best_checkpoint not auto-updated]"
            )
        ...
        # V26A Ankara-only ablation: control_basic_geo_ankara + full_v26a_ankara
        # (+ optional no_site_project_ablation). See README_v26a_ankara_only_run.md.
        v25_ablation_rows: list[dict[str, Any]] = []
        if bool(getattr(args, "run_v25_ablation", False)) or bool(getattr(args, "run_v26a_ablation", False)):
            cli_site_mode = str(getattr(args, "site_extraction_mode", "full") or "full")
            cli_site_enc = str(getattr(args, "site_project_encoding", "foldsafe_target") or "foldsafe_target")
            cli_dup_mode = str(getattr(args, "duplex_feature_mode", "full") or "full")
            v25_experiments = [
                ("control_basic_geo_ankara", "none", "none", "none"),
                ("full_v26a_ankara", cli_site_mode, cli_site_enc, cli_dup_mode),
                ("no_site_project_ablation", "none", "none", cli_dup_mode),
            ]
            for exp_name, site_m, enc_m, dup_m in v25_experiments:
                mode_dirs = build_output_dirs(out_dirs["base"] / f"ablation_v26a_{exp_name}")
                cached_ms_path = mode_dirs["reports"] / "metrics_summary_v25_core_good_ok.json"
                ms = None
                if cached_ms_path.exists():
                    try:
                        ms = json.loads(cached_ms_path.read_text(encoding="utf-8"))
                        if ms.get("r2") is None and not (ms.get("ensemble") or {}).get("r2"):
                            ms = None
                    except Exception:
                        ms = None
                if ms is not None:
                    _plog(f"\n========== V26A RESUME SKIP (cached): {exp_name} ==========")
                else:
                    _pstage(f"V26A ablation: {exp_name}")
                    # control_basic_geo_ankara: county/district categorical + core physical + lat/lon
                    # geo only — no geo-context (POI/coast), no site encoding, no duplex features.
                    exp_location_mode = "geo" if exp_name != "control_basic_geo_ankara" else "geo"
                    exp_geo_context_mode = selected_geo_context_mode if exp_name != "control_basic_geo_ankara" else "none"
                    _plog(
                        f"\n========== V26A ANKARA-ONLY ABLATION: {exp_name} "
                        f"site={site_m} enc={enc_m} duplex={dup_m} geo_context={exp_geo_context_mode} =========="
                    )
                    _b, ms, _d, _f = run_one_mode(
                        selected_demo,
                        selected_attr,
                        ...
                        spread_layer="none",
                        karamursel_baseline="none",
                        location_mode=exp_location_mode,
                        geo_context_mode=exp_geo_context_mode,
                        location_scope="global",
                        comparable_mode="none",
                        site_extraction_mode=site_m,
                        site_project_encoding=enc_m,
                        duplex_feature_mode=dup_m,
                        ablation_experiment_name=exp_name,
                    )
                    try:
                        payload_ms = dict(ms) if isinstance(ms, dict) else {}
                        payload_ms.update(
                            {
                                "model_version": V25_MODEL_VERSION,
                                "model_scope": V25_MODEL_SCOPE,
                                "city": args.city,
                                "county_policy": policy_info["county_policy"],
                                "included_counties": policy_info["included_counties"],
                                "excluded_counties": policy_info["excluded_counties"],
                                "experiment": exp_name,
                                "kocaeli_v25_reference": KOCAELI_V25_REFERENCE,
                            }
                        )
                        (mode_dirs["reports"] / "metrics_summary_v25_core_good_ok.json").write_text(
                            json.dumps(payload_ms, indent=2, ensure_ascii=False, default=str),
                            encoding="utf-8",
                        )
                    except Exception:
                        pass
                    try:
                        fi = mode_dirs["reports"] / "feature_importance_v18_basiskele.csv"
                        enc_names = []
                        _fidf = None
                        if fi.exists():
                            _fidf = pd.read_csv(fi)
                            _col = "feature" if "feature" in _fidf.columns else _fidf.columns[0]
                            enc_names = _fidf[_col].astype(str).tolist()
                        heat_all = heating_yerden_audit(sales_clean, rentals_clean)
                        yerden_n = 0
                        if not heat_all.empty:
                            sub = heat_all[(heat_all["purpose"] == "sale") & (heat_all["county"] == "__ALL__")]
                            if len(sub):
                                yerden_n = int(sub["yerden_count"].iloc[0])
                        diff = build_feature_schema_diff(
                            v25_schema_cols=list(sales_clean.columns),
                            v25_encoded_names=enc_names,
                            v24_importance_csv=None,
                            heating_raw_yerden_count=yerden_n,
                            fast_mode=bool(getattr(args, "fast", False)),
                            duplex_feature_mode=str(dup_m),
                            site_project_encoding=str(enc_m),
                        )
                        (mode_dirs["reports"] / "feature_schema_guard_v26a.json").write_text(
                            json.dumps(diff, indent=2, ensure_ascii=False), encoding="utf-8"
                        )
                        if exp_name == "full_v26a_ankara":
                            (out_dirs["reports"] / "feature_schema_guard_v26a.json").write_text(
                                json.dumps(diff, indent=2, ensure_ascii=False), encoding="utf-8"
                            )
                            heat_v26a = build_heating_feature_audit_v26a(sales_clean, rentals_clean, enc_names)
                            heat_v26a.to_csv(out_dirs["reports"] / "heating_feature_audit_v26a.csv", index=False, encoding="utf-8-sig")
                            yerden_encoded = bool(heat_v26a.loc[heat_v26a["heating"].astype(str).str.lower().str.contains("yerden"), "encoded_feature_present"].any()) if not heat_v26a.empty else False
                            _plog(f"V26A Yerden Isıtma survives into encoded features: {yerden_encoded}")
                            if _fidf is not None:
                                _fidf.to_csv(out_dirs["reports"] / "feature_importance_v26a.csv", index=False, encoding="utf-8-sig")
                            # Site/project coverage, merge audit, alias review, app-safe options catalog.
                            if str(site_m) not in {"", "none"}:
                                try:
                                    opt_adder = SiteProjectExtractionAdder(
                                        site_extraction_mode=str(site_m),
                                        site_project_encoding=str(enc_m),
                                        min_site_freq=1,
                                        merge_gap_warning_tl=float(getattr(cfg, "merge_gap_warning_tl", 8000.0)),
                                    )
                                    opt_frame = opt_adder.fit_transform(sales_final_base)
                                    coverage_by_county(opt_frame).to_csv(
                                        out_dirs["reports"] / "site_project_coverage_v26a_by_county.csv",
                                        index=False,
                                        encoding="utf-8-sig",
                                    )
                                    merge_df = pd.DataFrame(getattr(opt_adder, "merge_rows_", None) or [])
                                    merge_df.to_csv(out_dirs["reports"] / "site_project_merge_audit_v26a.csv", index=False, encoding="utf-8-sig")
                                    severe_n = int(count_severe_bad_merges(merge_df)) if not merge_df.empty else 0
                                    severe_rows_df = severe_merge_rows(merge_df) if not merge_df.empty else pd.DataFrame()
                                    _plog(f"V26A site merge audit: severe_groups={severe_n} severe_rows={len(severe_rows_df)} total_flagged_rows={len(merge_df)}")
                                    review_cols = [c for c in ("scoped_id", "county", "canonical_id", "n_listings") if c in merge_df.columns]
                                    if review_cols:
                                        merge_df[review_cols].drop_duplicates().to_csv(
                                            out_dirs["reports"] / "site_project_alias_review_needed_v26a.csv",
                                            index=False,
                                            encoding="utf-8-sig",
                                        )
                                    else:
                                        pd.DataFrame(columns=["scoped_id", "county", "canonical_id", "n_listings"]).to_csv(
                                            out_dirs["reports"] / "site_project_alias_review_needed_v26a.csv", index=False, encoding="utf-8-sig"
                                        )
                                    if severe_n == 0:
                                        write_site_project_options(opt_frame, out_dirs["artifacts"])
                                    else:
                                        warnings.warn(f"V26A site arm has {severe_n} severe merge warnings; skipping app-safe option export.")
                                except Exception as _site_exc:
                                    warnings.warn(f"V26A site export post-arm warning: {_site_exc}")
                        if str(diff.get("status")) == "FAIL" and exp_name != "control_basic_geo_ankara":
                            raise RuntimeError(
                                f"V26A critical feature guard FAIL for {exp_name}: {diff.get('critical_missing_features')}"
                            )
                        heat_all.to_csv(mode_dirs["reports"] / "heating_feature_audit_v25.csv", index=False, encoding="utf-8-sig")
                    except RuntimeError:
                        raise
                    except Exception as _guard_exc:
                        warnings.warn(f"V26A feature guard post-arm warning: {_guard_exc}")

                ens = ms.get("ensemble", {}) if isinstance(ms, dict) else {}
                r2 = float(ms.get("r2") if ms.get("r2") is not None else ens.get("r2") or float("nan"))
                mape = float(ms.get("mape") if ms.get("mape") is not None else ens.get("mape") or float("nan"))
                vr = float(ms.get("variance_ratio") if ms.get("variance_ratio") is not None else ens.get("basiskele_variance_ratio") or float("nan"))
                leak = ms.get("site_project_encoding_leakage_guard") or ens.get("site_project_encoding_leakage_guard") or {}
                severe = int(ms.get("merge_audit_severe_warnings") or 0)
                county_metrics = ms.get("county_metrics") or []
                county_r2 = {str(c.get("county")): c for c in county_metrics if isinstance(c, dict)}
                stale = assert_no_stale_basiskele_metadata(ms if isinstance(ms, dict) else {})
                row = {
                    "experiment": exp_name,
                    "site_extraction_mode": site_m,
                    "site_project_encoding": enc_m,
                    ...
                    "mape": mape,
                    "variance_ratio": vr,
                    "large_home_r2": ms.get("large_home_r2"),
                    "expensive_decile_bias": ms.get("expensive_decile_bias"),
                    "merge_audit_severe_warnings": severe,
                    "leakage_pass": bool(leak.get("pass", True)) if isinstance(leak, dict) else True,
                    "stale_metadata_warnings": "|".join(stale),
                    "county_metrics_n": len(county_metrics),
                    "selected": False,
                    "notes": "",
                }
                for k in policy_info["included_counties"]:
                    row[f"r2__{k}"] = (county_r2.get(k) or {}).get("r2")
                v25_ablation_rows.append(row)

            by_exp = {r["experiment"]: r for r in v25_ablation_rows}
            control = by_exp.get("control_basic_geo_ankara") or {}
            full = by_exp.get("full_v26a_ankara") or {}
            no_site = by_exp.get("no_site_project_ablation") or {}
            ctrl_r2 = float(control.get("r2") or float("nan"))
            ctrl_mape = float(control.get("mape") or float("nan"))
            pick = "control_basic_geo_ankara"
            best = False
            notes = []

            def _gate(cand: dict[str, Any], cand_name: str) -> tuple[bool, list[str]]:
                ok = True
                reasons: list[str] = []
                if not cand:
                    return False, ["arm_missing"]
                if not bool(cand.get("leakage_pass", True)):
                    ok = False
                    reasons.append("leakage_fail")
                if cand_name != "no_site_project_ablation" and int(cand.get("merge_audit_severe_warnings") or 0) > 0:
                    ok = False
                    reasons.append("severe_merge")
                cr2 = float(cand.get("r2") or float("nan"))
                cmape = float(cand.get("mape") or float("nan"))
                if np.isfinite(ctrl_r2) and np.isfinite(cr2) and cr2 + 1e-12 < ctrl_r2:
                    ok = False
                    reasons.append("r2_below_control")
                if np.isfinite(ctrl_mape) and np.isfinite(cmape) and cmape > ctrl_mape + 0.005 + 1e-12:
                    ok = False
                    reasons.append("mape_worse_than_control_plus_0.005")
                weak_counties = [
                    k for k in policy_info["included_counties"]
                    if isinstance(cand.get(f"r2__{k}"), (int, float)) and cand.get(f"r2__{k}") is not None and cand[f"r2__{k}"] < -0.5
                ]
                if len(weak_counties) > len(policy_info["included_counties"]) // 2:
                    ok = False
                    reasons.append(f"catastrophically_weak_counties={weak_counties}")
                return ok, reasons

            schema_status = "PASS"
            schema_path = out_dirs["reports"] / "feature_schema_guard_v26a.json"
            if schema_path.exists():
                try:
                    schema_status = str(json.loads(schema_path.read_text(encoding="utf-8")).get("status"))
                except Exception:
                    pass

            full_ok, full_reasons = _gate(full, "full_v26a_ankara")
            if schema_status == "FAIL":
                full_ok = False
                full_reasons.append("feature_guard_fail")
            if full_ok:
                pick = "full_v26a_ankara"
                best = True
                notes = full_reasons + ["full_v26a_ankara passes all gates vs control_basic_geo_ankara"]
            else:
                notes = [f"full_v26a_ankara rejected: {full_reasons}"]
                no_site_ok, no_site_reasons = _gate(no_site, "no_site_project_ablation")
                if no_site_ok:
                    pick = "no_site_project_ablation"
                    best = True
                    notes.append(f"fell back to no_site_project_ablation: passes gates ({no_site_reasons or 'ok'})")
                else:
                    notes.append(f"no_site_project_ablation also rejected: {no_site_reasons}; kept control_basic_geo_ankara")

            for r in v25_ablation_rows:
                r["selected"] = r.get("experiment") == pick
                if r["selected"]:
                    r["notes"] = "; ".join(notes)
            pd.DataFrame(v25_ablation_rows).to_csv(
                out_dirs["reports"] / "metrics_v26a_ankara_ablation.csv",
                index=False,
                encoding="utf-8-sig",
            )
            pd.DataFrame(
                [
                    {
                        ...
                        "expensive_decile_bias": r.get("expensive_decile_bias"),
                        "r2": r.get("r2"),
                        "mape": r.get("mape"),
            ...
            ).to_csv(out_dirs["reports"] / "expensive_decile_bias_comparison_v26a.csv", index=False, encoding="utf-8-sig")
            selection_payload = {
                "status": "selected_checkpoint_v26a_ankara_only_core_good_ok" if best else "diagnostic_v26a_ankara_only_core_good_ok",
                "selected_experiment": pick,
                "best_checkpoint": bool(best),
                "promote_to_production": False,
                "multi_city_v27_candidate": False,
                "model_version": V25_MODEL_VERSION,
                "model_scope": V25_MODEL_SCOPE,
                "city": args.city,
                "county_policy": policy_info["county_policy"],
                ...
                "control": control,
                "full_v26a": full,
                "no_site_project_ablation": no_site,
                "kocaeli_v25_reference": KOCAELI_V25_REFERENCE,
                "note": "Research checkpoint only. App/EDER/Kocaeli V25 artifacts not touched in this task.",
            }
            (out_dirs["reports"] / "selection_decision_v26a.json").write_text(
                json.dumps(selection_payload, indent=2, ensure_ascii=False, default=str),
                encoding="utf-8",
            )
            (out_dirs["reports"] / "metrics_summary_v26a_ankara.json").write_text(
                json.dumps(selection_payload, indent=2, ensure_ascii=False, default=str),
                encoding="utf-8",
            )
            # README
            readme_lines = [
                "# V26A Ankara-Only Baseline — core_good_ok",
                "",
                "Research checkpoint only. No multi-city training. No app/backend/production changes.",
                "",
                f"- model_version: `{V25_MODEL_VERSION}`",
                f"- model_scope: `{V25_MODEL_SCOPE}`",
                f"- city: `{args.city}`",
                f"- county_policy: `{policy_info['county_policy']}`",
                f"- included: {', '.join(policy_info['included_counties'])}",
                f"- excluded WEAK: {', '.join(policy_info['excluded_counties'])}",
                f"- selected_experiment: `{pick}`",
                f"- best_checkpoint: `{best}`",
                f"- promote_to_production: `false`",
                "",
                "## Arms",
                f"- control_basic_geo_ankara: R2={control.get('r2')} MAPE={control.get('mape')}",
                f"- full_v26a_ankara: R2={full.get('r2')} MAPE={full.get('mape')}",
                f"- no_site_project_ablation: R2={no_site.get('r2')} MAPE={no_site.get('mape')}",
                "",
                "## Cross-city reference (informational only, different data — not a selection gate)",
                f"- Kocaeli V25 core_good_ok: R2={KOCAELI_V25_REFERENCE['r2']} MAPE={KOCAELI_V25_REFERENCE['mape']} rows={KOCAELI_V25_REFERENCE['rows']}",
                "",
                "Do not update EDER/app artifacts or Kocaeli V25 outputs from this run.",
                "",
            ]
            (out_dirs["base"] / "README_v26a_ankara_only_run.md").write_text("\n".join(readme_lines), encoding="utf-8")
            # Do not also run the FINAL single arm when V26A ablation already produced selected arms.
            progress.finish("tamamlandı")
            set_progress(None)
            print(json.dumps(selection_payload, indent=2, ensure_ascii=False))
            print(f"\nOutputs written to: {out_dirs['base'].resolve()}")
            return 0

        # V24.1 Kocaeli site-merge repair ablation
        duplex_ablation_rows = []  # kept for summary compatibility
        site_ablation_rows_v24: list[dict[str, Any]] = []
        severe_audit_frames: list[pd.DataFrame] = []
        if bool(getattr(args, "run_site_ablation", False)) and not (bool(getattr(args, "run_v25_ablation", False)) or bool(getattr(args, "run_v26a_ablation", False))):
            site_repair_only = bool(getattr(args, "site_repair_only", False) or getattr(cfg, "site_repair_only", False))
            if site_repair_only:
                v24_experiments = [
                    ("site_frequency_global", "dict", "frequency", "none"),
                    ("site_foldsafe_global", "dict", "foldsafe_target", "none"),
                    ("site_plus_duplex_global", "dict", "foldsafe_target", "full"),
                    ("full_v24", "full", "foldsafe_target", "full"),
                ]
                # Seed frozen V24 refs into ablation table for selection context
                site_ablation_rows_v24.append(
                    {
                        **V24_CONTROL_REF,
                        "site_extraction_mode": "none",
                        "site_project_encoding": "none",
                        "duplex_feature_mode": "none",
                        "rows": 6667,
                        "large_home_r2": float("nan"),
                        "expensive_decile_bias": float("nan"),
                        "basiskele_mape": float("nan"),
                        "selected": False,
                        "notes": "frozen_v24_control_ref; not retrained in site_repair_only",
                    }
                )
                site_ablation_rows_v24.append(
                    {
                        **V24_SAFE_DUPLEX_REF,
                        ...
                        "basiskele_mape": float("nan"),
                        "selected": False,
                        "notes": "frozen_v24_safe_duplex_ref; not retrained in site_repair_only",
                    }
                )
            else:
                v24_experiments = [
                    ("control_v17_or_v21_like_global", "none", "none", "none"),
                    ("site_frequency_global", "dict", "frequency", "none"),
                    ("site_foldsafe_global", "dict", "foldsafe_target", "none"),
                    ("duplex_largehome_global", "none", "none", "full"),
                    ("site_plus_duplex_global", "dict", "foldsafe_target", "full"),
                    ("full_v24", "full", "foldsafe_target", "full"),
                ]
            for exp_name, site_m, enc_m, dup_m in v24_experiments:
                mode_dirs = build_output_dirs(out_dirs["base"] / f"ablation_v24_{exp_name}")
                cached_ms_path = mode_dirs["reports"] / "metrics_summary_v24_kocaeli.json"
                ms = None
                if cached_ms_path.exists():
                    try:
                        ms = json.loads(cached_ms_path.read_text(encoding="utf-8"))
                        if ms.get("merge_audit_severe_warnings") is None and "r2" not in ms:
                            ms = None
                    except Exception:
                        ms = None
                if ms is not None:
                    _plog(f"\n========== V24.1 RESUME SKIP (cached): {exp_name} ==========")
                else:
                    _pstage(f"V24.1 site ablation: {exp_name}")
                    _plog(
                        f"\n========== V24.1 KOCAELI ABLATION: {exp_name} "
                        f"site={site_m} enc={enc_m} duplex={dup_m} =========="
                    )
                    _b, ms, _d, _f = run_one_mode(
                        selected_demo,
                        selected_attr,
                ...
                severe = int(ms.get("merge_audit_severe_warnings") or 0)
                county_metrics = ms.get("county_metrics") or []
                bas = next((c for c in county_metrics if str(c.get("county")) == "Başiskele"), {})
                izm = next((c for c in county_metrics if str(c.get("county")) == "İzmit"), {})
                gol = next((c for c in county_metrics if str(c.get("county")) in {"Gölcük", "Golcuk"}), {})
                kar = next((c for c in county_metrics if str(c.get("county")) in {"Karamürsel", "Karamursel"}), {})
                karte = next((c for c in county_metrics if str(c.get("county")) == "Kartepe"), {})
                notes = []
                if leak and leak.get("pass") is False:
                    notes.append("leakage_guard_fail")
                if severe > 0:
                    notes.append(f"severe_possible_bad_merge={severe}")
                if cached_ms_path.exists() and ms is not None and "cached_resume" not in notes:
                    notes.append("cached_resume")
                site_ablation_rows_v24.append(
                    {
                        "experiment": exp_name,
                        "site_extraction_mode": site_m,
                        ...
                        "large_home_r2": lh_r2,
                        "expensive_decile_bias": exp_bias,
                        "basiskele_r2": bas.get("r2"),
                        "izmit_r2": izm.get("r2"),
                        "golcuk_r2": gol.get("r2"),
                        "karamursel_r2": kar.get("r2"),
                        "kartepe_r2": karte.get("r2"),
                        "basiskele_mape": bas.get("mape"),
                        "merge_audit_severe_warnings": severe,
                        "leakage_pass": bool((leak or {}).get("pass", True)),
                        "selected": False,
                        "notes": "; ".join(notes),
                    }
                )
                # Arm-specific county coverage + severe merge audit copies
                cov_src = mode_dirs["reports"] / "site_project_coverage_v24_by_county.csv"
                if cov_src.exists() and site_m not in {"", "none"}:
                    shutil.copy2(
                        cov_src,
                        out_dirs["reports"] / f"site_project_coverage_v24_by_county__{exp_name}.csv",
                    )
                sev_src = mode_dirs["reports"] / "site_project_merge_audit_severe_v24.csv"
                if sev_src.exists():
                    try:
                        sdf = pd.read_csv(sev_src)
                        if not sdf.empty:
                            if "experiment" not in sdf.columns or sdf["experiment"].astype(str).eq("").all():
                                sdf["experiment"] = exp_name
                            severe_audit_frames.append(sdf)
                            sdf.to_csv(
                                out_dirs["reports"] / f"site_project_merge_audit_severe_v24__{exp_name}.csv",
                                index=False,
                                encoding="utf-8-sig",
                            )
                    except Exception:
                        pass
                audit_src = mode_dirs["reports"] / "site_project_merge_audit_v24.csv"
                if audit_src.exists() and site_m not in {"", "none"}:
                    shutil.copy2(
                        audit_src,
                        out_dirs["reports"] / f"site_project_merge_audit_v24__{exp_name}.csv",
                    )

            if severe_audit_frames:
                pd.concat(severe_audit_frames, ignore_index=True).to_csv(
                    out_dirs["reports"] / "site_project_merge_audit_severe_v24.csv",
                    index=False,
                    encoding="utf-8-sig",
                )
            ...
            by_exp = {r["experiment"]: r for r in site_ablation_rows_v24}
            duplex_ref = by_exp.get("duplex_largehome_global") or dict(V24_SAFE_DUPLEX_REF)
            dup_r2 = float(duplex_ref.get("r2") if duplex_ref.get("r2") is not None else V24_SAFE_DUPLEX_REF["r2"])
            dup_mape = float(duplex_ref.get("mape") if duplex_ref.get("mape") is not None else V24_SAFE_DUPLEX_REF["mape"])
            site_arm_names = {
                "site_frequency_global",
                "site_foldsafe_global",
                "site_plus_duplex_global",
                "full_v24",
            }

            def _county_ok(row: dict[str, Any]) -> bool:
                """Reject material per-county R2 regression vs safe duplex ref."""
                for key, floor_key in [
                    ("basiskele_r2", "basiskele_r2"),
                    ("izmit_r2", "izmit_r2"),
                    ("golcuk_r2", "golcuk_r2"),
                    ("karamursel_r2", "karamursel_r2"),
                    ("kartepe_r2", "kartepe_r2"),
                ]:
                    ref_v = float(V24_SAFE_DUPLEX_REF.get(floor_key) or float("nan"))
                    cur = float(row.get(key) if row.get(key) is not None else float("nan"))
                    if np.isfinite(ref_v) and np.isfinite(cur) and cur < ref_v - 0.02:
                        return False
                return True

            def _v241_eligible(row: dict[str, Any]) -> bool:
                if str(row.get("experiment")) not in site_arm_names:
                    return False
                if not bool(row.get("leakage_pass", True)):
                    return False
                if int(row.get("merge_audit_severe_warnings") or 0) > 0:
                    return False
                try:
                    r2 = float(row.get("r2") or -9)
                    mape = float(row.get("mape") or 9)
                    if not np.isfinite(dup_r2) or r2 <= dup_r2:
                        return False
                    if np.isfinite(dup_mape) and mape > dup_mape + 0.005 + 1e-12:
                        return False
                    if not _county_ok(row):
                        return False
                    return True
                except Exception:
                    return False

            candidates = [r for r in site_ablation_rows_v24 if _v241_eligible(r)]
            if candidates:
                pick = max(
                    candidates,
                    key=lambda r: (float(r.get("r2") or -9), float(r.get("basiskele_r2") or -9), -float(r.get("mape") or 9)),
                )["experiment"]
            else:
                pick = "duplex_largehome_global"
            for r in site_ablation_rows_v24:
                r["selected"] = r.get("experiment") == pick
                if r["selected"] and str(pick) in site_arm_names and not r.get("notes"):
                    r["notes"] = (
                        "selected vs duplex_largehome_global "
                        "(severe=0 + R2/MAPE + per-county + leakage gates); best_checkpoint still false"
                    )
                if r["selected"] and pick == "duplex_largehome_global":
                    r["notes"] = (
                        str(r.get("notes") or "")
                        + ("; " if r.get("notes") else "")
                        + "kept_safe_duplex; site arms failed severe/gates"
                    ).strip("; ")
            pd.DataFrame(site_ablation_rows_v24).to_csv(
                out_dirs["reports"] / "metrics_kocaeli_site_ablation_v24.csv",
                index=False,
                encoding="utf-8-sig",
            )
            pd.DataFrame(site_ablation_rows_v24).to_csv(
                out_dirs["reports"] / "metrics_kocaeli_site_ablation_v24_1.csv",
                index=False,
                encoding="utf-8-sig",
            )
            pd.DataFrame(
                [
                    {
                        ...
                        "r2": r.get("r2"),
                        "mape": r.get("mape"),
                        "merge_audit_severe_warnings": r.get("merge_audit_severe_warnings"),
                        "selected": r.get("selected"),
                    }
                    for r in site_ablation_rows_v24
                ]
            ).to_csv(out_dirs["reports"] / "expensive_decile_bias_comparison_v24.csv", index=False, encoding="utf-8-sig")
            picked = by_exp.get(pick) or dict(V24_SAFE_DUPLEX_REF)
            selected_site_mode = str(picked.get("site_extraction_mode") or selected_site_mode)
            selected_site_encoding = str(picked.get("site_project_encoding") or selected_site_encoding)
            selected_duplex_mode = str(picked.get("duplex_feature_mode") or selected_duplex_mode)
            selection_payload = {
                "selected_experiment": pick,
                "best_checkpoint": False,
                "promote": False,
                "site_extraction_mode": selected_site_mode,
                "site_project_encoding": selected_site_encoding,
                "duplex_feature_mode": selected_duplex_mode,
                "safe_duplex_reference": V24_SAFE_DUPLEX_REF,
                "control_reference": V24_CONTROL_REF,
                "eligible_site_arms": [c["experiment"] for c in candidates],
                "note": (
                    "Do not promote V24.1 until severe merges reviewed/fixed and gates pass. "
                    "best_checkpoint remains false."
                ),
            }
            (out_dirs["reports"] / "selection_decision_v24_1.json").write_text(
                json.dumps(selection_payload, indent=2, ensure_ascii=False),
                encoding="utf-8",
            )
            _plog(
                f"Selected V24.1 after ablation: {pick} "
                f"(site={selected_site_mode}, enc={selected_site_encoding}, duplex={selected_duplex_mode}) "
                f"[best_checkpoint not auto-updated]"
            )

            # Site-repair-only: skip expensive FINAL retrain when safe duplex remains selected
            if site_repair_only and pick == "duplex_largehome_global":
                _plog(
                    "V24.1 site_repair_only: keeping duplex_largehome_global — "
                    "skipping FINAL retrain (use V24 full duplex artifacts)."
                )
                (out_dirs["base"] / "run_config_v24_kocaeli.json").write_text(
                    json.dumps({**asdict(cfg), "selected_experiment": pick, "site_repair_only": True}, indent=2, ensure_ascii=False),
                    encoding="utf-8",
                )
                # Still export site catalog/coverage with full extraction for review
                try:
                    opt_adder = SiteProjectExtractionAdder(
                    ...
                    write_site_project_options(opt_frame, out_dirs["artifacts"])
                    coverage_by_county(opt_frame).to_csv(
                        out_dirs["reports"] / "site_project_coverage_v24_by_county__full_extraction_catalog.csv",
                        index=False,
                        encoding="utf-8-sig",
                    )
                    merge_df = pd.DataFrame(opt_adder.merge_rows_ or [])
                    if "county" not in merge_df.columns and not merge_df.empty:
                        merge_df["county"] = merge_df["canonical_id"].astype(str).map(
                            lambda x: x.split("::", 1)[0] if "::" in str(x) else ""
                        )
                    merge_df.to_csv(
                        out_dirs["reports"] / "site_project_merge_audit_v24__full_extraction_catalog.csv",
                        index=False,
                        encoding="utf-8-sig",
                    )
                    severe_merge_rows(merge_df, experiment="full_extraction_catalog").to_csv(
                        out_dirs["reports"] / "site_project_merge_audit_severe_v24__full_extraction_catalog.csv",
                        index=False,
                        encoding="utf-8-sig",
                    )
                except Exception as opt_exc:
                    warnings.warn(f"V24.1 catalog export skipped: {opt_exc}")
                progress.finish("tamamlandı")
                set_progress(None)
                print(json.dumps(selection_payload, indent=2, ensure_ascii=False))
                print(f"\nOutputs written to: {out_dirs['base'].resolve()}")
        ...
        final_cfg_dict = {
            **asdict(cfg),
            ...
            "site_project_encoding": selected_site_encoding,
            "duplex_feature_mode": selected_duplex_mode,
            "ablation_experiment_name": "final",
        }
        ...
        # V25 single-run (smoke / non-ablation) feature + metadata guards
        try:
            fi = out_dirs["reports"] / "feature_importance_v18_basiskele.csv"
            enc_names = []
            if fi.exists():
                _fidf = pd.read_csv(fi)
                _col = "feature" if "feature" in _fidf.columns else _fidf.columns[0]
                enc_names = _fidf[_col].astype(str).tolist()
                _fidf.to_csv(out_dirs["reports"] / "feature_importance_v25.csv", index=False, encoding="utf-8-sig")
            heat_all = heating_yerden_audit(sales_clean, rentals_clean)
            yerden_n = 0
            if not heat_all.empty:
                sub = heat_all[(heat_all["purpose"] == "sale") & (heat_all["county"] == "__ALL__")]
            ...
            diff = build_feature_schema_diff(
                v25_schema_cols=list(sales_clean.columns),
                v25_encoded_names=enc_names,
                v24_importance_csv=Path(
                    "v3/outputs/v24_1_kocaeli_site_merge_repair/ablation_v24_full_v24/reports/feature_importance_v18_basiskele.csv"
                ),
                heating_raw_yerden_count=yerden_n,
                fast_mode=bool(getattr(args, "fast", False)),
                duplex_feature_mode=str(selected_duplex_mode),
                site_project_encoding=str(selected_site_encoding),
            )
            (out_dirs["reports"] / "feature_schema_diff_v24_1_vs_v25.json").write_text(
                json.dumps(diff, indent=2, ensure_ascii=False), encoding="utf-8"
            )
            if str(diff.get("status")) == "FAIL":
                raise RuntimeError(f"V25 critical feature guard FAIL: {diff.get('critical_missing_features')}")
            if isinstance(metrics_summary, dict):
                metrics_summary["model_version"] = V25_MODEL_VERSION
                metrics_summary["model_scope"] = V25_MODEL_SCOPE
                metrics_summary["county_policy"] = policy_info["county_policy"]
                metrics_summary["included_counties"] = policy_info["included_counties"]
                metrics_summary["excluded_counties"] = policy_info["excluded_counties"]
                metrics_summary["v24_1_old_reference"] = V24_1_REFERENCE
                metrics_summary["feature_schema_diff_status"] = diff.get("status")
                (out_dirs["reports"] / "metrics_summary_v25_core_good_ok.json").write_text(
                    json.dumps(metrics_summary, indent=2, ensure_ascii=False, default=str),
                    encoding="utf-8",
                )
            # Duplicate key site reports with v25 names when present
            for src_name, dst_name in [
                ("site_project_coverage_v24_by_county.csv", "site_project_coverage_v25_by_county.csv"),
                ("site_project_merge_audit_v24.csv", "site_project_merge_audit_v25.csv"),
            ]:
                src = out_dirs["reports"] / src_name
                if src.exists():
                    (out_dirs["reports"] / dst_name).write_bytes(src.read_bytes())
        except RuntimeError:
            raise
        except Exception as _v25_final_exc:
            warnings.warn(f"V25 single-run guard/report warning: {_v25_final_exc}")
    ...
    # App-safe site options catalog (always full extraction so coverage is not wiped by site=none selection)
    try:
        sales_for_opt = sales_final_base
        opt_adder = SiteProjectExtractionAdder(
            site_extraction_mode="full",
            site_project_encoding="foldsafe_target",
            min_site_freq=1,
        ...
        opt_frame = opt_adder.fit_transform(sales_for_opt)
        write_site_project_options(opt_frame, out_dirs["artifacts"])
        coverage_by_county(opt_frame).to_csv(
            out_dirs["reports"] / "site_project_coverage_v24_by_county.csv",
            index=False,
        ...
        candidates_table(opt_frame).to_csv(
            out_dirs["reports"] / "site_project_candidates_v24_by_county.csv",
            index=False,
            encoding="utf-8-sig",
        )
    except Exception as opt_exc:
        warnings.warn(f"V24 site option export skipped: {opt_exc}")
    ...
    metrics_summary["duplex_ablation"] = duplex_ablation_rows
    metrics_summary["selected_duplex_feature_mode"] = selected_duplex_mode
    metrics_summary["v21_old_reference"] = {"r2": 0.5059, "mape": 0.1055, "variance_ratio": 0.4590}
    metrics_summary["primary_comparison"] = "control_v21_on_new_data"
    ...
    # V23 post-hoc duplex / large-home OOF segment reports
    try:
        oof_path = out_dirs["output"] / "oof_predictions_v18_basiskele.csv"
        if oof_path.exists():
            oof = pd.read_csv(oof_path)
            audit = extract_duplex_frame(oof if "detail_konut_tipi" in oof.columns or "title" in oof.columns else sales_final_base)
            # align lengths cautiously
            if len(audit) == len(oof):
                pred_col = "pred_ensemble" if "pred_ensemble" in oof.columns else (
                    "pred_ensemble_base" if "pred_ensemble_base" in oof.columns else None
                )
                y_col = "actual_unit_price_gross" if "actual_unit_price_gross" in oof.columns else None
                if pred_col and y_col:
                    tmp = oof.copy()
                    tmp["duplex_type"] = audit["duplex_type"].values
                    tmp["is_duplex"] = audit["is_duplex"].values
                    gross = pd.to_numeric(tmp.get("gross_m2"), errors="coerce") if "gross_m2" in tmp.columns else pd.Series(np.nan, index=tmp.index)
                    tmp["is_large_home"] = (gross >= LARGE_HOME_M2).fillna(False).astype(int)
                    rows = []
                    for key, mask in [
                        ("duplex", tmp["is_duplex"].astype(bool)),
                        ("non_duplex", ~tmp["is_duplex"].astype(bool)),
                        ("roof", tmp["duplex_type"] == "roof"),
                        ("garden", tmp["duplex_type"] == "garden"),
                        ("middle_floor", tmp["duplex_type"] == "middle_floor"),
                        ("standard", tmp["duplex_type"] == "standard"),
                    ]:
                        sub = tmp.loc[mask]
                        if len(sub) < 5:
                            rows.append({"segment": key, "rows": int(len(sub)), "r2": np.nan, "mape": np.nan})
                            continue
                        yv = pd.to_numeric(sub[y_col], errors="coerce")
                        pv = pd.to_numeric(sub[pred_col], errors="coerce")
                        ok = yv.notna() & pv.notna() & (yv > 0)
                        yv, pv = yv[ok], pv[ok]
                        ape = (pv - yv).abs() / yv
                        rows.append({
                            "segment": key,
                            "rows": int(ok.sum()),
                            "r2": float(r2_score(yv, pv)) if len(yv) > 1 else np.nan,
                            "mape": float(ape.mean()) if len(yv) else np.nan,
                        })
                    pd.DataFrame(rows).to_csv(out_dirs["reports"] / "duplex_segment_performance_v23.csv", index=False, encoding="utf-8-sig")
                    lh_rows = []
                    for key, mask in [("large_home", tmp["is_large_home"] == 1), ("non_large_home", tmp["is_large_home"] == 0)]:
                        sub = tmp.loc[mask]
                        yv = pd.to_numeric(sub[y_col], errors="coerce")
                        pv = pd.to_numeric(sub[pred_col], errors="coerce")
                        ok = yv.notna() & pv.notna() & (yv > 0)
                    ...
                    pd.DataFrame(lh_rows).to_csv(out_dirs["reports"] / "large_home_performance_v23.csv", index=False, encoding="utf-8-sig")
                    # top expensive underpredicted
                    y_all = pd.to_numeric(tmp[y_col], errors="coerce")
                    p_all = pd.to_numeric(tmp[pred_col], errors="coerce")
                    q90 = y_all.quantile(0.9)
                    exp = tmp.loc[y_all >= q90].copy()
                    exp["residual"] = pd.to_numeric(exp[y_col], errors="coerce") - pd.to_numeric(exp[pred_col], errors="coerce")
                    under = exp.sort_values("residual", ascending=False).head(50)
                    keep_cols = [c for c in ["classified_id", "district", "duplex_type", "is_large_home", y_col, pred_col, "residual", "gross_m2"] if c in under.columns]
                    under[keep_cols].to_csv(out_dirs["reports"] / "top_expensive_underpredicted_v23.csv", index=False, encoding="utf-8-sig")
    except Exception as v23_seg_exc:
        warnings.warn(f"V23 duplex segment reports skipped: {v23_seg_exc}")
