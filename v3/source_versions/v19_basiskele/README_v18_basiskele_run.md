# Historical reference — V18 Başiskele-only comparable pipeline

> **Historical copy only.** This file documents the closed V18 checkpoint that V19
> starts from. It is **not** the active V19 run guide.
>
> - Active V19 package: [`README.md`](README.md) in this folder
> - Original V18 tree: `v2/source_versions/v18_basiskele/`
> - Curated V18 pack: `v2/best_checkpoints/best_basiskele_only_checkpoint/`
>
> **Do not treat this as current training instructions for V19.**
...
Ablate fold-safe comparable market features on a Başiskele-only training set
against a geo + coast control. Outcome: comparables rejected; geo control kept.
...
Canonical pointer (in the original V18 tree): `V18_BEST_CHECKPOINT.json`
...
- Default V18 Başiskele research config stays `comparable_mode=none`.
- Comparable code may remain for **diagnostic / confidence** side channels only —
  not as training inputs to the final ensemble.
- V19 keeps `comparable_mode=none` (forced) and explores calibration instead.
...
## Reproduce final checkpoint (original V18 tree)
...
Prefer the archived path after layout migration:
