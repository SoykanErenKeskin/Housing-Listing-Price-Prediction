...
"""
V7 DB -> clean -> train -> report pipeline for Kocaeli housing unit price model.
...
Example:
  python train_v7_db_pipeline.py ^
    --out outputs/v7_kocaeli ^
    --city Kocaeli ^
...
def clean_sales_and_rentals(
    ...
    sales_clean.to_csv(out_dirs["input"] / "sales_cleaned_v7.csv", index=False, encoding="utf-8-sig")
    rentals_clean.to_csv(out_dirs["input"] / "rentals_cleaned_v7.csv", index=False, encoding="utf-8-sig")
    pd.concat([sales_removed_basic, sales_removed_iqr], ignore_index=True).to_csv(
        out_dirs["input"] / "sales_removed_v7.csv", index=False, encoding="utf-8-sig"
    )
    pd.concat([rentals_removed_basic, rentals_removed_iqr], ignore_index=True).to_csv(
        out_dirs["input"] / "rentals_removed_v7.csv", index=False, encoding="utf-8-sig"
    )
...
def train_and_evaluate(X: pd.DataFrame, y: pd.Series, cfg: RunConfig, out_dirs: dict[str, Path]) -> tuple[ModelBundle, dict[str, Any]]:
    ...
    for name, estimator in specs.items():
        ...
        joblib.dump(final_estimator, out_dirs["artifacts"] / f"model_{name}_v7.joblib")

    model_comparison = pd.DataFrame(comparison_rows).sort_values(["mape", "mae_tl_per_m2"])
    model_comparison.to_csv(out_dirs["reports"] / "model_comparison_v7.csv", index=False, encoding="utf-8-sig")
    ...
    ensemble_metrics["model"] = "ensemble_top_models_v7"
    ...
    joblib.dump(bundle, out_dirs["artifacts"] / "ensemble_model_bundle_v7.joblib")
    ...
    oof.to_csv(out_dirs["output"] / "oof_predictions_v7.csv", index=False, encoding="utf-8-sig")

    metrics_summary = {
        "ensemble": ensemble_metrics,
        "model_comparison": model_comparison.to_dict(orient="records"),
    }
    (out_dirs["reports"] / "metrics_summary_v7.json").write_text(
...
def save_group_error_reports(oof: pd.DataFrame, reports_dir: Path) -> None:
    ...
    for col in group_cols:
        ...
        rep.to_csv(reports_dir / f"error_by_{col}_v7.csv", index=False, encoding="utf-8-sig")
...
def save_plots(oof: pd.DataFrame, reports_dir: Path) -> None:
    ...
    plt.title("V7 OOF: Gerçek ve Tahmin Edilen Birim Fiyat")
    plt.grid(True, linestyle="--", alpha=0.45)
    plt.tight_layout()
    plt.savefig(reports_dir / "actual_vs_predicted_v7.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    plt.title("V7 OOF: Hata Dağılımı")
    plt.grid(True, axis="y", linestyle="--", alpha=0.45)
    plt.tight_layout()
    plt.savefig(reports_dir / "residual_distribution_v7.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    if "district" in oof.columns:
        ...
        if not rep.empty:
            ...
            plt.xlabel("MAPE (%)")
            plt.title("V7 OOF: Mahalle Bazlı En Yüksek MAPE")
            plt.grid(True, axis="x", linestyle="--", alpha=0.45)
            plt.tight_layout()
            plt.savefig(reports_dir / "mape_by_district_v7.png", dpi=300, bbox_inches="tight")
            plt.close()
...
def save_feature_importance(models: dict[str, Any], reports_dir: Path) -> None:
    ...
    rep.to_csv(reports_dir / "feature_importance_v7.csv", index=False, encoding="utf-8-sig")
    top = rep.groupby("feature", as_index=False)["importance"].mean().sort_values("importance", ascending=False).head(50)
    top.to_csv(reports_dir / "feature_importance_top50_v7.csv", index=False, encoding="utf-8-sig")
...
def write_run_readme(out_dirs: dict[str, Path], cfg: RunConfig, metrics_summary: dict[str, Any], cleaning_report: dict[str, Any], feature_reports: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "- data/input/sales_cleaned_v7.csv",
        "- data/input/rentals_cleaned_v7.csv",
        "- data/output/oof_predictions_v7.csv",
        "- reports/model_comparison_v7.csv",
        "- reports/metrics_summary_v7.json",
        "- reports/error_by_*_v7.csv",
        "- reports/*.png",
        "- artifacts/model_*_v7.joblib",
        "- artifacts/ensemble_model_bundle_v7.joblib",
        "",
    ]
    (out_dirs["base"] / "README_v7_run.md").write_text("\n".join(lines), encoding="utf-8")
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--out", default="outputs/v7_db_pipeline")
    ap.add_argument("--target-mode", choices=["log", "raw"], default="log")
...
def main() -> None:
    ...
    (out_dirs["base"] / "run_config_v7.json").write_text(json.dumps(asdict(cfg), indent=2, ensure_ascii=False), encoding="utf-8")
    ...
    sales_final.to_csv(out_dirs["input"] / "sales_training_table_v7.csv", index=False, encoding="utf-8-sig")
