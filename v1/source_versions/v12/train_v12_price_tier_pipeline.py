...
"""
V12 DB -> clean -> residual-target train -> anomaly/county-expert + demographics + price-tier correction pipeline for Kocaeli housing unit price model.
...
Example:
  python train_v12_demographics_pipeline.py ^
...
class RunConfig:
    ...
    enable_price_tier_correction: bool
    price_tier_low_quantile: float
    price_tier_high_quantile: float
    price_tier_min_rows: int
...
class ModelBundle:
    ...
    def __init__(
        ...
        tier_models: dict[str, Any] | None = None,
        tier_weights: dict[str, float] | None = None,
        tier_blend_weight: float = 0.0,
        tier_scope: str = "disabled",
        tier_low_threshold: float | None = None,
        tier_high_threshold: float | None = None,
        tier_clip_low: float = -0.35,
        tier_clip_high: float = 0.50,
    ):
        self.models = models
        ...
        self.tier_blend_weight = float(tier_blend_weight or 0.0)
        self.tier_scope = str(tier_scope or "disabled")
        self.tier_low_threshold = tier_low_threshold
        self.tier_high_threshold = tier_high_threshold
        self.tier_clip_low = float(tier_clip_low)
        self.tier_clip_high = float(tier_clip_high)
    ...
    def predict(self, X: pd.DataFrame) -> np.ndarray:
        ...
        if self.tier_models and self.tier_weights and self.tier_blend_weight > 0:
            correction_pct = self._weighted_predict(aligned_X, self.tier_models, self.tier_weights)
            correction_pct = np.clip(np.asarray(correction_pct, dtype=float), self.tier_clip_low, self.tier_clip_high)
            if self.tier_scope == "extremes_only" and self.tier_low_threshold is not None and self.tier_high_threshold is not None:
                tier_mask = (pred <= float(self.tier_low_threshold)) | (pred >= float(self.tier_high_threshold))
            elif self.tier_scope == "all_rows":
                tier_mask = np.ones(len(pred), dtype=bool)
            else:
                tier_mask = np.zeros(len(pred), dtype=bool)
            if tier_mask.any():
                pred[tier_mask] = pred[tier_mask] * (1.0 + self.tier_blend_weight * correction_pct[tier_mask])
...
def fetch_listing_table(engine, table: str, purpose: str, city: str, limit: int | None = None) -> pd.DataFrame:
    """Fetch city listings (canonical DB + legacy DF aliases)."""
    try:
        ...
        for _p in [_here.parent, *_here.parents]:
            ...
            if (_cand / "canonical_db.py").is_file():
                ...
                break
        from canonical_db import fetch_listing_table as _fetch_listing_table
    source = DEFAULT_SOURCE_SITE if "DEFAULT_SOURCE_SITE" in globals() else None
    return _fetch_listing_table(
        ...
        limit=limit,
        source_site=source,
        filter_source_site=bool(source),
    )
...
def attach_demographic_features(listings: pd.DataFrame, demo_features: pd.DataFrame, mode: str, out_dirs: dict[str, Path] | None = None) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if out_dirs is not None:
        ...
        if present and "county" in out.columns and "district" in out.columns:
            ...
            cov.to_csv(out_dirs["reports"] / f"demographic_feature_coverage_{mode}_v12.csv", index=False, encoding="utf-8-sig")
    return out, report
...
def clean_sales_and_rentals(
    ...
    sales_clean.to_csv(out_dirs["input"] / "sales_cleaned_v12.csv", index=False, encoding="utf-8-sig")
    rentals_clean.to_csv(out_dirs["input"] / "rentals_cleaned_v12.csv", index=False, encoding="utf-8-sig")
    pd.concat([sales_removed_basic, sales_removed_iqr], ignore_index=True).to_csv(
        out_dirs["input"] / "sales_removed_v12.csv", index=False, encoding="utf-8-sig"
    )
    pd.concat([rentals_removed_basic, rentals_removed_iqr], ignore_index=True).to_csv(
        out_dirs["input"] / "rentals_removed_v12.csv", index=False, encoding="utf-8-sig"
    )
...
def apply_location_outlier_filter(sales: pd.DataFrame, cfg: RunConfig, out_dirs: dict[str, Path]) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    ...
    removed.to_csv(out_dirs["input"] / "sales_removed_location_outliers_v12.csv", index=False, encoding="utf-8-sig")
    clean.to_csv(out_dirs["input"] / "sales_after_location_outlier_filter_v12.csv", index=False, encoding="utf-8-sig")
    ...
    for segment_name in segment_names:
        ...
        if use_segment:
            ...
            for model_name, model_obj in selected.items():
                joblib.dump(model_obj, out_dirs["artifacts"] / f"model_segment_{segment_name}_{model_name}_v12.joblib")
        ...
        seg_comparison.to_csv(out_dirs["reports"] / f"model_comparison_segment_{segment_name}_v12.csv", index=False, encoding="utf-8-sig")

    report = pd.DataFrame(report_rows)
    report.to_csv(out_dirs["reports"] / "segment_layer_report_v12.csv", index=False, encoding="utf-8-sig")
...
def save_anomaly_reports(anom: pd.DataFrame, reports_dir: Path, output_dir: Path) -> None:
    if anom.empty:
        return
    anom.sort_values("anomaly_score", ascending=False).to_csv(output_dir / "listing_anomaly_scores_v12.csv", index=False, encoding="utf-8-sig")
    anom.sort_values("anomaly_score", ascending=False).head(250).to_csv(reports_dir / "top_listing_anomalies_v12.csv", index=False, encoding="utf-8-sig")
    ...
    def group_anomaly(col: str) -> None:
        ...
        rep.to_csv(reports_dir / f"anomaly_by_{col}_v12.csv", index=False, encoding="utf-8-sig")
...
def train_county_expert_layer(
    ...
    if not cfg.enable_county_experts or "county" not in X.columns:
        ...
        report.to_csv(out_dirs["reports"] / "county_expert_report_v12.csv", index=False, encoding="utf-8-sig")
    ...
    for county_name in counties:
        ...
        comp.to_csv(out_dirs["reports"] / f"model_comparison_county_{safe_county}_v12.csv", index=False, encoding="utf-8-sig")
        ...
        if use_county:
            ...
            for model_name, model_obj in selected.items():
                joblib.dump(model_obj, out_dirs["artifacts"] / f"model_county_{safe_county}_{model_name}_v12.joblib")
...
# =========================
# V12 price-tier correction layer
# =========================

def price_tier_slice_report(y: pd.Series, pred_before: pd.Series, pred_after: pd.Series, out_dirs: dict[str, Path]) -> pd.DataFrame:
    actual = pd.to_numeric(y, errors="coerce").reset_index(drop=True)
    before = pd.to_numeric(pred_before, errors="coerce").reset_index(drop=True)
    after = pd.to_numeric(pred_after, errors="coerce").reset_index(drop=True)
    valid = actual.notna() & before.notna() & after.notna() & (actual > 0)
    df = pd.DataFrame({"actual": actual[valid], "pred_before": before[valid], "pred_after": after[valid]})
    if df.empty:
        return pd.DataFrame()
    try:
        df["actual_decile"] = pd.qcut(df["actual"], 10, labels=False, duplicates="drop")
    except Exception:
        df["actual_decile"] = 0
    rows = []
    for decile, g in df.groupby("actual_decile", dropna=False):
        mb = metric_dict(g["actual"], g["pred_before"])
        ma = metric_dict(g["actual"], g["pred_after"])
        rows.append({
            "actual_decile": int(decile) if pd.notna(decile) else -1,
            "rows": int(len(g)),
            "actual_mean": float(g["actual"].mean()),
            "pred_before_mean": float(g["pred_before"].mean()),
            "pred_after_mean": float(g["pred_after"].mean()),
            "before_mape": mb["mape"],
            "after_mape": ma["mape"],
            "before_mae_tl_per_m2": mb["mae_tl_per_m2"],
            "after_mae_tl_per_m2": ma["mae_tl_per_m2"],
            "mean_bias_before": float((g["pred_before"] - g["actual"]).mean()),
            "mean_bias_after": float((g["pred_after"] - g["actual"]).mean()),
        })
    rep = pd.DataFrame(rows).sort_values("actual_decile")
    rep.to_csv(out_dirs["reports"] / "price_tier_decile_report_v12.csv", index=False, encoding="utf-8-sig")
    return rep


def train_price_tier_correction_layer(
    X: pd.DataFrame,
    y: pd.Series,
    current_pred: pd.Series,
    cfg: RunConfig,
    out_dirs: dict[str, Path],
) -> tuple[pd.Series, dict[str, Any], dict[str, float], dict[str, Any], pd.DataFrame, pd.DataFrame]:
    """Learn an app-safe residual-percent correction for cheap/expensive tails.

    The correction target is based on OOF current predictions, then learned from app-safe
    features only. Candidate blends are validated OOF and applied only when MAPE improves.
    """
    if not bool(getattr(cfg, "enable_price_tier_correction", True)):
        report = pd.DataFrame([{"status": "disabled"}])
        return current_pred.copy(), {}, {}, {"status": "disabled", "blend_weight": 0.0}, report, pd.DataFrame()

    n = len(X)
    if n < int(getattr(cfg, "price_tier_min_rows", 250)):
        report = pd.DataFrame([{"status": "skipped_too_few_rows", "rows": int(n)}])
        return current_pred.copy(), {}, {}, {"status": "skipped_too_few_rows", "blend_weight": 0.0}, report, pd.DataFrame()

    yv = pd.to_numeric(y, errors="coerce").reset_index(drop=True)
    base = pd.to_numeric(current_pred, errors="coerce").reset_index(drop=True).replace(0, np.nan)
    correction_target = ((yv - base) / base).replace([np.inf, -np.inf], np.nan).clip(-0.60, 0.80)
    valid = correction_target.notna() & yv.notna() & base.notna() & (yv > 0) & (base > 0)
    if int(valid.sum()) < int(getattr(cfg, "price_tier_min_rows", 250)):
        report = pd.DataFrame([{"status": "skipped_too_few_valid_rows", "rows": int(valid.sum())}])
        return current_pred.copy(), {}, {}, {"status": "skipped_too_few_valid_rows", "blend_weight": 0.0}, report, pd.DataFrame()

    Xc = X.loc[valid].reset_index(drop=True)
    yc = correction_target.loc[valid].reset_index(drop=True)
    y_actual = yv.loc[valid].reset_index(drop=True)
    base_valid = base.loc[valid].reset_index(drop=True)

    low_q = float(getattr(cfg, "price_tier_low_quantile", 0.15))
    high_q = float(getattr(cfg, "price_tier_high_quantile", 0.85))
    low_thr = float(np.nanquantile(base_valid, low_q))
    high_thr = float(np.nanquantile(base_valid, high_q))
    extreme_mask = (base_valid <= low_thr) | (base_valid >= high_thr)

    selected = [m for m in cfg.selected_models if m in {"ridge", "gradient_boosting", "extra_trees", "random_forest"}]
    if not selected:
        selected = ["ridge", "gradient_boosting", "extra_trees"]
    specs = model_specs("raw", selected_models=selected, fast_mode=cfg.fast_mode)
    split_count = min(cfg.n_splits, max(2, int(valid.sum()) // 40))
    cv = KFold(n_splits=split_count, shuffle=True, random_state=cfg.random_state)

    correction_oof = pd.DataFrame({"actual_correction_pct": yc})
    model_rows = []
    fitted: dict[str, Any] = {}
    for model_name, estimator in specs.items():
        print(f"Training V12 price-tier correction OOF model: {model_name}")
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            pred_corr = cross_val_predict(clone(estimator), Xc, yc, cv=cv, n_jobs=None, method="predict")
        pred_corr = np.clip(np.asarray(pred_corr, dtype=float), -0.35, 0.50)
        correction_oof[f"corr_{model_name}"] = pred_corr
        corrected = np.maximum(base_valid * (1.0 + pred_corr), 0)
        m = metric_dict(y_actual, corrected)
        m["model"] = model_name
        model_rows.append(m)
        final_estimator = clone(estimator)
        final_estimator.fit(Xc, yc)
    ...
    model_comp = pd.DataFrame(model_rows).sort_values(["mape", "mae_tl_per_m2"])
    model_comp.to_csv(out_dirs["reports"] / "model_comparison_price_tier_correction_v12.csv", index=False, encoding="utf-8-sig")
    weights = choose_ensemble_weights(model_comp, max_models=3)
    corr_ens = np.zeros(len(Xc), dtype=float)
    total_w = sum(weights.values()) if weights else 0.0
    if total_w <= 0:
        report = pd.DataFrame([{"status": "no_correction_weights"}])
        return current_pred.copy(), {}, {}, {"status": "no_correction_weights", "blend_weight": 0.0}, report, pd.DataFrame()
    for model_name, w in weights.items():
        corr_ens += correction_oof[f"corr_{model_name}"].to_numpy(dtype=float) * (w / total_w)
    corr_ens = np.clip(corr_ens, -0.35, 0.50)

    blend_candidates = [0.10, 0.15, 0.25, 0.35, 0.50, 0.65, 0.80, 1.00]
    scopes = ["extremes_only", "all_rows"]
    base_metrics = metric_dict(y_actual, base_valid)
    candidate_rows = []
    for scope in scopes:
        mask = extreme_mask.to_numpy(dtype=bool) if scope == "extremes_only" else np.ones(len(Xc), dtype=bool)
        for bw in blend_candidates:
            cand = base_valid.to_numpy(dtype=float).copy()
            cand[mask] = cand[mask] * (1.0 + bw * corr_ens[mask])
            cand = np.maximum(cand, 0)
            cm = metric_dict(y_actual, cand)
            candidate_rows.append({
                "scope": scope,
                "blend_weight": bw,
                "r2": cm["r2"],
                "log_r2": cm["log_r2"],
                "mape": cm["mape"],
                "mae_tl_per_m2": cm["mae_tl_per_m2"],
                "pred": cand,
            })
    best = min(candidate_rows, key=lambda row: (row["mape"], row["mae_tl_per_m2"], -row["r2"]))
    use_layer = bool(best["mape"] < base_metrics["mape"] - 1e-6)

    final_pred = current_pred.reset_index(drop=True).copy()
    selected_models: dict[str, Any] = {}
    layer_info: dict[str, Any] = {
        "status": "used_blend" if use_layer else "kept_current",
        "rows": int(n),
        ...
        "scope": best["scope"] if use_layer else "disabled",
        "blend_weight": float(best["blend_weight"]) if use_layer else 0.0,
        "base_r2": base_metrics["r2"],
        "base_log_r2": base_metrics["log_r2"],
        "base_mape": base_metrics["mape"],
        "base_mae_tl_per_m2": base_metrics["mae_tl_per_m2"],
        ...
        "best_blended_mae_tl_per_m2": best["mae_tl_per_m2"],
        "weights": json.dumps(weights, ensure_ascii=False),
        "blend_candidates": json.dumps([{k: v for k, v in row.items() if k != "pred"} for row in candidate_rows], ensure_ascii=False),
    }

    if use_layer:
        idx = np.flatnonzero(valid.to_numpy())
        final_pred.iloc[idx] = best["pred"]
        selected_models = {name: fitted[name] for name in weights}
        for model_name, model_obj in selected_models.items():
            joblib.dump(model_obj, out_dirs["artifacts"] / f"model_price_tier_correction_{model_name}_v12.joblib")

    report = pd.DataFrame([layer_info])
    report.to_csv(out_dirs["reports"] / "price_tier_correction_report_v12.csv", index=False, encoding="utf-8-sig")
    decile_report = price_tier_slice_report(y.reset_index(drop=True), current_pred.reset_index(drop=True), final_pred.reset_index(drop=True), out_dirs)
    return final_pred, selected_models, weights if use_layer else {}, layer_info, report, decile_report
...
def save_feature_importance_by_county(county_models: dict[str, dict[str, Any]], reports_dir: Path) -> None:
    ...
    rep.to_csv(reports_dir / "feature_importance_by_county_v12.csv", index=False, encoding="utf-8-sig")
    top = (
        rep.groupby(["county", "feature"], as_index=False)["importance"]
        .mean()
        .sort_values(["county", "importance"], ascending=[True, False])
    )
    top.groupby("county", group_keys=False).head(40).to_csv(reports_dir / "feature_importance_by_county_top40_v12.csv", index=False, encoding="utf-8-sig")
...
def train_and_evaluate(X: pd.DataFrame, y: pd.Series, cfg: RunConfig, out_dirs: dict[str, Path]) -> tuple[ModelBundle, dict[str, Any]]:
    ...
    for name, estimator in specs.items():
        ...
        joblib.dump(final_estimator, out_dirs["artifacts"] / f"model_{name}_v12.joblib")

    model_comparison = pd.DataFrame(comparison_rows).sort_values(["mape", "mae_tl_per_m2"])
    model_comparison.to_csv(out_dirs["reports"] / "model_comparison_v12.csv", index=False, encoding="utf-8-sig")
    ...
    base_metrics["model"] = "ensemble_base_top_models_v12"
    ...
    segment_metrics_final["model"] = "segment_aware_before_county_v12"
    ...
    # V12 final specialist: price-tier residual-percent correction for cheap/expensive tails.
    tier_pred, tier_models, tier_weights, tier_info, tier_report, tier_decile_report = train_price_tier_correction_layer(
        X=X,
        y=y,
    ...
    oof["pred_ensemble"] = np.maximum(tier_pred.to_numpy(dtype=float), 0)

    ensemble_metrics = metric_dict(y, oof["pred_ensemble"])
    ensemble_metrics["model"] = "price_tier_county_expert_segment_aware_ensemble_v12"
    ...
    ensemble_metrics["price_tier_correction"] = tier_info
    ...
    bundle = ModelBundle(
        ...
        tier_blend_weight=float(tier_info.get("blend_weight", 0.0) or 0.0),
        tier_scope=str(tier_info.get("scope", "disabled")),
        tier_low_threshold=tier_info.get("low_threshold"),
        tier_high_threshold=tier_info.get("high_threshold"),
    )
    joblib.dump(bundle, out_dirs["artifacts"] / "model_bundle_v12.joblib")
    ...
    oof.to_csv(out_dirs["output"] / "oof_predictions_v12.csv", index=False, encoding="utf-8-sig")

    county_metrics = county_metric_report(oof, pred_col="pred_ensemble")
    if not county_metrics.empty:
        county_metrics.to_csv(out_dirs["reports"] / "county_metrics_v12.csv", index=False, encoding="utf-8-sig")

    anomaly_metric_report = anomaly_metric_diagnostics(oof)
    if not anomaly_metric_report.empty:
        anomaly_metric_report.to_csv(out_dirs["reports"] / "anomaly_metric_diagnostics_v12.csv", index=False, encoding="utf-8-sig")
    ...
    metrics_summary = {
        ...
        "price_tier_correction_layer": tier_report.to_dict(orient="records") if not tier_report.empty else [],
        "price_tier_decile_report": tier_decile_report.to_dict(orient="records") if not tier_decile_report.empty else [],
    ...
    (out_dirs["reports"] / "metrics_summary_v12.json").write_text(
...
def save_group_error_reports(oof: pd.DataFrame, reports_dir: Path) -> None:
    ...
    for col in group_cols:
        ...
        rep.to_csv(reports_dir / f"error_by_{col}_v12.csv", index=False, encoding="utf-8-sig")
...
def save_plots(oof: pd.DataFrame, reports_dir: Path) -> None:
    ...
    plt.tight_layout()
    plt.savefig(reports_dir / "actual_vs_predicted_v12.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(reports_dir / "residual_distribution_v12.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    if "district" in oof.columns:
        ...
        if not rep.empty:
            ...
            plt.tight_layout()
            plt.savefig(reports_dir / "mape_by_district_v12.png", dpi=300, bbox_inches="tight")
            plt.close()
...
def save_feature_importance(models: dict[str, Any], reports_dir: Path) -> None:
    ...
    rep.to_csv(reports_dir / "feature_importance_v12.csv", index=False, encoding="utf-8-sig")
    top = rep.groupby("feature", as_index=False)["importance"].mean().sort_values("importance", ascending=False).head(50)
    top.to_csv(reports_dir / "feature_importance_top50_v12.csv", index=False, encoding="utf-8-sig")
...
def write_run_readme(out_dirs: dict[str, Path], cfg: RunConfig, metrics_summary: dict[str, Any], cleaning_report: dict[str, Any], feature_reports: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "- data/input/sales_cleaned_v12.csv",
        "- data/input/rentals_cleaned_v12.csv",
        "- data/output/oof_predictions_v12.csv",
        "- reports/model_comparison_v12.csv",
        "- reports/metrics_summary_v12.json",
        "- reports/error_by_*_v12.csv",
        "- reports/*.png",
        "- artifacts/model_*_v12.joblib",
        "- artifacts/model_bundle_v12.joblib",
        "",
    ]
    (out_dirs["base"] / "README_v12_run.md").write_text("\n".join(lines), encoding="utf-8")
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--out", default="outputs/v12_price_tier_pipeline")
    ...
    ap.add_argument("--price-tier-correction", action=argparse.BooleanOptionalAction, default=True, help="Enable V12 residual-percent price-tier correction layer for cheap/expensive tails.")
    ap.add_argument("--price-tier-low-quantile", type=float, default=0.15, help="Predicted-price lower quantile used for cheap-tail correction scope.")
    ap.add_argument("--price-tier-high-quantile", type=float, default=0.85, help="Predicted-price upper quantile used for premium-tail correction scope.")
    ap.add_argument("--price-tier-min-rows", type=int, default=250, help="Minimum rows needed to train the V12 price-tier correction layer.")
...
def main() -> None:
    ...
    cfg = RunConfig(
        ...
        enable_price_tier_correction=bool(args.price_tier_correction),
        price_tier_low_quantile=float(args.price_tier_low_quantile),
        price_tier_high_quantile=float(args.price_tier_high_quantile),
        price_tier_min_rows=int(args.price_tier_min_rows),
    )

    out_dirs = build_output_dirs(args.out)
    (out_dirs["base"] / "run_config_v12.json").write_text(json.dumps(asdict(cfg), indent=2, ensure_ascii=False), encoding="utf-8")
    ...
    def run_one_demographics_mode(mode: str, mode_out_dirs: dict[str, Path], final_run: bool = False):
        ...
        sales_with_demo.to_csv(mode_out_dirs["input"] / f"sales_training_table_v12_{mode}.csv", index=False, encoding="utf-8-sig")
        ...
        if mode_cfg.exclude_anomalies_threshold and mode_cfg.exclude_anomalies_threshold > 0:
            ...
            pre_anom.assign(training_excluded_by_threshold=(~keep_mask).astype(int)).to_csv(
                mode_out_dirs["reports"] / f"anomaly_training_filter_{mode}_v12.csv", index=False, encoding="utf-8-sig"
            )
            if excluded > 0:
        ...
        (mode_out_dirs["reports"] / "metrics_summary_v12.json").write_text(
            json.dumps(metrics_summary, indent=2, ensure_ascii=False), encoding="utf-8"
        )
    ...
    if not ablation_df.empty:
        ...
        ablation_df[keep_cols].to_csv(out_dirs["reports"] / "metrics_demographics_ablation_v12.csv", index=False, encoding="utf-8-sig")

    final_mode = str(args.demographics_mode)
    if bool(args.run_demographics_ablation):
        print(f"\n========== V12 FINAL SELECTED MODE: {final_mode} ==========")
