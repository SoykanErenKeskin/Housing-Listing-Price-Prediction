...
"""
V11 DB -> clean -> residual-target train -> anomaly/county-expert + demographics pipeline for Kocaeli housing unit price model.
...
DEMOGRAPHIC_SAFE_NUMERIC_FEATURES = [
    ...
    "demo_has_demographics", "demo_age_coverage", "demo_education_coverage", "demo_income_available",
...
def build_demographic_features(demo_raw: pd.DataFrame, mode: str = "safe") -> pd.DataFrame:
    ...
    flag_cols = ["demo_has_demographics", "demo_age_coverage", "demo_education_coverage", "demo_income_available", "demo_ses_available", "demo_market_activity_available", "demo_infrastructure_available"]
    out["demo_coverage_score"] = out[flag_cols].mean(axis=1)
...
def attach_demographic_features(listings: pd.DataFrame, demo_features: pd.DataFrame, mode: str, out_dirs: dict[str, Path] | None = None) -> tuple[pd.DataFrame, dict[str, Any]]:
    if mode == "none" or demo_features is None or demo_features.empty:
        ...
        return out, {"mode": mode, "demo_rows": 0, "matched_listing_rows": 0, "match_rate": 0.0}
    out = listings.copy()
    for key in ["city_id", "county_id", "district_id"]:
    ...
    report = {"mode": mode, "demo_rows": int(len(demo)), "listing_rows": int(before), "matched_listing_rows": matched, "match_rate": float(matched / before) if before else 0.0}
