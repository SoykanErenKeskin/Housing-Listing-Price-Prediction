"""V15 Başiskele premium specialist + large-home redesign features.
...
class LargeHomeFeatureAdder(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in LARGE_HOME_NUMERIC_FEATURES:
            if col not in df.columns:
                df[col] = 0.0
        return df
...
class BasiskelePremiumSpecialistAdder(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if not getattr(self, "enabled_", False):
            for col in BASISKELE_PREMIUM_NUMERIC_FEATURES:
                if col not in df.columns:
                    df[col] = 0.0
            if "basiskele_premium_bucket" not in df.columns:
                df["basiskele_premium_bucket"] = "none"
            return df
...
class BasiskelePremiumTargetStatsAdder(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        df = X.copy()
        if not getattr(self, "enabled_", False):
            for col in BASISKELE_TARGET_STAT_FEATURES:
                if col not in df.columns:
                    df[col] = 0.0
            return df
