"""Interactive arrow-key CLI prompts for V15 training.
...
def apply_interactive_prompts(args: Any, argv: list[str] | None = None) -> Any:
    ...
    print("\n════════════════════════════════════════")
    print(" V15 etkileşimli ayar sihirbazı")
    ...
    if not cli_flag_provided(argv, "--out"):
        args.out = ask_text(
            ...
            str(getattr(args, "out", "outputs/v15_full")),
        )
    ...
    if not cli_flag_provided(argv, "--basiskele-variance-lift"):
        cur = str(getattr(args, "basiskele_variance_lift", "conservative"))
        opts = [
            ChoiceOption("none", "none", "Variance-lift layer kapalı."),
            ChoiceOption("conservative", "conservative", "Küçük lambda; R2 artmazsa kapanır (önerilen)."),
        ...
        args.basiskele_variance_lift = ask_choice(
            ...
            "OOF-safe second-stage delta; mean-pulling azaltma denemesi.",
            opts,
            default_i,
        )
    ...
    if not cli_flag_provided(argv, "--county-expert-min-rows"):
        cur = int(getattr(args, "county_expert_min_rows", 250))
        opts = [
            ChoiceOption(250, "250 (default)", "Karamürsel genelde expert almaz; V15.0 ana run."),
            ChoiceOption(180, "180 (deney)", "Karamürsel expert açılabilir; attribute ile karıştırma."),
        ]
        default_i = 0 if cur >= 250 else 1
    ...
    print(f"  basiskele-variance-lift={getattr(args, 'basiskele_variance_lift', 'conservative')}")
...
def build_early_arg_parser() -> Any:
    ...
    ap.add_argument("--counties", default="İzmit,Başiskele,Gölcük,Karamürsel")
    ...
    ap.add_argument("--county-expert-min-rows", type=int, default=250)
    ...
    ap.add_argument("--basiskele-variance-lift", choices=["none", "conservative", "full"], default="conservative")
