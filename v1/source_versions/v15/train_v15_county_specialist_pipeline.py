...
"""
V15 county-specialist pipeline for Kocaeli housing unit price model.
...
Example:
  python train_v15_county_specialist_pipeline.py --out outputs/v15_full \\
...
if __name__ == "__main__":
    _v15_dir = str(Path(__file__).resolve().parent)
    if _v15_dir not in sys.path:
        sys.path.insert(0, _v15_dir)
    sys.stdout.write("V15 ayar sihirbazı açılıyor...\n")
    sys.stdout.flush()
...
try:
    ...
    from v15.attribute_features import (
        ATTRIBUTE_BASIC_NUMERIC_FEATURES,
        ATTRIBUTE_EFFECT_NUMERIC_FEATURES,
        AttributeEffectEncoder,
        AttributeInteractionCompleter,
...
try:
    ...
    from v15.diagnostics_v15 import (
        attribute_feature_coverage,
        evaluate_decision,
        run_basiskele_variance_diagnostics,
        run_karamursel_sensitivity,
...
try:
    ...
    from v15.detail_premium_features import (
        DETAIL_EFFECT_GROUP_NUMERIC_FEATURES,
        V13_DEFAULT_REF,
        V14_DEFAULT_REF,
        LocalDetailPremiumEncoder,
...
try:
    ...
    from v15.county_specialist_features import (
        BasiskelePremiumSpecialistAdder,
        BasiskelePremiumTargetStatsAdder,
        LargeHomeFeatureAdder,
        get_county_specialist_feature_names,
...
try:
    ...
    from v15.train_progress import TrainProgress, estimate_training_units, get_progress, set_progress
...
try:
    ...
    # Prefer local v15/.env next to this file (cwd-independent).
...
def attach_demographic_features(listings: pd.DataFrame, demo_features: pd.DataFrame, mode: str, out_dirs: dict[str, Path] | None = None) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if out_dirs is not None:
        ...
        if present and "county" in out.columns and "district" in out.columns:
            ...
            cov.to_csv(out_dirs["reports"] / f"demographic_feature_coverage_{mode}_v15.csv", index=False, encoding="utf-8-sig")
    return out, report
...
def clean_sales_and_rentals(
    ...
    sales_clean.to_csv(out_dirs["input"] / "sales_cleaned_v15.csv", index=False, encoding="utf-8-sig")
    rentals_clean.to_csv(out_dirs["input"] / "rentals_cleaned_v15.csv", index=False, encoding="utf-8-sig")
    pd.concat([sales_removed_basic, sales_removed_iqr], ignore_index=True).to_csv(
        out_dirs["input"] / "sales_removed_v15.csv", index=False, encoding="utf-8-sig"
    )
    pd.concat([rentals_removed_basic, rentals_removed_iqr], ignore_index=True).to_csv(
        out_dirs["input"] / "rentals_removed_v15.csv", index=False, encoding="utf-8-sig"
    )
...
def apply_location_outlier_filter(sales: pd.DataFrame, cfg: RunConfig, out_dirs: dict[str, Path]) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    ...
    removed.to_csv(out_dirs["input"] / "sales_removed_location_outliers_v15.csv", index=False, encoding="utf-8-sig")
    clean.to_csv(out_dirs["input"] / "sales_after_location_outlier_filter_v15.csv", index=False, encoding="utf-8-sig")
...
def build_preprocessor(
    ...
    numeric_features = resolve_numeric_features(attribute_mode, detail_effect_mode, basiskele_specialist_mode)
...
def build_pipeline(
    ...
    numeric_features = resolve_numeric_features(mode, detail_mode, specialist_mode)
    steps = [
    ...
    # V15 county specialist: Başiskele premium (deterministic) → large_home redesign → fold-safe target stats
...
def model_specs(target_mode: str, selected_models: list[str] | None = None, fast_mode: bool = False, attribute_mode: str = "full", detail_effect_mode: str = "group", basiskele_specialist_mode: str = "premium_target_stats") -> dict[str, Any]:
...
def train_segment_layer(
    ...
    specs = model_specs(cfg.target_mode, selected_models=cfg.selected_models, fast_mode=cfg.fast_mode, attribute_mode=str(getattr(cfg, "attribute_mode", "full")), detail_effect_mode=str(getattr(cfg, "detail_effect_mode", "group")), basiskele_specialist_mode=str(getattr(cfg, "basiskele_specialist_mode", "premium_target_stats")))
    ...
    for segment_name in segment_names:
        ...
        if use_segment:
            ...
            for model_name, model_obj in selected.items():
                joblib.dump(model_obj, out_dirs["artifacts"] / f"model_segment_{segment_name}_{model_name}_v15.joblib")
        ...
        seg_comparison.to_csv(out_dirs["reports"] / f"model_comparison_segment_{segment_name}_v15.csv", index=False, encoding="utf-8-sig")

    report = pd.DataFrame(report_rows)
    report.to_csv(out_dirs["reports"] / "segment_layer_report_v15.csv", index=False, encoding="utf-8-sig")
    ...
    try:
        ...
        if not lh.empty:
            ...
            ).to_csv(out_dirs["reports"] / "large_home_diagnostics_v15.csv", index=False, encoding="utf-8-sig")
    except Exception as exc:
...
def apply_basiskele_variance_lift_layer(
    ...
    """OOF-safe Başiskele variance-lift skeleton (V15.0 conservative / full).
    ...
    if mode in {"", "none"} or specialist in {"", "none"}:
        ...
        pd.DataFrame([report]).to_csv(
            out_dirs["reports"] / "basiskele_variance_lift_report_v15.csv", index=False, encoding="utf-8-sig"
        )
...
def save_anomaly_reports(anom: pd.DataFrame, reports_dir: Path, output_dir: Path) -> None:
    if anom.empty:
        return
    anom.sort_values("anomaly_score", ascending=False).to_csv(output_dir / "listing_anomaly_scores_v15.csv", index=False, encoding="utf-8-sig")
    anom.sort_values("anomaly_score", ascending=False).head(250).to_csv(reports_dir / "top_listing_anomalies_v15.csv", index=False, encoding="utf-8-sig")
    ...
    def group_anomaly(col: str) -> None:
        ...
        rep.to_csv(reports_dir / f"anomaly_by_{col}_v15.csv", index=False, encoding="utf-8-sig")
...
def train_county_expert_layer(
    ...
    if not cfg.enable_county_experts or "county" not in X.columns:
        ...
        report.to_csv(out_dirs["reports"] / "county_expert_report_v15.csv", index=False, encoding="utf-8-sig")
    ...
    for county_name in counties:
        ...
        comp.to_csv(out_dirs["reports"] / f"model_comparison_county_{safe_county}_v15.csv", index=False, encoding="utf-8-sig")
        ...
        if use_county:
            ...
            for model_name, model_obj in selected.items():
                joblib.dump(model_obj, out_dirs["artifacts"] / f"model_county_{safe_county}_{model_name}_v15.joblib")
    ...
    report = pd.DataFrame(report_rows)
    report.to_csv(out_dirs["reports"] / "county_expert_layer_report_v15.csv", index=False, encoding="utf-8-sig")
    report.to_csv(out_dirs["reports"] / "county_expert_report_v15.csv", index=False, encoding="utf-8-sig")
    return final_pred, used_county_models, all_county_models, county_weights, county_blend_weights, report
...
def save_feature_importance_by_county(county_models: dict[str, dict[str, Any]], reports_dir: Path) -> None:
    ...
    rep.to_csv(reports_dir / "feature_importance_by_county_v15.csv", index=False, encoding="utf-8-sig")
    top = (
        rep.groupby(["county", "feature"], as_index=False)["importance"]
        .mean()
        .sort_values(["county", "importance"], ascending=[True, False])
    )
    top.groupby("county", group_keys=False).head(40).to_csv(reports_dir / "feature_importance_by_county_top40_v15.csv", index=False, encoding="utf-8-sig")
...
def train_and_evaluate(X: pd.DataFrame, y: pd.Series, cfg: RunConfig, out_dirs: dict[str, Path]) -> tuple[ModelBundle, dict[str, Any]]:
    ...
    for name, estimator in specs.items():
        ...
        joblib.dump(final_estimator, out_dirs["artifacts"] / f"model_{name}_v15.joblib")
        _ptick(f"fit {name}")

    model_comparison = pd.DataFrame(comparison_rows).sort_values(["mape", "mae_tl_per_m2"])
    model_comparison.to_csv(out_dirs["reports"] / "model_comparison_v15.csv", index=False, encoding="utf-8-sig")
    ...
    base_metrics["model"] = "ensemble_base_top_models_v15"
    ...
    segment_metrics_final["model"] = "segment_aware_before_county_v15"
    ...
    # Optional Başiskele variance-lift (OOF-safe); may reject itself if no lift.
    ...
    ensemble_metrics["model"] = "county_expert_segment_aware_ensemble_v15"
    ...
    ensemble_metrics["basiskele_variance_lift"] = str(getattr(cfg, "basiskele_variance_lift", "conservative"))
    ...
    joblib.dump(bundle, out_dirs["artifacts"] / "model_bundle_v15.joblib")
    ...
    oof.to_csv(out_dirs["output"] / "oof_predictions_v15.csv", index=False, encoding="utf-8-sig")

    county_metrics = county_metric_report(oof, pred_col="pred_ensemble")
    if not county_metrics.empty:
        county_metrics.to_csv(out_dirs["reports"] / "county_metrics_v15.csv", index=False, encoding="utf-8-sig")

    anomaly_metric_report = anomaly_metric_diagnostics(oof)
    if not anomaly_metric_report.empty:
        anomaly_metric_report.to_csv(out_dirs["reports"] / "anomaly_metric_diagnostics_v15.csv", index=False, encoding="utf-8-sig")
    ...
    # V15 attribute + detail premium diagnostics
    try:
        ...
        decision["selected_basiskele_variance_lift_mode"] = str(getattr(cfg, "basiskele_variance_lift", "conservative"))
        ...
        try:
            ...
            pd.DataFrame(
                ...
                out_dirs["reports"] / "basiskele_premium_specialist_diagnostics_v15.csv",
                index=False,
                encoding="utf-8-sig",
            )
        ...
        warnings.warn(f"V15 sensitivity diagnostics failed: {exc}")
        metrics_summary["decision"] = {"overall": "UNKNOWN", "error": str(exc)}

    if not segment_report.empty:
        segment_report.to_csv(out_dirs["reports"] / "segment_layer_report_v15.csv", index=False, encoding="utf-8-sig")
    if not county_report.empty:
        county_report.to_csv(out_dirs["reports"] / "county_expert_layer_report_v15.csv", index=False, encoding="utf-8-sig")

    (out_dirs["reports"] / "metrics_summary_v15.json").write_text(
...
def save_group_error_reports(oof: pd.DataFrame, reports_dir: Path) -> None:
    ...
    for col in group_cols:
        ...
        rep.to_csv(reports_dir / f"error_by_{col}_v15.csv", index=False, encoding="utf-8-sig")
...
def save_plots(oof: pd.DataFrame, reports_dir: Path) -> None:
    ...
    plt.tight_layout()
    plt.savefig(reports_dir / "actual_vs_predicted_v15.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(reports_dir / "residual_distribution_v15.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    if "district" in oof.columns:
        ...
        if not rep.empty:
            ...
            plt.tight_layout()
            plt.savefig(reports_dir / "mape_by_district_v15.png", dpi=300, bbox_inches="tight")
            plt.close()
...
def save_feature_importance(models: dict[str, Any], reports_dir: Path) -> None:
    ...
    rep.to_csv(reports_dir / "feature_importance_v15.csv", index=False, encoding="utf-8-sig")
    top = rep.groupby("feature", as_index=False)["importance"].mean().sort_values("importance", ascending=False).head(50)
    top.to_csv(reports_dir / "feature_importance_top50_v15.csv", index=False, encoding="utf-8-sig")
...
def write_run_readme(out_dirs: dict[str, Path], cfg: RunConfig, metrics_summary: dict[str, Any], cleaning_report: dict[str, Any], feature_reports: dict[str, Any]) -> None:
    ...
    (out_dirs["base"] / "README_v15_run.md").write_text("\n".join(lines), encoding="utf-8")
...
def parse_args() -> argparse.Namespace:
    ...
    ap = argparse.ArgumentParser(
        description="V15 attribute-sensitivity training pipeline. "
    ...
    ap.add_argument("--out", default="outputs/v15_full")
    ...
    ap.add_argument(
        ...
        help="Optional OOF-safe Başiskele variance-lift layer.",
    )
    ap.add_argument(
    ...
    if bool(getattr(args, "interactive", True)):
        try:
            ...
            from v15.interactive_cli import apply_interactive_prompts
...
def main() -> None:
    ...
    cfg = RunConfig(
        ...
        basiskele_variance_lift=str(getattr(args, "basiskele_variance_lift", "conservative")),
        large_home_specialist_mode=str(getattr(args, "large_home_specialist_mode", "redesigned")),
    )

    out_dirs = build_output_dirs(args.out)
    (out_dirs["base"] / "run_config_v15.json").write_text(json.dumps(asdict(cfg), indent=2, ensure_ascii=False), encoding="utf-8")
    ...
    def run_one_mode(
        ...
        vmode = str(variance_lift if variance_lift is not None else getattr(cfg, "basiskele_variance_lift", "conservative"))
        mode_cfg = RunConfig(
            **{
                **asdict(cfg),
        ...
        sales_with_demo.to_csv(mode_out_dirs["input"] / f"sales_training_table_v15_{demo_mode}_{attr_mode}.csv", index=False, encoding="utf-8-sig")
        ...
        if mode_cfg.exclude_anomalies_threshold and mode_cfg.exclude_anomalies_threshold > 0:
            ...
            pre_anom.assign(training_excluded_by_threshold=(~keep_mask).astype(int)).to_csv(
                mode_out_dirs["reports"] / f"anomaly_training_filter_{demo_mode}_{attr_mode}_v15.csv", index=False, encoding="utf-8-sig"
            )
            if excluded > 0:
        ...
        (mode_out_dirs["reports"] / "metrics_summary_v15.json").write_text(
            json.dumps(metrics_summary, indent=2, ensure_ascii=False), encoding="utf-8"
        )
    ...
    total_units = estimate_training_units(
        ...
        n_detail_runs=n_detail_runs + n_specialist_runs,
        include_final=True,
        n_segments=4,
        n_counties_est=4,
    )
    ...
    try:
        ...
        if bool(args.run_attribute_ablation):
            for attr_mode in ["none", "basic", "full"]:
                ...
                _plog(f"\n========== V15 ATTRIBUTE ABLATION: demo={selected_demo} attr={attr_mode} ==========")
            ...
            attr_df.to_csv(out_dirs["reports"] / "metrics_attribute_ablation_v15.csv", index=False, encoding="utf-8-sig")
        ...
        if bool(args.run_demographics_ablation):
            for demo_mode in ["none", "safe", "full"]:
                ...
                _plog(f"\n========== V15 DEMOGRAPHICS ABLATION: demo={demo_mode} attr={selected_attr} ==========")
            ...
            if not demo_df.empty:
                demo_df[keep_cols].to_csv(out_dirs["reports"] / "metrics_demographics_ablation_v15.csv", index=False, encoding="utf-8-sig")
        ...
        if bool(getattr(args, "run_detail_effect_ablation", False)):
            for detail_mode in ["none", "group", "full"]:
                ...
                _plog(
                    f"\n========== V15 DETAIL-EFFECT ABLATION: demo={selected_demo} attr={selected_attr} detail={detail_mode} =========="
                )
            ...
            detail_df.to_csv(out_dirs["reports"] / "metrics_detail_effect_ablation_v15.csv", index=False, encoding="utf-8-sig")
        ...
        if bool(getattr(args, "run_basiskele_specialist_ablation", False)):
            for smode in ["none", "premium", "premium_target_stats", "premium_target_stats_variance_lift"]:
                ...
                _plog(
                    f"\n========== V15 BASISKELE SPECIALIST ABLATION: specialist={smode} =========="
                )
            ...
            pd.DataFrame(specialist_ablation_rows).to_csv(
                out_dirs["reports"] / "metrics_basiskele_specialist_ablation_v15.csv",
                index=False,
                encoding="utf-8-sig",
            )
        ...
        _plog(
            f"\n========== V15 FINAL: demo={selected_demo} attr={selected_attr} detail={selected_detail} "
