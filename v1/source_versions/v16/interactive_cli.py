...
def apply_interactive_prompts(args: Any, argv: list[str] | None = None) -> Any:
    ...
    print("\n════════════════════════════════════════")
    print(" V16 etkileşimli ayar sihirbazı")
    ...
    if not cli_flag_provided(argv, "--karamursel-baseline-mode"):
        cur = str(getattr(args, "karamursel_baseline_mode", "location_age"))
        opts = [
            ChoiceOption("none", "none", "Karamürsel location-age baseline yok."),
            ChoiceOption("location_age", "location_age", "Fold-safe district×age residual medians (önerilen)."),
        ]
        default_i = next((i for i, o in enumerate(opts) if o.value == cur), 1)
        args.karamursel_baseline_mode = ask_choice(
            "Karamürsel baseline (--karamursel-baseline-mode)",
            "Aggressive expert yerine düzenli baseline feature.",
            opts,
            default_i,
        )
    ...
    print(f"  basiskele-large-home-regime={getattr(args, 'basiskele_large_home_regime', 'simple')}")
    print(f"  basiskele-spread-layer={getattr(args, 'basiskele_spread_layer', 'conservative')}")
    print(f"  karamursel-baseline-mode={getattr(args, 'karamursel_baseline_mode', 'location_age')}")
...
def build_early_arg_parser() -> Any:
    ...
    ap.add_argument("--basiskele-large-home-regime", choices=["none", "simple", "residual"], default="simple")
    ap.add_argument("--basiskele-spread-layer", choices=["none", "conservative", "full"], default="conservative")
    ap.add_argument("--karamursel-baseline-mode", choices=["none", "location_age"], default="location_age")
