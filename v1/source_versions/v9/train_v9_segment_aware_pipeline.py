...
"""
V9 DB -> clean -> residual-target train -> report pipeline for Kocaeli housing unit price model.
...
Example:
  python train_v9_db_residual_pipeline.py ^
...
class ModelBundle:
    ...
    V9 supports an optional segment-aware layer. The base ensemble always predicts first.
...
def train_segment_layer(
    ...
    The base ensemble remains the fallback for every row. A segment model is used only if its
    OOF MAPE is better than the base ensemble inside the same segment. This avoids forcing a
    weak segment model into production.
    """
    ...
    for segment_name in segment_names:
        ...
        base_metrics = metric_dict(y.loc[mask], base_oof.loc[mask, "pred_ensemble_base"])
        segment_metrics = metric_dict(ys, seg_pred)
        use_segment = bool(segment_metrics["mape"] < base_metrics["mape"])

        blended_pred = (1.0 - blend_weight) * base_oof.loc[mask, "pred_ensemble_base"].to_numpy(dtype=float) + blend_weight * seg_pred
        blended_metrics = metric_dict(y.loc[mask], blended_pred)

        if use_segment:
            final_pred.loc[mask] = blended_pred
            selected = {name: fitted[name] for name in weights}
            segment_models[segment_name] = selected
            segment_weights[segment_name] = weights
            segment_blend_weights[segment_name] = blend_weight
            for model_name, model_obj in selected.items():
                joblib.dump(model_obj, out_dirs["artifacts"] / f"model_segment_{segment_name}_{model_name}_v9.joblib")

        row = {
            "segment": segment_name,
            "rows": n,
            "status": "used" if use_segment else "kept_base",
            "blend_weight": blend_weight if use_segment else 0.0,
            ...
            "blended_log_r2": blended_metrics["log_r2"],
            "blended_mape": blended_metrics["mape"],
            "blended_mae_tl_per_m2": blended_metrics["mae_tl_per_m2"],
            "weights": json.dumps(weights, ensure_ascii=False),
        }
        report_rows.append(row)

        seg_comparison.insert(0, "segment", segment_name)
        seg_comparison.to_csv(out_dirs["reports"] / f"model_comparison_segment_{segment_name}_v9.csv", index=False, encoding="utf-8-sig")

    report = pd.DataFrame(report_rows)
    report.to_csv(out_dirs["reports"] / "segment_layer_report_v9.csv", index=False, encoding="utf-8-sig")
...
def train_and_evaluate(X: pd.DataFrame, y: pd.Series, cfg: RunConfig, out_dirs: dict[str, Path]) -> tuple[ModelBundle, dict[str, Any]]:
    ...
    for name, estimator in specs.items():
        ...
        joblib.dump(final_estimator, out_dirs["artifacts"] / f"model_{name}_v9.joblib")

    model_comparison = pd.DataFrame(comparison_rows).sort_values(["mape", "mae_tl_per_m2"])
    model_comparison.to_csv(out_dirs["reports"] / "model_comparison_v9.csv", index=False, encoding="utf-8-sig")
    ...
    base_metrics["model"] = "ensemble_base_top_models_v9"
    ...
    ensemble_metrics["model"] = "segment_aware_ensemble_v9"
    ...
    joblib.dump(bundle, out_dirs["artifacts"] / "segment_aware_model_bundle_v9.joblib")
    ...
    oof.to_csv(out_dirs["output"] / "oof_predictions_v9.csv", index=False, encoding="utf-8-sig")

    county_metrics = county_metric_report(oof, pred_col="pred_ensemble")
    if not county_metrics.empty:
        county_metrics.to_csv(out_dirs["reports"] / "county_metrics_v9.csv", index=False, encoding="utf-8-sig")
    ...
    (out_dirs["reports"] / "metrics_summary_v9.json").write_text(
...
def save_group_error_reports(oof: pd.DataFrame, reports_dir: Path) -> None:
    ...
    for col in group_cols:
        ...
        rep.to_csv(reports_dir / f"error_by_{col}_v9.csv", index=False, encoding="utf-8-sig")
...
def save_plots(oof: pd.DataFrame, reports_dir: Path) -> None:
    ...
    plt.tight_layout()
    plt.savefig(reports_dir / "actual_vs_predicted_v9.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(reports_dir / "residual_distribution_v9.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    if "district" in oof.columns:
        ...
        if not rep.empty:
            ...
            plt.tight_layout()
            plt.savefig(reports_dir / "mape_by_district_v9.png", dpi=300, bbox_inches="tight")
            plt.close()
...
def save_feature_importance(models: dict[str, Any], reports_dir: Path) -> None:
    ...
    rep.to_csv(reports_dir / "feature_importance_v9.csv", index=False, encoding="utf-8-sig")
