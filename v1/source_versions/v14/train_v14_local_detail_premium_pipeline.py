...
"""
V14 local detail premium pipeline for Kocaeli housing unit price model.

Built on V13 (attribute sensitivity + demographics + segment + county experts).
Adds fold-safe LocalDetailPremiumEncoder on listing detail binaries.

Example:
  python train_v14_local_detail_premium_pipeline.py --out outputs/v14_full --demographics-mode safe --attribute-mode full --detail-effect-mode group
"""
...
if __name__ == "__main__":
    _v14_dir = str(Path(__file__).resolve().parent)
    if _v14_dir not in sys.path:
        sys.path.insert(0, _v14_dir)
    sys.stdout.write("V14 ayar sihirbazı açılıyor...\n")
    sys.stdout.flush()
...
try:
    ...
    from v14.attribute_features import (
        ATTRIBUTE_BASIC_NUMERIC_FEATURES,
        ATTRIBUTE_EFFECT_NUMERIC_FEATURES,
        AttributeEffectEncoder,
        AttributeInteractionCompleter,
...
try:
    ...
    from v14.diagnostics_v14 import (
        attribute_feature_coverage,
        evaluate_decision,
        run_basiskele_variance_diagnostics,
        run_karamursel_sensitivity,
...
try:
    ...
    from v14.detail_premium_features import (
        DETAIL_EFFECT_GROUP_NUMERIC_FEATURES,
        V13_DEFAULT_REF,
        LocalDetailPremiumEncoder,
        detail_feature_coverage,
...
try:
    ...
    from v14.train_progress import TrainProgress, estimate_training_units, get_progress, set_progress
...
try:
    ...
    # Prefer local v14/.env next to this file (cwd-independent).
...
def attach_demographic_features(listings: pd.DataFrame, demo_features: pd.DataFrame, mode: str, out_dirs: dict[str, Path] | None = None) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if out_dirs is not None:
        ...
        if present and "county" in out.columns and "district" in out.columns:
            ...
            cov.to_csv(out_dirs["reports"] / f"demographic_feature_coverage_{mode}_v14.csv", index=False, encoding="utf-8-sig")
    return out, report
...
def clean_sales_and_rentals(
    ...
    sales_clean.to_csv(out_dirs["input"] / "sales_cleaned_v14.csv", index=False, encoding="utf-8-sig")
    rentals_clean.to_csv(out_dirs["input"] / "rentals_cleaned_v14.csv", index=False, encoding="utf-8-sig")
    pd.concat([sales_removed_basic, sales_removed_iqr], ignore_index=True).to_csv(
        out_dirs["input"] / "sales_removed_v14.csv", index=False, encoding="utf-8-sig"
    )
    pd.concat([rentals_removed_basic, rentals_removed_iqr], ignore_index=True).to_csv(
        out_dirs["input"] / "rentals_removed_v14.csv", index=False, encoding="utf-8-sig"
    )
...
def apply_location_outlier_filter(sales: pd.DataFrame, cfg: RunConfig, out_dirs: dict[str, Path]) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    ...
    removed.to_csv(out_dirs["input"] / "sales_removed_location_outliers_v14.csv", index=False, encoding="utf-8-sig")
    clean.to_csv(out_dirs["input"] / "sales_after_location_outlier_filter_v14.csv", index=False, encoding="utf-8-sig")
...
def resolve_numeric_features(attribute_mode: str = "full", detail_effect_mode: str = "group") -> list[str]:
    return (
...
def build_preprocessor(
    ...
    numeric_features = resolve_numeric_features(attribute_mode, detail_effect_mode)
...
def build_pipeline(
    ...
    numeric_features = resolve_numeric_features(mode, detail_mode)
    steps = [
...
def model_specs(target_mode: str, selected_models: list[str] | None = None, fast_mode: bool = False, attribute_mode: str = "full", detail_effect_mode: str = "group") -> dict[str, Any]:
...
def train_segment_layer(
    ...
    specs = model_specs(cfg.target_mode, selected_models=cfg.selected_models, fast_mode=cfg.fast_mode, attribute_mode=str(getattr(cfg, "attribute_mode", "full")), detail_effect_mode=str(getattr(cfg, "detail_effect_mode", "group")))
    ...
    for segment_name in segment_names:
        ...
        if use_segment:
            ...
            for model_name, model_obj in selected.items():
                joblib.dump(model_obj, out_dirs["artifacts"] / f"model_segment_{segment_name}_{model_name}_v14.joblib")
        ...
        seg_comparison.to_csv(out_dirs["reports"] / f"model_comparison_segment_{segment_name}_v14.csv", index=False, encoding="utf-8-sig")

    report = pd.DataFrame(report_rows)
    report.to_csv(out_dirs["reports"] / "segment_layer_report_v14.csv", index=False, encoding="utf-8-sig")
...
def save_anomaly_reports(anom: pd.DataFrame, reports_dir: Path, output_dir: Path) -> None:
    if anom.empty:
        return
    anom.sort_values("anomaly_score", ascending=False).to_csv(output_dir / "listing_anomaly_scores_v14.csv", index=False, encoding="utf-8-sig")
    anom.sort_values("anomaly_score", ascending=False).head(250).to_csv(reports_dir / "top_listing_anomalies_v14.csv", index=False, encoding="utf-8-sig")
    ...
    def group_anomaly(col: str) -> None:
        ...
        rep.to_csv(reports_dir / f"anomaly_by_{col}_v14.csv", index=False, encoding="utf-8-sig")
...
def train_county_expert_layer(
    ...
    if not cfg.enable_county_experts or "county" not in X.columns:
        ...
        report.to_csv(out_dirs["reports"] / "county_expert_report_v14.csv", index=False, encoding="utf-8-sig")
    ...
    for county_name in counties:
        ...
        comp.to_csv(out_dirs["reports"] / f"model_comparison_county_{safe_county}_v14.csv", index=False, encoding="utf-8-sig")
        ...
        if use_county:
            ...
            for model_name, model_obj in selected.items():
                joblib.dump(model_obj, out_dirs["artifacts"] / f"model_county_{safe_county}_{model_name}_v14.joblib")
...
def save_feature_importance_by_county(county_models: dict[str, dict[str, Any]], reports_dir: Path) -> None:
    ...
    rep.to_csv(reports_dir / "feature_importance_by_county_v14.csv", index=False, encoding="utf-8-sig")
    top = (
        rep.groupby(["county", "feature"], as_index=False)["importance"]
        .mean()
        .sort_values(["county", "importance"], ascending=[True, False])
    )
    top.groupby("county", group_keys=False).head(40).to_csv(reports_dir / "feature_importance_by_county_top40_v14.csv", index=False, encoding="utf-8-sig")
...
def train_and_evaluate(X: pd.DataFrame, y: pd.Series, cfg: RunConfig, out_dirs: dict[str, Path]) -> tuple[ModelBundle, dict[str, Any]]:
    ...
    for name, estimator in specs.items():
        ...
        joblib.dump(final_estimator, out_dirs["artifacts"] / f"model_{name}_v14.joblib")
        _ptick(f"fit {name}")

    model_comparison = pd.DataFrame(comparison_rows).sort_values(["mape", "mae_tl_per_m2"])
    model_comparison.to_csv(out_dirs["reports"] / "model_comparison_v14.csv", index=False, encoding="utf-8-sig")
    ...
    base_metrics["model"] = "ensemble_base_top_models_v14"
    ...
    segment_metrics_final["model"] = "segment_aware_before_county_v14"
    ...
    oof["pred_ensemble"] = oof["pred_after_county"].copy()

    ensemble_metrics = metric_dict(y, oof["pred_ensemble"])
    ensemble_metrics["model"] = "county_expert_segment_aware_ensemble_v14"
    ...
    bundle = ModelBundle(
        ...
        attribute_mode=str(getattr(cfg, "attribute_mode", "full")), detail_effect_mode=str(getattr(cfg, "detail_effect_mode", "group")),
    )
    joblib.dump(bundle, out_dirs["artifacts"] / "model_bundle_v14.joblib")
    ...
    oof.to_csv(out_dirs["output"] / "oof_predictions_v14.csv", index=False, encoding="utf-8-sig")

    county_metrics = county_metric_report(oof, pred_col="pred_ensemble")
    if not county_metrics.empty:
        county_metrics.to_csv(out_dirs["reports"] / "county_metrics_v14.csv", index=False, encoding="utf-8-sig")

    anomaly_metric_report = anomaly_metric_diagnostics(oof)
    if not anomaly_metric_report.empty:
        anomaly_metric_report.to_csv(out_dirs["reports"] / "anomaly_metric_diagnostics_v14.csv", index=False, encoding="utf-8-sig")
    ...
    # V14 attribute + detail premium diagnostics
    try:
        ...
        warnings.warn(f"V14 sensitivity diagnostics failed: {exc}")
        metrics_summary["decision"] = {"overall": "UNKNOWN", "error": str(exc)}

    if not segment_report.empty:
        segment_report.to_csv(out_dirs["reports"] / "segment_layer_report_v14.csv", index=False, encoding="utf-8-sig")
    if not county_report.empty:
        county_report.to_csv(out_dirs["reports"] / "county_expert_layer_report_v14.csv", index=False, encoding="utf-8-sig")

    (out_dirs["reports"] / "metrics_summary_v14.json").write_text(
...
def save_group_error_reports(oof: pd.DataFrame, reports_dir: Path) -> None:
    ...
    for col in group_cols:
        ...
        rep.to_csv(reports_dir / f"error_by_{col}_v14.csv", index=False, encoding="utf-8-sig")
...
def save_plots(oof: pd.DataFrame, reports_dir: Path) -> None:
    ...
    plt.tight_layout()
    plt.savefig(reports_dir / "actual_vs_predicted_v14.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(reports_dir / "residual_distribution_v14.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    if "district" in oof.columns:
        ...
        if not rep.empty:
            ...
            plt.tight_layout()
            plt.savefig(reports_dir / "mape_by_district_v14.png", dpi=300, bbox_inches="tight")
            plt.close()
...
def save_feature_importance(models: dict[str, Any], reports_dir: Path) -> None:
    ...
    rep.to_csv(reports_dir / "feature_importance_v14.csv", index=False, encoding="utf-8-sig")
    top = rep.groupby("feature", as_index=False)["importance"].mean().sort_values("importance", ascending=False).head(50)
    top.to_csv(reports_dir / "feature_importance_top50_v14.csv", index=False, encoding="utf-8-sig")
...
def write_run_readme(out_dirs: dict[str, Path], cfg: RunConfig, metrics_summary: dict[str, Any], cleaning_report: dict[str, Any], feature_reports: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "",
        "### Rent note",
        "V14 trains sale unit-price only. If the app rent path is `district_rent_m2_median * gross_m2`,",
        ...
        "- data/input/sales_cleaned_v14.csv",
        "- data/input/rentals_cleaned_v14.csv",
        "- data/output/oof_predictions_v14.csv",
        "- reports/model_comparison_v14.csv",
        "- reports/metrics_summary_v14.json",
        "- reports/feature_sensitivity_v14.csv",
        "- reports/karamursel_sensitivity_v14.csv",
        "- reports/basiskele_variance_diagnostics_v14.csv",
        "- reports/metrics_attribute_ablation_v14.csv",
        "- reports/error_by_*_v14.csv",
        "- reports/*.png",
        "- artifacts/model_*_v14.joblib",
        "- artifacts/model_bundle_v14.joblib",
        "",
    ]
    (out_dirs["base"] / "README_v14_run.md").write_text("\n".join(lines), encoding="utf-8")
...
def parse_args() -> argparse.Namespace:
    ...
    ap = argparse.ArgumentParser(
        description="V14 attribute-sensitivity training pipeline. "
    ...
    ap.add_argument("--out", default="outputs/v14_full")
    ...
    if bool(getattr(args, "interactive", True)):
        try:
            ...
            from v14.interactive_cli import apply_interactive_prompts
...
def main() -> None:
    ...
    (out_dirs["base"] / "run_config_v14.json").write_text(json.dumps(asdict(cfg), indent=2, ensure_ascii=False), encoding="utf-8")
    ...
    def run_one_mode(demo_mode: str, attr_mode: str, mode_out_dirs: dict[str, Path], detail_mode: str | None = None):
        dmode = str(detail_mode if detail_mode is not None else getattr(cfg, "detail_effect_mode", "group"))
        mode_cfg = RunConfig(**{**asdict(cfg), "demographics_mode": demo_mode, "attribute_mode": attr_mode, "detail_effect_mode": dmode})
        ...
        sales_with_demo.to_csv(mode_out_dirs["input"] / f"sales_training_table_v14_{demo_mode}_{attr_mode}.csv", index=False, encoding="utf-8-sig")

        X, y = make_training_matrix(sales_with_demo)
        _plog(f"[demo={demo_mode} attr={attr_mode}] rows before anomaly filter: {len(X)}")
        if len(X) == 0:
        ...
        if mode_cfg.exclude_anomalies_threshold and mode_cfg.exclude_anomalies_threshold > 0:
            ...
            pre_anom.assign(training_excluded_by_threshold=(~keep_mask).astype(int)).to_csv(
                mode_out_dirs["reports"] / f"anomaly_training_filter_{demo_mode}_{attr_mode}_v14.csv", index=False, encoding="utf-8-sig"
            )
            if excluded > 0:
        ...
        (mode_out_dirs["reports"] / "metrics_summary_v14.json").write_text(
            json.dumps(metrics_summary, indent=2, ensure_ascii=False), encoding="utf-8"
        )
    ...
    try:
        ...
        if bool(args.run_attribute_ablation):
            for attr_mode in ["none", "basic", "full"]:
                ...
                _plog(f"\n========== V14 ATTRIBUTE ABLATION: demo={selected_demo} attr={attr_mode} ==========")
            ...
            attr_df.to_csv(out_dirs["reports"] / "metrics_attribute_ablation_v14.csv", index=False, encoding="utf-8-sig")
        ...
        if bool(args.run_demographics_ablation):
            for demo_mode in ["none", "safe", "full"]:
                ...
                _plog(f"\n========== V14 DEMOGRAPHICS ABLATION: demo={demo_mode} attr={selected_attr} ==========")
            ...
            if not demo_df.empty:
                demo_df[keep_cols].to_csv(out_dirs["reports"] / "metrics_demographics_ablation_v14.csv", index=False, encoding="utf-8-sig")
        ...
        if bool(getattr(args, "run_detail_effect_ablation", False)):
            for detail_mode in ["none", "group", "full"]:
                ...
                _plog(
                    f"\n========== V14 DETAIL-EFFECT ABLATION: demo={selected_demo} attr={selected_attr} detail={detail_mode} =========="
                )
            ...
            detail_df.to_csv(out_dirs["reports"] / "metrics_detail_effect_ablation_v14.csv", index=False, encoding="utf-8-sig")
        ...
        _pstage(f"FINAL demo={selected_demo} attr={selected_attr} detail={selected_detail}")
        _plog(
            f"\n========== V14 FINAL: demo={selected_demo} attr={selected_attr} detail={selected_detail} =========="
        )
        final_cfg_dict = {
            **asdict(cfg),
        ...
        bundle, metrics_summary, demo_feature_report, anomaly_filter_report = run_one_mode(
            selected_demo, selected_attr, out_dirs, detail_mode=selected_detail
        )
