"""V14 fold-safe local detail premium features.
...
# V13 default reference (V14 decision / guardrails)
V13_DEFAULT_REF = {
    "r2": 0.6800,
    "mape": 0.1279,
    "basiskele_r2": 0.4449,
...
def detail_feature_coverage(X: pd.DataFrame, reports_dir=None, unreliable_min_pos: int = 10) -> pd.DataFrame:
    ...
    if reports_dir is not None and not rep.empty:
        rep.to_csv(reports_dir / "detail_feature_coverage_v14.csv", index=False, encoding="utf-8-sig")
    return rep
...
def export_detail_premium_effect_tables(encoder: LocalDetailPremiumEncoder, reports_dir) -> dict[str, pd.DataFrame]:
    ...
    if not by_county.empty:
        by_county.to_csv(reports_dir / "detail_premium_effects_by_county_v14.csv", index=False, encoding="utf-8-sig")
    ...
    if not bas.empty:
        bas.to_csv(reports_dir / "basiskele_detail_premium_diagnostics_v14.csv", index=False, encoding="utf-8-sig")
    ...
    if not group_sum.empty:
        group_sum.to_csv(reports_dir / "detail_premium_group_summary_v14.csv", index=False, encoding="utf-8-sig")
