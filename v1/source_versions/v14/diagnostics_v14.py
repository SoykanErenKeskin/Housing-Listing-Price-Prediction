"""V14 sensitivity / coverage / decision diagnostics."""
...
try:
    from detail_premium_features import V13_DEFAULT_REF
except ImportError:
    from v14.detail_premium_features import V13_DEFAULT_REF
...
def run_prediction_pair_tests(bundle: Any, X: pd.DataFrame, reports_dir) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    pair.to_csv(reports_dir / "prediction_pair_tests_v14.csv", index=False, encoding="utf-8-sig")
    pair.to_csv(reports_dir / "feature_sensitivity_v14.csv", index=False, encoding="utf-8-sig")
    county = (
    ...
    county.to_csv(reports_dir / "county_feature_sensitivity_v14.csv", index=False, encoding="utf-8-sig")
...
def run_karamursel_sensitivity(bundle: Any, X: pd.DataFrame, reports_dir) -> dict[str, Any]:
    ...
    rep.to_csv(reports_dir / "karamursel_sensitivity_v14.csv", index=False, encoding="utf-8-sig")
...
def run_basiskele_variance_diagnostics(
    ...
    pd.DataFrame([{k: v for k, v in row.items() if k != "decile_bias_json"}]).to_csv(
        reports_dir / "basiskele_variance_diagnostics_v14.csv", index=False, encoding="utf-8-sig"
    )
    return row
...
def attribute_feature_coverage(X: pd.DataFrame, reports_dir) -> pd.DataFrame:
    ...
    rep.to_csv(reports_dir / "attribute_feature_coverage_v14.csv", index=False, encoding="utf-8-sig")
    return rep
...
def save_attribute_feature_importance(models: dict[str, Any], reports_dir) -> None:
    ...
    rep.to_csv(reports_dir / "attribute_feature_importance_v14.csv", index=False, encoding="utf-8-sig")
...
def save_detail_premium_feature_importance(models: dict[str, Any], reports_dir, county: str | None = None) -> None:
    ...
    rep = pd.DataFrame(rows).sort_values("importance", ascending=False)
    rep["rank"] = range(1, len(rep) + 1)
    rep.to_csv(reports_dir / "detail_premium_feature_importance_v14.csv", index=False, encoding="utf-8-sig")
...
def evaluate_decision(
    ...
    """V14 go/no-go vs V13 defaults (plus retained V12 sensitivity checks)."""
    r2 = float(ensemble_metrics.get("r2", np.nan))
    mape = float(ensemble_metrics.get("mape", np.nan))
    pass_global_guardrail = (mape <= V13_DEFAULT_REF["mape"] + 0.005) and (r2 >= V13_DEFAULT_REF["r2"] - 0.01)
    # keep V12-based alias for attribute ablation compatibility
    ...
    pass_basiskele_lift = bool(pd.notna(basiskele_r2) and basiskele_r2 > float(V13_DEFAULT_REF["basiskele_r2"]))
    if pd.notna(basiskele_var):
        # soft: variance should not collapse further; lift preferred but R2 is required
        pass
    karamursel_r2 = _county_r2("Karamürsel")
    if not pd.notna(karamursel_r2):
        karamursel_r2 = float("nan")
    pass_karamursel_guardrail = (not pd.notna(karamursel_r2)) or (
        karamursel_r2 >= float(V13_DEFAULT_REF["karamursel_r2"]) - 1e-9
    )
    ...
    if not pass_global_guardrail:
        ...
        qa.append({"finding": "Global MAPE/R2 outside V13 tolerance", "severity": "High"})
        top_risks.append("Global metrics outside V13 guardrail")
    if not pass_basiskele_lift:
        warnings.append("basiskele_no_r2_lift")
        qa.append(
            {
                "finding": f"Başiskele R2 {basiskele_r2:.4f} not above V13 {V13_DEFAULT_REF['basiskele_r2']}",
                "severity": "Medium",
            }
        )
    else:
    ...
    if not pass_karamursel_guardrail:
        warnings.append("karamursel_r2_regression")
        qa.append(
            {
                "finding": f"Karamürsel R2 {karamursel_r2:.4f} below V13 {V13_DEFAULT_REF['karamursel_r2']}",
                "severity": "Medium",
            }
        )
        top_risks.append("Karamürsel R2 regression vs V13")
    ...
    ship_ready = all(pd.notna(v) and float(v) >= 0.65 for v in county_r2s.values())
    if not ship_ready:
        qa.append(
            {
                "finding": "not ship-ready until all counties R2 >= 0.65",
                "severity": "Info",
                "county_r2": county_r2s,
            }
        )

    # Overall PASS: global guardrail + sensitivity; county lifts are reported separately
    ...
    return {
        ...
        "basiskele_lift": {
            "r2": basiskele_r2,
            "r2_delta_vs_v13": basiskele_r2 - float(V13_DEFAULT_REF["basiskele_r2"]),
