"""Interactive arrow-key CLI prompts for V13 training.
...
def apply_interactive_prompts(args: Any, argv: list[str] | None = None) -> Any:
    ...
    print("\n════════════════════════════════════════")
    print(" V13 etkileşimli ayar sihirbazı")
    ...
    if not cli_flag_provided(argv, "--out"):
        args.out = ask_text(
            ...
            str(getattr(args, "out", "outputs/v13_full")),
        )
    ...
    if not cli_flag_provided(argv, "--county-expert-min-rows"):
        ...
        opts = [
            ChoiceOption(250, "250 (default)", "Karamürsel genelde expert almaz; V13.0 ana run."),
