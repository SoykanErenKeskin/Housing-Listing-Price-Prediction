...
"""
V13 attribute-sensitivity pipeline for Kocaeli housing unit price model.

Built on V12 (demographics + segment + county experts) without price-tier correction.
Adds deterministic attr_* quality features and fold-safe attr_effect_* premiums.

Example:
  python train_v13_attribute_sensitivity_pipeline.py --out outputs/v13_full --demographics-mode safe --attribute-mode full --run-attribute-ablation
"""
...
if __name__ == "__main__":
    _v13_dir = str(Path(__file__).resolve().parent)
    if _v13_dir not in sys.path:
        sys.path.insert(0, _v13_dir)
    sys.stdout.write("V13 ayar sihirbazı açılıyor...\n")
    sys.stdout.flush()
...
try:
    ...
    from v13.attribute_features import (
        ATTRIBUTE_BASIC_NUMERIC_FEATURES,
        ATTRIBUTE_EFFECT_NUMERIC_FEATURES,
        AttributeEffectEncoder,
        AttributeInteractionCompleter,
...
try:
    ...
    from v13.diagnostics_v13 import (
        attribute_feature_coverage,
        evaluate_decision,
        run_basiskele_variance_diagnostics,
        run_karamursel_sensitivity,
...
try:
    ...
    from v13.train_progress import TrainProgress, estimate_training_units, get_progress, set_progress
...
def attach_demographic_features(listings: pd.DataFrame, demo_features: pd.DataFrame, mode: str, out_dirs: dict[str, Path] | None = None) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if out_dirs is not None:
        ...
        if present and "county" in out.columns and "district" in out.columns:
            ...
            cov.to_csv(out_dirs["reports"] / f"demographic_feature_coverage_{mode}_v13.csv", index=False, encoding="utf-8-sig")
    return out, report
...
def clean_sales_and_rentals(
    ...
    sales_clean.to_csv(out_dirs["input"] / "sales_cleaned_v13.csv", index=False, encoding="utf-8-sig")
    rentals_clean.to_csv(out_dirs["input"] / "rentals_cleaned_v13.csv", index=False, encoding="utf-8-sig")
    pd.concat([sales_removed_basic, sales_removed_iqr], ignore_index=True).to_csv(
        out_dirs["input"] / "sales_removed_v13.csv", index=False, encoding="utf-8-sig"
    )
    pd.concat([rentals_removed_basic, rentals_removed_iqr], ignore_index=True).to_csv(
        out_dirs["input"] / "rentals_removed_v13.csv", index=False, encoding="utf-8-sig"
    )
...
def apply_location_outlier_filter(sales: pd.DataFrame, cfg: RunConfig, out_dirs: dict[str, Path]) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    ...
    removed.to_csv(out_dirs["input"] / "sales_removed_location_outliers_v13.csv", index=False, encoding="utf-8-sig")
    clean.to_csv(out_dirs["input"] / "sales_after_location_outlier_filter_v13.csv", index=False, encoding="utf-8-sig")
...
def resolve_numeric_features(attribute_mode: str = "full") -> list[str]:
    return list(NUMERIC_FEATURES) + get_attribute_feature_names(attribute_mode)


def build_preprocessor(scale_numeric: bool = True, attribute_mode: str = "full") -> ColumnTransformer:
    # keep_empty_features prevents fold/segment-specific all-missing columns from being dropped.
    numeric_features = resolve_numeric_features(attribute_mode)
...
def build_pipeline(model: Any, scale_numeric: bool, target_mode: str, attribute_mode: str = "full") -> Any:
    mode = str(attribute_mode or "full").lower()
    numeric_features = resolve_numeric_features(mode)
    steps = [
    ...
    steps.extend(
        [
            ...
            ("preprocess", build_preprocessor(scale_numeric=scale_numeric, attribute_mode=mode)),
            ("model", model),
        ]
    )
...
def model_specs(target_mode: str, selected_models: list[str] | None = None, fast_mode: bool = False, attribute_mode: str = "full") -> dict[str, Any]:
...
def train_segment_layer(
    ...
    specs = model_specs(cfg.target_mode, selected_models=cfg.selected_models, fast_mode=cfg.fast_mode, attribute_mode=str(getattr(cfg, "attribute_mode", "full")))
    ...
    for segment_name in segment_names:
        ...
        if use_segment:
            ...
            for model_name, model_obj in selected.items():
                joblib.dump(model_obj, out_dirs["artifacts"] / f"model_segment_{segment_name}_{model_name}_v13.joblib")
        ...
        seg_comparison.to_csv(out_dirs["reports"] / f"model_comparison_segment_{segment_name}_v13.csv", index=False, encoding="utf-8-sig")

    report = pd.DataFrame(report_rows)
    report.to_csv(out_dirs["reports"] / "segment_layer_report_v13.csv", index=False, encoding="utf-8-sig")
...
def save_anomaly_reports(anom: pd.DataFrame, reports_dir: Path, output_dir: Path) -> None:
    if anom.empty:
        return
    anom.sort_values("anomaly_score", ascending=False).to_csv(output_dir / "listing_anomaly_scores_v13.csv", index=False, encoding="utf-8-sig")
    anom.sort_values("anomaly_score", ascending=False).head(250).to_csv(reports_dir / "top_listing_anomalies_v13.csv", index=False, encoding="utf-8-sig")
    ...
    def group_anomaly(col: str) -> None:
        ...
        rep.to_csv(reports_dir / f"anomaly_by_{col}_v13.csv", index=False, encoding="utf-8-sig")
...
def train_county_expert_layer(
    ...
    if not cfg.enable_county_experts or "county" not in X.columns:
        ...
        report.to_csv(out_dirs["reports"] / "county_expert_report_v13.csv", index=False, encoding="utf-8-sig")
    ...
    for county_name in counties:
        ...
        comp.to_csv(out_dirs["reports"] / f"model_comparison_county_{safe_county}_v13.csv", index=False, encoding="utf-8-sig")
        ...
        if use_county:
            ...
            for model_name, model_obj in selected.items():
                joblib.dump(model_obj, out_dirs["artifacts"] / f"model_county_{safe_county}_{model_name}_v13.joblib")
...
def save_feature_importance_by_county(county_models: dict[str, dict[str, Any]], reports_dir: Path) -> None:
    ...
    rep.to_csv(reports_dir / "feature_importance_by_county_v13.csv", index=False, encoding="utf-8-sig")
    top = (
        rep.groupby(["county", "feature"], as_index=False)["importance"]
        .mean()
        .sort_values(["county", "importance"], ascending=[True, False])
    )
    top.groupby("county", group_keys=False).head(40).to_csv(reports_dir / "feature_importance_by_county_top40_v13.csv", index=False, encoding="utf-8-sig")
...
def train_and_evaluate(X: pd.DataFrame, y: pd.Series, cfg: RunConfig, out_dirs: dict[str, Path]) -> tuple[ModelBundle, dict[str, Any]]:
    ...
    for name, estimator in specs.items():
        ...
        joblib.dump(final_estimator, out_dirs["artifacts"] / f"model_{name}_v13.joblib")
        _ptick(f"fit {name}")

    model_comparison = pd.DataFrame(comparison_rows).sort_values(["mape", "mae_tl_per_m2"])
    model_comparison.to_csv(out_dirs["reports"] / "model_comparison_v13.csv", index=False, encoding="utf-8-sig")
    ...
    base_metrics["model"] = "ensemble_base_top_models_v13"
    ...
    segment_metrics_final["model"] = "segment_aware_before_county_v13"
    ...
    ensemble_metrics["model"] = "county_expert_segment_aware_ensemble_v13"
    ...
    joblib.dump(bundle, out_dirs["artifacts"] / "model_bundle_v13.joblib")
    ...
    oof.to_csv(out_dirs["output"] / "oof_predictions_v13.csv", index=False, encoding="utf-8-sig")

    county_metrics = county_metric_report(oof, pred_col="pred_ensemble")
    if not county_metrics.empty:
        county_metrics.to_csv(out_dirs["reports"] / "county_metrics_v13.csv", index=False, encoding="utf-8-sig")

    anomaly_metric_report = anomaly_metric_diagnostics(oof)
    if not anomaly_metric_report.empty:
        anomaly_metric_report.to_csv(out_dirs["reports"] / "anomaly_metric_diagnostics_v13.csv", index=False, encoding="utf-8-sig")
    ...
    # V13 attribute sensitivity diagnostics
    try:
        ...
        decision = evaluate_decision(ensemble_metrics, karamursel, pair_df, basiskele)
        ...
        warnings.warn(f"V13 sensitivity diagnostics failed: {exc}")
        metrics_summary["decision"] = {"overall": "UNKNOWN", "error": str(exc)}

    if not segment_report.empty:
        segment_report.to_csv(out_dirs["reports"] / "segment_layer_report_v13.csv", index=False, encoding="utf-8-sig")
    if not county_report.empty:
        county_report.to_csv(out_dirs["reports"] / "county_expert_layer_report_v13.csv", index=False, encoding="utf-8-sig")

    (out_dirs["reports"] / "metrics_summary_v13.json").write_text(
...
def save_group_error_reports(oof: pd.DataFrame, reports_dir: Path) -> None:
    ...
    for col in group_cols:
        ...
        rep.to_csv(reports_dir / f"error_by_{col}_v13.csv", index=False, encoding="utf-8-sig")
...
def save_plots(oof: pd.DataFrame, reports_dir: Path) -> None:
    ...
    plt.tight_layout()
    plt.savefig(reports_dir / "actual_vs_predicted_v13.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(reports_dir / "residual_distribution_v13.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    if "district" in oof.columns:
        ...
        if not rep.empty:
            ...
            plt.tight_layout()
            plt.savefig(reports_dir / "mape_by_district_v13.png", dpi=300, bbox_inches="tight")
            plt.close()
...
def save_feature_importance(models: dict[str, Any], reports_dir: Path) -> None:
    ...
    rep.to_csv(reports_dir / "feature_importance_v13.csv", index=False, encoding="utf-8-sig")
    top = rep.groupby("feature", as_index=False)["importance"].mean().sort_values("importance", ascending=False).head(50)
    top.to_csv(reports_dir / "feature_importance_top50_v13.csv", index=False, encoding="utf-8-sig")
...
def write_run_readme(out_dirs: dict[str, Path], cfg: RunConfig, metrics_summary: dict[str, Any], cleaning_report: dict[str, Any], feature_reports: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "",
        "### Rent note",
        "V13 trains sale unit-price only. If the app rent path is `district_rent_m2_median * gross_m2`,",
        ...
        "- data/input/sales_cleaned_v13.csv",
        "- data/input/rentals_cleaned_v13.csv",
        "- data/output/oof_predictions_v13.csv",
        "- reports/model_comparison_v13.csv",
        "- reports/metrics_summary_v13.json",
        "- reports/feature_sensitivity_v13.csv",
        "- reports/karamursel_sensitivity_v13.csv",
        "- reports/basiskele_variance_diagnostics_v13.csv",
        "- reports/metrics_attribute_ablation_v13.csv",
        "- reports/error_by_*_v13.csv",
        "- reports/*.png",
        "- artifacts/model_*_v13.joblib",
        "- artifacts/model_bundle_v13.joblib",
        "",
    ]
    (out_dirs["base"] / "README_v13_run.md").write_text("\n".join(lines), encoding="utf-8")
...
def parse_args() -> argparse.Namespace:
    ...
    ap = argparse.ArgumentParser(
        description="V13 attribute-sensitivity training pipeline. "
    ...
    ap.add_argument("--out", default="outputs/v13_full")
    ...
    if bool(getattr(args, "interactive", True)):
        try:
            ...
            from v13.interactive_cli import apply_interactive_prompts
...
def main() -> None:
    ...
    (out_dirs["base"] / "run_config_v13.json").write_text(json.dumps(asdict(cfg), indent=2, ensure_ascii=False), encoding="utf-8")
    ...
    def run_one_mode(demo_mode: str, attr_mode: str, mode_out_dirs: dict[str, Path]):
        mode_cfg = RunConfig(**{**asdict(cfg), "demographics_mode": demo_mode, "attribute_mode": attr_mode})
        ...
        sales_with_demo.to_csv(mode_out_dirs["input"] / f"sales_training_table_v13_{demo_mode}_{attr_mode}.csv", index=False, encoding="utf-8-sig")
        ...
        if mode_cfg.exclude_anomalies_threshold and mode_cfg.exclude_anomalies_threshold > 0:
            ...
            pre_anom.assign(training_excluded_by_threshold=(~keep_mask).astype(int)).to_csv(
                mode_out_dirs["reports"] / f"anomaly_training_filter_{demo_mode}_{attr_mode}_v13.csv", index=False, encoding="utf-8-sig"
            )
            if excluded > 0:
        ...
        (mode_out_dirs["reports"] / "metrics_summary_v13.json").write_text(
            json.dumps(metrics_summary, indent=2, ensure_ascii=False), encoding="utf-8"
        )
    ...
    try:
        ...
        if bool(args.run_attribute_ablation):
            for attr_mode in ["none", "basic", "full"]:
                _plog(f"\n========== V13 ATTRIBUTE ABLATION: demo={selected_demo} attr={attr_mode} ==========")
                mode_dirs = build_output_dirs(out_dirs["base"] / f"ablation_attr_{attr_mode}")
                _b, ms, _d, _f = run_one_mode(selected_demo, attr_mode, mode_dirs)
            ...
            attr_df.to_csv(out_dirs["reports"] / "metrics_attribute_ablation_v13.csv", index=False, encoding="utf-8-sig")
        ...
        if bool(args.run_demographics_ablation):
            for demo_mode in ["none", "safe", "full"]:
                _plog(f"\n========== V13 DEMOGRAPHICS ABLATION: demo={demo_mode} attr={selected_attr} ==========")
                mode_dirs = build_output_dirs(out_dirs["base"] / f"ablation_{demo_mode}")
                _b, ms, demo_rep, filt_rep = run_one_mode(demo_mode, selected_attr, mode_dirs)
            ...
            if not demo_df.empty:
                demo_df[keep_cols].to_csv(out_dirs["reports"] / "metrics_demographics_ablation_v13.csv", index=False, encoding="utf-8-sig")

        # Final run into main out dirs with selected modes
        _plog(f"\n========== V13 FINAL: demo={selected_demo} attr={selected_attr} ==========")
        final_cfg_dict = {**asdict(cfg), "demographics_mode": selected_demo, "attribute_mode": selected_attr}
        cfg = RunConfig(**final_cfg_dict)
        (out_dirs["base"] / "run_config_v13.json").write_text(json.dumps(asdict(cfg), indent=2, ensure_ascii=False), encoding="utf-8")
        bundle, metrics_summary, demo_feature_report, anomaly_filter_report = run_one_mode(selected_demo, selected_attr, out_dirs)
    finally:
