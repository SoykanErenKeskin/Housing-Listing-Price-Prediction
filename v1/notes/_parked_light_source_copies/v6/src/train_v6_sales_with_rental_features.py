...
INPUT_PATH = ROOT / "data/input/sale_listings.csv"
RENTAL_INPUT_PATH = ROOT / "data/input/rental_listings.csv"
...
def main():
    ...
    coverage_df = pd.DataFrame(detail_coverage_rows)
    if coverage_df.empty:
        coverage_df = pd.DataFrame(
            columns=["feature", "ones", "ratio", "selected_min_ones"]
        )
        warnings.warn(
            "Satış CSV'de front_/view_/near_ vb. binary detay kolonları yok "
            f"({INPUT_PATH}). Sadece detail_* metin alanları varsa model bu bayrakları kullanamaz. "
            "Neon export / raw JSON'dan binary kolonların CSV'ye açılması gerekir.",
            stacklevel=2,
        )
    else:
        coverage_df = coverage_df.sort_values("ones", ascending=False)
    coverage_df.to_csv(
    ...
    comparison.to_csv(REPORTS / "model_comparison_v6_rental.csv", index=False, encoding="utf-8-sig")
    ...
    report = {
        ...
        "note": "V6 includes rental market features plus rare detail feature selection, grouped detail scores, district-detail interactions, tuned models, weighted ensembles, and manual CatBoost CV."
    }
    (REPORTS / "model_metrics_v6_rental.json").write_text(json.dumps(report, indent=2, ensure_ascii=False), encoding="utf-8")
