...
INPUT_PATH = ROOT / "data/input/sales_cleaned.csv"
RENTAL_INPUT_PATH = ROOT / "data/input/rental_cleaned.csv"
...
def expand_raw_json_columns(df):
    """
    Neon exports keep many helper-generated fields inside the `raw` JSONB column.
    Older CSV exports had front_*, view_*, transport_*, near_*, out_*, in_*, subtype_* as normal columns.
    This function restores those columns from raw JSON when they are missing at top level.
    """
    if "raw" not in df.columns:
        return df

    df = df.copy()
    raw_objects = df["raw"].map(safe_json_loads)

    keys_to_expand = set()
    useful_prefixes = DETAIL_PREFIXES
    useful_exact = {
    ...
    for obj in raw_objects:
        for k in obj.keys():
            if k in useful_exact or k.startswith(useful_prefixes):
                keys_to_expand.add(k)

    for k in sorted(keys_to_expand):
        if k not in df.columns:
            df[k] = raw_objects.map(lambda obj, kk=k: obj.get(kk, np.nan))
...
BASE_NUMERIC = [
    ...
    "district_rent_m2_median", "district_rent_m2_mean", "district_rent_m2_count",
    "district_rent_m2_iqr", "district_rent_m2_cv",
    "district_room_rent_m2_median", "district_room_rent_m2_count",
    "district_m2_group_rent_m2_median", "district_m2_group_rent_m2_count",
    "county_rent_m2_median", "county_rent_m2_count",
    "estimated_rent_m2_gross", "estimated_monthly_rent_gross",
...
def m2_group_from_value(x):
    ...
    if not np.isfinite(x):
        return np.nan
    if x <= 75: return "0-75"
    if x <= 100: return "76-100"
    if x <= 125: return "101-125"
    if x <= 150: return "126-150"
    if x <= 200: return "151-200"
    return "200+"

def normalize_key_series(s):
    return s.fillna("missing").astype(str).str.strip()

def prepare_rental_market_data(rental_raw):
    """Prepare rental listings as external market signal. Does not use any sales target."""
    r = rental_raw.copy()
    for c in r.columns:
        if r[c].dtype == "object":
            r[c] = r[c].map(clean_str)

    # raw JSON may contain fixed age fields, but for rent features we mostly need district/room/m2/rent.
    for c in ["monthly_rent", "rent_per_m2_gross", "gross_m2", "net_m2"]:
        if c in r.columns:
            r[c] = r[c].map(to_num)

    if "rent_per_m2_gross" not in r.columns:
        r["rent_per_m2_gross"] = np.nan
    if {"monthly_rent", "gross_m2"}.issubset(r.columns):
        m = r["rent_per_m2_gross"].isna() & r["monthly_rent"].notna() & r["gross_m2"].notna() & (r["gross_m2"] > 0)
        r.loc[m, "rent_per_m2_gross"] = r.loc[m, "monthly_rent"] / r.loc[m, "gross_m2"]

    for c in ["district", "county", "room_count"]:
        if c not in r.columns:
            r[c] = np.nan
        r[c] = normalize_key_series(r[c])

    if "gross_m2" not in r.columns:
        r["gross_m2"] = np.nan
    r["m2_group"] = r["gross_m2"].map(m2_group_from_value)
    r["m2_group"] = normalize_key_series(r["m2_group"])

    valid = (
        r["rent_per_m2_gross"].notna()
        & (r["rent_per_m2_gross"] > 20)
        & (r["rent_per_m2_gross"] < 10000)
        & r["district"].notna()
    )
    return r.loc[valid].copy()

def agg_rent_stats(df, group_cols, prefix):
    if df.empty:
        return pd.DataFrame(columns=group_cols)
    g = df.groupby(group_cols, dropna=False)["rent_per_m2_gross"]
    out = g.agg(
        **{
            f"{prefix}_median": "median",
            f"{prefix}_mean": "mean",
            f"{prefix}_count": "count",
            f"{prefix}_q25": lambda x: np.percentile(x, 25),
            f"{prefix}_q75": lambda x: np.percentile(x, 75),
            f"{prefix}_std": "std",
        }
    ).reset_index()
    ...
    return out.drop(columns=[f"{prefix}_q25", f"{prefix}_q75"], errors="ignore")

def attach_rental_features_to_sales(sales_raw, rental_raw):
    """Merge external rental market aggregates onto sales listings.

    Features are computed only from rental listings, so they are not sales-target leakage.
    """
    sales = sales_raw.copy()
    if rental_raw is None or len(rental_raw) == 0:
        # Make empty columns so downstream model still works.
        for c in [
            "district_rent_m2_median", "district_rent_m2_mean", "district_rent_m2_count",
            "district_rent_m2_iqr", "district_rent_m2_cv", "district_room_rent_m2_median",
            "district_room_rent_m2_count", "district_m2_group_rent_m2_median",
            "district_m2_group_rent_m2_count", "county_rent_m2_median", "county_rent_m2_count",
            "estimated_rent_m2_gross", "estimated_monthly_rent_gross", "rent_feature_confidence"
        ]:
            sales[c] = np.nan
        sales["rent_feature_level"] = "none"
        return sales, {"rental_rows_raw": 0, "rental_rows_used": 0, "note": "No rental file found or empty."}

    r = prepare_rental_market_data(rental_raw)

    for c in ["district", "county", "room_count"]:
        if c not in sales.columns:
            sales[c] = np.nan
        sales[c] = normalize_key_series(sales[c])
    if "gross_m2" in sales.columns:
        sales["_sales_gross_m2_for_rent"] = sales["gross_m2"].map(to_num)
    else:
        sales["_sales_gross_m2_for_rent"] = np.nan
    sales["m2_group"] = sales["_sales_gross_m2_for_rent"].map(m2_group_from_value)
    sales["m2_group"] = normalize_key_series(sales["m2_group"])

    district = agg_rent_stats(r, ["district"], "district_rent_m2")
    district_room = agg_rent_stats(r, ["district", "room_count"], "district_room_rent_m2")
    district_m2 = agg_rent_stats(r, ["district", "m2_group"], "district_m2_group_rent_m2")
    county = agg_rent_stats(r, ["county"], "county_rent_m2")

    sales = sales.merge(district, on="district", how="left")
    sales = sales.merge(district_room[["district", "room_count", "district_room_rent_m2_median", "district_room_rent_m2_count"]], on=["district", "room_count"], how="left")
    sales = sales.merge(district_m2[["district", "m2_group", "district_m2_group_rent_m2_median", "district_m2_group_rent_m2_count"]], on=["district", "m2_group"], how="left")
    sales = sales.merge(county[["county", "county_rent_m2_median", "county_rent_m2_count"]], on="county", how="left")

    # Fallback hierarchy for estimated rent/m2: district+room -> district+m2 -> district -> county.
    sales["estimated_rent_m2_gross"] = sales["district_room_rent_m2_median"]
    level = pd.Series("district_room", index=sales.index, dtype=object)
    m = sales["estimated_rent_m2_gross"].isna()
    sales.loc[m, "estimated_rent_m2_gross"] = sales.loc[m, "district_m2_group_rent_m2_median"]
    level.loc[m & sales["estimated_rent_m2_gross"].notna()] = "district_m2_group"
    m = sales["estimated_rent_m2_gross"].isna()
    sales.loc[m, "estimated_rent_m2_gross"] = sales.loc[m, "district_rent_m2_median"]
    level.loc[m & sales["estimated_rent_m2_gross"].notna()] = "district"
    m = sales["estimated_rent_m2_gross"].isna()
    sales.loc[m, "estimated_rent_m2_gross"] = sales.loc[m, "county_rent_m2_median"]
    level.loc[m & sales["estimated_rent_m2_gross"].notna()] = "county"
    level.loc[sales["estimated_rent_m2_gross"].isna()] = "none"
    sales["rent_feature_level"] = level

    sales["estimated_monthly_rent_gross"] = sales["estimated_rent_m2_gross"] * sales["_sales_gross_m2_for_rent"]

    # Confidence: prefer granular aggregates with enough observations.
    count_for_level = sales["district_room_rent_m2_count"].fillna(0)
    count_for_level = np.where(count_for_level > 0, count_for_level, sales["district_m2_group_rent_m2_count"].fillna(0))
    count_for_level = np.where(count_for_level > 0, count_for_level, sales["district_rent_m2_count"].fillna(0))
    count_for_level = np.where(count_for_level > 0, count_for_level, sales["county_rent_m2_count"].fillna(0))
    sales["rent_feature_confidence"] = np.minimum(np.asarray(count_for_level, dtype=float), 30.0) / 30.0

    report = {
        "rental_rows_raw": int(len(rental_raw)),
        "rental_rows_used": int(len(r)),
        "rental_districts": int(r["district"].nunique()) if len(r) else 0,
        "sales_rows_with_estimated_rent": int(sales["estimated_rent_m2_gross"].notna().sum()),
        "rent_feature_level_counts": sales["rent_feature_level"].value_counts(dropna=False).to_dict(),
        "note": "Rental features are external market aggregates. The sales target is not used to create them."
    }
    return sales.drop(columns=["_sales_gross_m2_for_rent"], errors="ignore"), report
...
def weighted_ensemble_candidates(preds, y):
    ...
    blends = {
        "ensemble_hgb_gb_et_v6_rental": {
            "hist_gradient_boosting_v6_rental": 0.45,
            "gradient_boosting_tuned_r2_v6_rental": 0.35,
            "extra_trees_v6_rental": 0.20
        },
        "ensemble_gb_hgb_v6_rental": {
            "gradient_boosting_tuned_r2_v6_rental": 0.50,
            "hist_gradient_boosting_v6_rental": 0.50
        },
        "ensemble_hgb_et_rf_v6_rental": {
            "hist_gradient_boosting_v6_rental": 0.45,
            "extra_trees_v6_rental": 0.35,
            "random_forest_v6_rental": 0.20
        }
    }
    ...
    if len(top3) >= 2:
        ...
        if grid_best is not None:
            ...
            pred_out["ensemble_grid_best_r2_v6_rental"] = grid_best
            ens_results["ensemble_grid_best_r2_v6_rental"] = grid_best_metric
...
def manual_catboost_cv(X, y, cv, num, cat):
    ...
    }, ARTIFACTS / "catboost_manual_v6_rental.joblib")

    feature_importance = pd.DataFrame({
        "feature": cols,
        "importance": final_model.get_feature_importance()
    }).sort_values("importance", ascending=False)
    feature_importance.to_csv(ARTIFACTS / "catboost_manual_v6_rental_feature_importance.csv", index=False, encoding="utf-8-sig")
...
def main():
    ...
    rental_raw = pd.read_csv(RENTAL_INPUT_PATH) if RENTAL_INPUT_PATH.exists() else pd.DataFrame()
    raw, rental_feature_report = attach_rental_features_to_sales(raw, rental_raw)
    df = prepare_raw(raw)
    # Save rental feature diagnostics before modeling.
    pd.DataFrame([rental_feature_report]).to_csv(REPORTS / "rental_feature_report_v6.csv", index=False, encoding="utf-8-sig")
    rent_cols = [c for c in df.columns if "rent" in c or c == "rent_feature_level"]
    df[[c for c in ["classified_id", "district", "room_count", "gross_m2"] + rent_cols if c in df.columns]].to_csv(
        OUTPUT / "sales_with_rental_features_preview.csv",
        index=False,
        encoding="utf-8-sig"
    )
    ...
    detail_coverage_df = pd.DataFrame(detail_coverage_rows)
    if detail_coverage_df.empty:
        detail_coverage_df = pd.DataFrame(columns=["feature", "ones", "ratio", "selected_min_ones"])
    detail_coverage_df.sort_values("ones", ascending=False).to_csv(
        REPORTS / "detail_feature_coverage_v6_rental.csv",
        index=False,
        encoding="utf-8-sig"
    )
    ...
    models = {
        "ridge_v6_rental": make_model(RidgeCV(alphas=np.logspace(-3, 4, 60)), num, cat, selected_detail_cols),
        "elasticnet_v6_rental": make_model(ElasticNetCV(
            l1_ratio=[.05, .1, .2, .3, .5, .7, .9],
            alphas=np.logspace(-4, 2, 60),
            max_iter=40000,
            random_state=RANDOM_STATE
        ), num, cat, selected_detail_cols),
        "gradient_boosting_v6_rental": make_model(GradientBoostingRegressor(
            random_state=RANDOM_STATE,
            learning_rate=.035,
            n_estimators=800,
            max_depth=2,
        ...
        "hist_gradient_boosting_v6_rental": make_model(HistGradientBoostingRegressor(
            random_state=RANDOM_STATE,
            max_iter=700,
            learning_rate=.03,
            max_leaf_nodes=31,
        ...
        "extra_trees_v6_rental": make_model(ExtraTreesRegressor(
            random_state=RANDOM_STATE,
            n_estimators=700,
            min_samples_leaf=2,
            max_features=.8,
        ...
        "random_forest_v6_rental": make_model(RandomForestRegressor(
            random_state=RANDOM_STATE,
            n_estimators=600,
            min_samples_leaf=3,
            max_features=.8,
    ...
    print("Tuning: gradient_boosting_tuned_r2_v6_rental")
    ...
    try:
        ...
        tuned.fit(X, y)
        results["gradient_boosting_tuned_r2_v6_rental"] = metric_dict(y, tuned_pred)
        results["gradient_boosting_tuned_r2_v6_rental"]["best_params"] = search.best_params_
        preds["gradient_boosting_tuned_r2_v6_rental"] = tuned_pred
        joblib.dump(tuned, ARTIFACTS / "gradient_boosting_tuned_r2_v6_rental.joblib")
        save_explainability(tuned, "gradient_boosting_tuned_r2_v6_rental")
    except Exception as e:
        results["gradient_boosting_tuned_r2_v6_rental"] = {"error": str(e)}

    print("Manual CatBoost CV")
    cb_pred, cb_result = manual_catboost_cv(X, y, cv, num, cat)
    if cb_pred is not None:
        preds["catboost_manual_v6_rental"] = cb_pred
    results["catboost_manual_v6_rental"] = cb_result
    ...
    if best_by_r2:
        ...
        if (ARTIFACTS / f"{best_by_r2}.joblib").exists():
            joblib.dump(joblib.load(ARTIFACTS / f"{best_by_r2}.joblib"), ARTIFACTS / "best_model_v6_rental_by_r2.joblib")
        elif best_by_r2 == "catboost_manual_v6_rental":
            shutil_path = ARTIFACTS / "catboost_manual_v6_rental.joblib"
            if shutil_path.exists():
                joblib.dump(joblib.load(shutil_path), ARTIFACTS / "best_model_v6_rental_by_r2.joblib")
    ...
    comparison.to_csv(REPORTS / "model_comparison_v6_1_rental.csv", index=False, encoding="utf-8-sig")
    ...
    (REPORTS / "model_aux_v6_rental.json").write_text(json.dumps(aux, indent=2, ensure_ascii=False), encoding="utf-8")

    report = {
        "target": TARGET,
        "rows_raw": int(len(raw)),
        "rows_used": int(len(df)),
        "rental_feature_report": rental_feature_report,
        "detail_selection": {
        ...
        "note": "V6.1 includes rental market features plus rare detail feature selection, grouped detail scores, district-detail interactions, tuned models, weighted ensembles, and manual CatBoost CV."
    }
    (REPORTS / "model_metrics_v6_1_rental.json").write_text(json.dumps(report, indent=2, ensure_ascii=False), encoding="utf-8")
