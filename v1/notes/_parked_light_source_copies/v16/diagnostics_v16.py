"""V16 sensitivity / coverage / decision diagnostics."""
...
try:
    ...
    from v16.detail_premium_features import V13_DEFAULT_REF, V14_DEFAULT_REF, V15_DEFAULT_REF
...
def run_prediction_pair_tests(bundle: Any, X: pd.DataFrame, reports_dir) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    pair.to_csv(reports_dir / "prediction_pair_tests_v16.csv", index=False, encoding="utf-8-sig")
    pair.to_csv(reports_dir / "feature_sensitivity_v16.csv", index=False, encoding="utf-8-sig")
    county = (
    ...
    county.to_csv(reports_dir / "county_feature_sensitivity_v16.csv", index=False, encoding="utf-8-sig")
...
def run_karamursel_sensitivity(bundle: Any, X: pd.DataFrame, reports_dir) -> dict[str, Any]:
    ...
    rep.to_csv(reports_dir / "karamursel_sensitivity_v16.csv", index=False, encoding="utf-8-sig")
...
def run_basiskele_variance_diagnostics(
    ...
    pd.DataFrame([{k: v for k, v in row.items() if k != "decile_bias_json"}]).to_csv(
        reports_dir / "basiskele_variance_diagnostics_v16.csv", index=False, encoding="utf-8-sig"
    )
    return row
...
def attribute_feature_coverage(X: pd.DataFrame, reports_dir) -> pd.DataFrame:
    ...
    rep.to_csv(reports_dir / "attribute_feature_coverage_v16.csv", index=False, encoding="utf-8-sig")
    return rep
...
def save_attribute_feature_importance(models: dict[str, Any], reports_dir) -> None:
    ...
    rep.to_csv(reports_dir / "attribute_feature_importance_v16.csv", index=False, encoding="utf-8-sig")
...
def save_detail_premium_feature_importance(models: dict[str, Any], reports_dir, county: str | None = None) -> None:
    ...
    rep = pd.DataFrame(rows).sort_values("importance", ascending=False)
    rep["rank"] = range(1, len(rep) + 1)
    rep.to_csv(reports_dir / "detail_premium_feature_importance_v16.csv", index=False, encoding="utf-8-sig")
...
def evaluate_decision(
    ...
    """V16 go/no-go vs V15 full defaults (V13/V14 retained for compatibility)."""
    r2 = float(ensemble_metrics.get("r2", np.nan))
    mape = float(ensemble_metrics.get("mape", np.nan))
    # V16 global guardrail: do not break V15/V14 band (MAPE soft ceiling 0.131 preferred)
    pass_global_guardrail = (mape <= 0.131) and (r2 >= 0.670)
    ...
    pass_basiskele_lift = bool(
        pd.notna(basiskele_r2) and basiskele_r2 >= float(V15_DEFAULT_REF["basiskele_r2"]) - 1e-9
    )
    pass_basiskele_variance_lift = bool(
        pd.notna(basiskele_var) and basiskele_var >= float(V15_DEFAULT_REF["basiskele_variance_ratio"]) - 1e-9
    )
    ...
    pass_basiskele_spread_lift = bool(
        ...
        or (
            ...
            and float(basiskele_var) > float(V15_DEFAULT_REF["basiskele_variance_ratio"]) + 1e-4
        )
    )
    ...
    pass_karamursel_lift = (not pd.notna(karamursel_r2)) or (
        karamursel_r2 >= float(V15_DEFAULT_REF["karamursel_r2"]) - 1e-9
    )
    ...
    if not pass_global_guardrail:
        ...
        qa.append({"finding": "Global MAPE/R2 outside V16 guardrail (R2>=0.670, MAPE<=0.131)", "severity": "High"})
        top_risks.append("Global metrics outside V16 guardrail")
    if not pass_basiskele_lift:
        warnings.append("basiskele_no_r2_lift")
        qa.append(
            {
                "finding": f"Başiskele R2 {basiskele_r2:.4f} below V15 {V15_DEFAULT_REF['basiskele_r2']}",
                "severity": "Medium",
            }
        )
    else:
        top_opportunities.append(f"Başiskele R2 {basiskele_r2:.4f} (vs V15 {V15_DEFAULT_REF['basiskele_r2']})")
    if not pass_basiskele_variance_lift:
        warnings.append("basiskele_variance_no_lift")
        qa.append(
            {
                "finding": f"Başiskele variance ratio {basiskele_var} below V15 {V15_DEFAULT_REF['basiskele_variance_ratio']}",
                "severity": "Medium",
            }
        )
    ...
    if not pass_karamursel_lift:
        ...
        qa.append(
            {
                "finding": f"Karamürsel R2 {karamursel_r2:.4f} below V15 {V15_DEFAULT_REF['karamursel_r2']}",
                "severity": "Medium",
            }
        )
        top_risks.append("Karamürsel R2 regression vs V15")
    ...
    if str(lh_rep.get("status", "")) not in {"", "applied"} and str(
        ensemble_metrics.get("basiskele_large_home_regime", "")
    ).lower() == "residual":
        disabled_layers.append(f"large_home_residual:{lh_rep.get('status')}")
    if str(spread_rep.get("status", "")) not in {"", "applied", "disabled"} and str(
        ensemble_metrics.get("basiskele_spread_layer", "none")
    ).lower() not in {"", "none"}:
        if str(spread_rep.get("status")) != "applied":
            disabled_layers.append(f"spread_residual:{spread_rep.get('status')}")
    ...
    return {
        ...
        "basiskele_lift": {
            ...
            "variance_delta_vs_v15": (
                (basiskele_var - float(V15_DEFAULT_REF["basiskele_variance_ratio"]))
        ...
        "v15_delta": {
            ...
            "basiskele_large_home_r2": (
                (float(lh_r2_after) - float(V15_DEFAULT_REF.get("basiskele_large_home_r2", 0.2396)))
                if lh_r2_after is not None and pd.notna(lh_r2_after)
                else np.nan
            ),
            "karamursel_r2": (karamursel_r2 - float(V15_DEFAULT_REF["karamursel_r2"]))
            if pd.notna(karamursel_r2)
            else np.nan,
            "golcuk_r2": (golcuk_r2 - float(V15_DEFAULT_REF["golcuk_r2"])) if pd.notna(golcuk_r2) else np.nan,
            "izmit_r2": (izmit_r2 - float(V15_DEFAULT_REF["izmit_r2"])) if pd.notna(izmit_r2) else np.nan,
        },
        "v12_delta": {
