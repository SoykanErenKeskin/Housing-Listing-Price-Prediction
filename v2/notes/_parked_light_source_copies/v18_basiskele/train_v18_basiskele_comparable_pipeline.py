...
Example:
  python train_v18_basiskele_comparable_pipeline.py --out outputs/v18_basiskele_full \\
...
if __name__ == "__main__":
    _v18_dir = str(Path(__file__).resolve().parent)
    if _v18_dir not in sys.path:
        sys.path.insert(0, _v18_dir)
    sys.stdout.write("V18 Başiskele ayar sihirbazı açılıyor...\n")
    sys.stdout.flush()
...
try:
    ...
    # Prefer local v18_basiskele/.env next to this file (cwd-independent).
...
def train_and_evaluate(X: pd.DataFrame, y: pd.Series, cfg: RunConfig, out_dirs: dict[str, Path]) -> tuple[ModelBundle, dict[str, Any]]:
    ...
    metrics_summary["selected_comparable_mode"] = comp_mode
...
def parse_args() -> argparse.Namespace:
    ...
    ap = argparse.ArgumentParser(
        description="V18 Başiskele-only comparable location pipeline. "
    ...
    ap.add_argument(
        ...
        default="full",
        help="Comparable market feature pack.",
    )
    ap.add_argument(
...
def main() -> None:
    ...
    cfg = RunConfig(
        ...
        comparable_mode=str(getattr(args, "comparable_mode", "full")),
    ...
    (out_dirs["base"] / "run_config_v18_basiskele.json").write_text(json.dumps(asdict(cfg), indent=2, ensure_ascii=False), encoding="utf-8")
