"""V17 sensitivity / coverage / decision / geo-context diagnostics."""
...
try:
    ...
    from v17.detail_premium_features import V13_DEFAULT_REF, V14_DEFAULT_REF, V15_DEFAULT_REF

# V16 full-run reference (regime ablation winner / published baseline)
V16_DEFAULT_REF = {
    "r2": 0.6803,
    "mape": 0.1285,
    "basiskele_r2": 0.4402,
...
def run_prediction_pair_tests(bundle: Any, X: pd.DataFrame, reports_dir) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    pair.to_csv(reports_dir / "prediction_pair_tests_v17.csv", index=False, encoding="utf-8-sig")
    pair.to_csv(reports_dir / "feature_sensitivity_v17.csv", index=False, encoding="utf-8-sig")
    county = (
    ...
    county.to_csv(reports_dir / "county_feature_sensitivity_v17.csv", index=False, encoding="utf-8-sig")
...
def run_karamursel_sensitivity(bundle: Any, X: pd.DataFrame, reports_dir) -> dict[str, Any]:
    ...
    rep.to_csv(reports_dir / "karamursel_sensitivity_v17.csv", index=False, encoding="utf-8-sig")
...
def run_basiskele_variance_diagnostics(
    ...
    pd.DataFrame([{k: v for k, v in row.items() if k != "decile_bias_json"}]).to_csv(
        reports_dir / "basiskele_variance_diagnostics_v17.csv", index=False, encoding="utf-8-sig"
    )
    return row
...
def attribute_feature_coverage(X: pd.DataFrame, reports_dir) -> pd.DataFrame:
    ...
    rep.to_csv(reports_dir / "attribute_feature_coverage_v17.csv", index=False, encoding="utf-8-sig")
    return rep
...
def save_attribute_feature_importance(models: dict[str, Any], reports_dir) -> None:
    ...
    rep.to_csv(reports_dir / "attribute_feature_importance_v17.csv", index=False, encoding="utf-8-sig")
...
def save_detail_premium_feature_importance(models: dict[str, Any], reports_dir, county: str | None = None) -> None:
    ...
    rep = pd.DataFrame(rows).sort_values("importance", ascending=False)
    rep["rank"] = range(1, len(rep) + 1)
    rep.to_csv(reports_dir / "detail_premium_feature_importance_v17.csv", index=False, encoding="utf-8-sig")
...
def write_geo_context_coverage_report(X: pd.DataFrame, reports_dir) -> pd.DataFrame:
    ...
    out = _P(reports_dir)
    df.to_csv(out / "geo_context_coverage_v17.csv", index=False, encoding="utf-8-sig")
    return df
...
def write_basiskele_geo_context_diagnostics(
    ...
    if oof.empty or "county" not in oof.columns:
        pd.DataFrame().to_csv(out / "basiskele_geo_context_diagnostics_v17.csv", index=False)
        return pd.DataFrame()
    ...
    df = pd.DataFrame(rows)
    df.to_csv(out / "basiskele_geo_context_diagnostics_v17.csv", index=False, encoding="utf-8-sig")
    return df
