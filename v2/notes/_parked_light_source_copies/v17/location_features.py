...
def compute_county_lat_lon_coverage(df: pd.DataFrame) -> dict[str, float]:
    ...
    lat = pd.to_numeric(df.get("latitude", df.get("lat", np.nan)), errors="coerce")
    lon = pd.to_numeric(df.get("longitude", df.get("lon", np.nan)), errors="coerce")
    has = lat.notna() & lon.notna()
    out: dict[str, float] = {}
    for c, idx in county.groupby(county).groups.items():
        n = len(idx)
        out[str(c)] = float(has.loc[idx].mean()) if n else 0.0
    return out
